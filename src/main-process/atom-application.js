...
module.exports = class AtomApplication extends EventEmitter {
  ...
  handleEvents() {
    ...
    this.on('application:inspect', ({ x, y, atomWindow }) => {
      ...
      if (atomWindow)
        atomWindow.browserWindow.inspectElement(Math.round(x), Math.round(y));
    });
  ...
  async openPaths({
    ...
    if (!createNewWindow) {
      ...
      if (!existingWindow && hasNonEmptyPath) {
        existingWindow = this.windowForLocations(locationsToOpen, devMode);
      }
      ...
      if (!existingWindow && addToLastWindow) {
        existingWindow = this.getLastFocusedWindow((win) => {
          return !win.isSpec && win.devMode === devMode;
        });
      }
