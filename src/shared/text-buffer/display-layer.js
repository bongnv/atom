...
module.exports = class DisplayLayer {
  constructor(id, buffer, params = {}) {
    ...
    this.showIndentGuides =
      params.showIndentGuides != null ? params.showIndentGuides : false;
    this.ratioForCharacter =
      params.ratioForCharacter != null ? params.ratioForCharacter : unitRatio;
    this.isWrapBoundary =
      params.isWrapBoundary != null ? params.isWrapBoundary : isWordStart;
    this.foldCharacter =
      params.foldCharacter != null ? params.foldCharacter : '⋯';
    this.atomicSoftTabs =
    ...
    if (params.spatialIndex) {
      ...
      this.rightmostScreenPosition = new Point(0, 0);
  ...
  doBackgroundWork(deadline) {
    this.populateSpatialIndexIfNeeded(
  ...
  bufferDidChangeLanguageMode() {
    this.cachedScreenLines.length = 0;
    if (this.languageModeDisposable) this.languageModeDisposable.dispose();
    this.languageModeDisposable =
      this.buffer.languageMode.onDidChangeHighlighting((bufferRange) => {
        bufferRange = Range.fromObject(bufferRange);
        this.populateSpatialIndexIfNeeded(bufferRange.end.row + 1, Infinity);
        const startBufferRow = this.findBoundaryPrecedingBufferRow(
          bufferRange.start.row
        );
        const endBufferRow = this.findBoundaryFollowingBufferRow(
          bufferRange.end.row + 1
        );
        ...
        const endRow = this.translateBufferPositionWithSpatialIndex(
          new Point(endBufferRow, 0),
          'backward'
        ).row;
        const extent = new Point(endRow - startRow, 0);
        spliceArray(
          this.cachedScreenLines,
          startRow,
          extent.row,
  ...
  addMarkerLayer(options) {
    const markerLayer = new DisplayMarkerLayer(
      this,
      this.buffer.addMarkerLayer(options),
      true
    );
  ...
  getMarkerLayer(id) {
    if (this.displayMarkerLayersById.has(id)) {
      ...
      if (bufferMarkerLayer) {
        const displayMarkerLayer = new DisplayMarkerLayer(
          this,
          bufferMarkerLayer,
          false
        );
  ...
  foldBufferRange(bufferRange) {
    bufferRange = Range.fromObject(bufferRange);
    const containingFoldMarkers = this.foldsMarkerLayer.findMarkers({
  ...
  destroyFoldsContainingBufferPositions(bufferPositions, excludeEndpoints) {
    ...
    for (const position of bufferPositions) {
      ...
      for (const marker of foundMarkers) {
        if (
          !excludeEndpoints ||
          marker.getRange().containsPoint(clippedPosition, true)
        ) {
          markersContainingPositions.add(marker);
        }
      }
    }
    const sortedMarkers = Array.from(markersContainingPositions).sort((a, b) =>
      a.compare(b)
    );
  ...
  translateBufferPosition(bufferPosition, options) {
    ...
    if (columnDelta !== 0) {
      bufferPosition = new Point(
        bufferPosition.row,
        bufferPosition.column + columnDelta
      );
    }
    ...
    if (tabCount > 0) {
      screenPosition = this.expandHardTabs(
        screenPosition,
        bufferPosition,
        tabCount
      );
      ...
      if (compare(bufferPosition, hunk.oldEnd) < 0) {
        if (compare(hunk.oldStart, bufferPosition) === 0) {
          ...
          if (clipDirection === 'backward') {
            ...
            const distanceFromFoldStart = traversal(
              bufferPosition,
              hunk.oldStart
            );
  ...
  translateScreenPosition(screenPosition, options) {
    ...
    this.populateSpatialIndexIfNeeded(
      this.buffer.getLineCount(),
      screenPosition.row + 1
    );
    screenPosition = this.constrainScreenPosition(
      screenPosition,
      clipDirection
    );
    const tabCount = this.tabCounts[screenPosition.row];
    if (tabCount > 0) {
      screenPosition = this.collapseHardTabs(
        screenPosition,
        tabCount,
        clipDirection
      );
    ...
    const columnDelta = this.getClipColumnDelta(bufferPosition, clipDirection);
    if (columnDelta !== 0) {
      return new Point(bufferPosition.row, bufferPosition.column + columnDelta);
    } else {
        ...
        if (this.isSoftWrapHunk(hunk)) {
          if (
            ...
            (clipDirection === 'closest' &&
              isEqual(hunk.newStart, screenPosition))
          ) {
            return this.translateScreenPositionWithSpatialIndex(
              traverse(hunk.newStart, new Point(0, -1)),
              clipDirection,
              skipSoftWrapIndentation
            );
          } else {
  ...
  expandHardTabs(targetScreenPosition, targetBufferPosition, tabCount) {
    const screenRowStart = new Point(targetScreenPosition.row, 0);
    const hunks = this.spatialIndex.getChangesInNewRange(
      screenRowStart,
    ...
    while (tabCount > 0) {
      ...
      if (bufferLine[bufferColumn] === '\t') {
        expandedScreenColumn +=
          this.tabLength - (expandedScreenColumn % this.tabLength);
        tabCount--;
      } else {
        expandedScreenColumn++;
      }
    ...
    expandedScreenColumn +=
      targetScreenPosition.column - unexpandedScreenColumn;
    if (expandedScreenColumn === targetScreenPosition.column) {
      return targetScreenPosition;
    } else {
      return new Point(targetScreenPosition.row, expandedScreenColumn);
    }
  }

  collapseHardTabs(targetScreenPosition, tabCount, clipDirection) {
    const screenRowStart = new Point(targetScreenPosition.row, 0);
    const screenRowEnd = new Point(
    ...
    while (tabCount > 0) {
      ...
      if (
        ...
        if (this.isSoftWrapHunk(nextHunk)) {
          if (hunkIndex !== 0) throw new Error('Unexpected soft wrap hunk');
          unexpandedScreenColumn = Math.min(
            targetScreenPosition.column,
            nextHunk.newEnd.column
          );
          expandedScreenColumn = unexpandedScreenColumn;
        } else {
          ({ row: bufferRow, column: bufferColumn } = nextHunk.oldEnd);
      ...
      if (bufferLine[bufferColumn] === '\t') {
        const nextTabStopColumn =
          expandedScreenColumn +
          this.tabLength -
          (expandedScreenColumn % this.tabLength);
        if (nextTabStopColumn > targetScreenPosition.column) {
          if (clipDirection === 'backward') {
            return new Point(targetScreenPosition.row, unexpandedScreenColumn);
          } else if (clipDirection === 'forward') {
            return new Point(
              targetScreenPosition.row,
              unexpandedScreenColumn + 1
            );
          } else {
            if (
              targetScreenPosition.column >
    ...
    unexpandedScreenColumn +=
      targetScreenPosition.column - expandedScreenColumn;
    if (unexpandedScreenColumn === targetScreenPosition.column) {
      return targetScreenPosition;
    } else {
      return new Point(targetScreenPosition.row, unexpandedScreenColumn);
  ...
  getClipColumnDelta(bufferPosition, clipDirection) {
    ...
    var previousCharacter = this.buffer.getCharacterAtPosition([
      row,
      column - 1,
    ]);
    if (
  ...
  getApproximateScreenLineCount() {
    if (this.indexedBufferRowCount > 0) {
      return Math.floor(
        (this.buffer.getLineCount() * this.screenLineLengths.length) /
  ...
  getScreenLine(screenRow) {
    return (
      this.cachedScreenLines[screenRow] ||
      this.getScreenLines(screenRow, screenRow + 1)[0]
    );
  }

  getScreenLines(screenStartRow = 0, screenEndRow = this.getScreenLineCount()) {
    return this.screenLineBuilder.buildScreenLines(
      screenStartRow,
      screenEndRow
    );
  }
  ...
  bufferRowsForScreenRows(startRow, endRow) {
    ...
    const startPosition = new Point(startRow, 0);
    const bufferRows = [];
    let lastScreenRow = startRow;
    let lastBufferRow =
      this.translateScreenPositionWithSpatialIndex(startPosition).row;
    const hunks = this.spatialIndex.getChangesInNewRange(
      startPosition,
      new Point(endRow, 0)
    );
    for (let i = 0; i < hunks.length; i++) {
      ...
      lastBufferRow = this.isSoftWrapHunk(hunk)
        ? hunk.oldEnd.row
        : hunk.oldEnd.row + 1;
    }
  ...
  leadingWhitespaceLengthForSurroundingLines(startBufferRow) {
    ...
    for (
      let bufferRow = startBufferRow + 1;
      bufferRow < lineCount;
      bufferRow++
    ) {
      const line = this.buffer.lineForRow(bufferRow);
      if (line.length > 0) {
        length = Math.max(
          length,
          this.leadingWhitespaceLengthForNonEmptyLine(line)
        );
        break;
      }
    }
  ...
  findTrailingWhitespaceStartColumn(bufferRow) {
    ...
    for (
      position = {
        row: bufferRow,
        column: this.buffer.lineLengthForRow(bufferRow) - 1,
      };
      position.column >= 0;
      position.column--
    ) {
  ...
  bufferWillChange(change) {
    ...
    while (
      endRow + 1 < lineCount &&
      this.buffer.lineLengthForRow(endRow + 1) === 0
    ) {
      endRow++;
    }
  ...
  bufferDidChange({ oldRange, newRange }) {
    ...
    this.didChange(
      this.updateSpatialIndex(startRow, oldEndRow + 1, newEndRow + 1, Infinity)
    );
  }
  ...
  updateSpatialIndex(
    ...
    const folds = this.computeFoldsInBufferRowRange(
      startBufferRow,
      newEndBufferRow
    );
    ...
    let rightmostInsertedScreenPosition = new Point(0, -1);
    ...
    while (true) {
      ...
      while (bufferColumn <= bufferLineLength) {
        ...
        const character = foldEnd
          ? this.foldCharacter
          : bufferLine[bufferColumn];
        ...
        if (insertSoftLineBreak) {
          ...
          const unexpandedWrapColumn =
            lastWrapBoundaryUnexpandedScreenColumn || unexpandedScreenColumn;
          const expandedWrapColumn =
            lastWrapBoundaryExpandedScreenColumn || expandedScreenColumn;
          const wrapWidth = lastWrapBoundaryScreenLineWidth || screenLineWidth;
          this.spatialIndex.splice(
            new Point(screenRow, unexpandedWrapColumn),
            Point.ZERO,
            new Point(1, indentLength)
          );
          ...
          for (let i = 0; i < currentScreenLineTabColumns.length; i++) {
            ...
            if (tabColumn < unexpandedWrapColumn) {
              ...
              const tabColumnAfterWrap =
                indentLength + tabColumn - unexpandedWrapColumn;
              expandedScreenColumnAfterLastTab +=
                tabColumnAfterWrap - unexpandedScreenColumnAfterLastTab;
              expandedScreenColumnAfterLastTab +=
                this.tabLength -
                (expandedScreenColumnAfterLastTab % this.tabLength);
              unexpandedScreenColumnAfterLastTab = tabColumnAfterWrap + 1;
              currentScreenLineTabColumns[i - tabCountPrecedingWrap] =
                tabColumnAfterWrap;
            }
          }
          ...
          unexpandedScreenColumn =
            unexpandedScreenColumn - unexpandedWrapColumn + indentLength;
          expandedScreenColumn =
            expandedScreenColumnAfterLastTab +
            unexpandedScreenColumn -
            unexpandedScreenColumnAfterLastTab;
          screenLineWidth =
            indentLength * this.ratioForCharacter(' ') +
        ...
        if (foldEnd) {
          ...
          if (character === '\t') {
            ...
            screenLineWidth +=
              distanceToNextTabStop * this.ratioForCharacter(' ');
          } else {
            expandedScreenColumn++;
            screenLineWidth += characterWidth;
          }
    ...
    if (
      rightmostInsertedScreenPosition.column >
      this.rightmostScreenPosition.column
    ) {
      ...
      this.rightmostScreenPosition.row +=
        insertedScreenLineLengths.length - oldScreenRowCount;
    } else if (startScreenRow <= this.rightmostScreenPosition.row) {
      this.rightmostScreenPosition = new Point(0, 0);
      for (
        let row = 0, rowCount = this.screenLineLengths.length;
        row < rowCount;
        row++
      ) {
    ...
    return {
      start: new Point(startScreenRow, 0),
      oldExtent: new Point(oldScreenRowCount, 0),
      newExtent: new Point(insertedScreenLineLengths.length, 0),
    };
  }

  populateSpatialIndexIfNeeded(
    endBufferRow,
    endScreenRow,
    deadline = NullDeadline
  ) {
    ...
    if (
      endBufferRow > this.indexedBufferRowCount &&
      endScreenRow > this.screenLineLengths.length
    ) {
      this.updateSpatialIndex(
        this.indexedBufferRowCount,
        endBufferRow,
  ...
  findBoundaryPrecedingBufferRow(bufferRow) {
    while (true) {
      ...
      let bufferPosition = this.translateScreenPositionWithSpatialIndex(
        new Point(screenPosition.row, 0),
        'backward',
        false
      );
  ...
  findBoundaryFollowingBufferRow(bufferRow) {
    while (true) {
      ...
      if (screenPosition.column === 0) {
        ...
        const endOfScreenRow = new Point(
          screenPosition.row,
          this.screenLineLengths[screenPosition.row]
        );
        bufferRow =
          this.translateScreenPositionWithSpatialIndex(
            endOfScreenRow,
            'forward',
            false
          ).row + 1;
    ...
    if (endBufferRow >= this.indexedBufferRowCount) {
      for (let i = 0; i < foldMarkers.length; i++) {
        ...
        if (
          ...
          (!nextMarker ||
            nextMarker.getEndPosition().row < marker.getEndPosition().row)
        ) {
  ...
  setParams(params) {
    ...
    if (
      params.hasOwnProperty('tabLength') &&
      params.tabLength !== this.tabLength
    ) {
      paramsChanged = true;
    ...
    if (
      params.hasOwnProperty('isWrapBoundary') &&
      params.isWrapBoundary !== this.isWrapBoundary
    ) {
      paramsChanged = true;
      this.isWrapBoundary = params.isWrapBoundary;
    }
    if (
      params.hasOwnProperty('foldCharacter') &&
      params.foldCharacter !== this.foldCharacter
    ) {
      paramsChanged = true;
      this.foldCharacter = params.foldCharacter;
    }
    if (
      params.hasOwnProperty('atomicSoftTabs') &&
      params.atomicSoftTabs !== this.atomicSoftTabs
    ) {
      paramsChanged = true;
