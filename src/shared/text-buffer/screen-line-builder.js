...
module.exports = class ScreenLineBuilder {
  ...
  buildScreenLines(startScreenRow, endScreenRow) {
    ...
    this.bufferPosition = {
      row: this.displayLayer.findBoundaryPrecedingBufferRow(
        this.displayLayer.translateScreenPositionWithSpatialIndex(
    ...
    this.screenRow = this.displayLayer.translateBufferPositionWithSpatialIndex(
      new Point(this.bufferPosition.row, 0)
    ).row;
    ...
    const decorationIterator =
      this.displayLayer.buffer.languageMode.buildHighlightIterator();
    const hunks = this.displayLayer.spatialIndex.getChangesInNewRange(
      new Point(this.screenRow, 0),
      new Point(endScreenRow, 0)
    );
    let hunkIndex = 0;
    ...
    screenRowLoop: while (this.screenRow < endScreenRow) {
      var cachedScreenLine =
        this.displayLayer.cachedScreenLines[this.screenRow];
      if (cachedScreenLine) {
      ...
      this.bufferLineLength = this.displayLayer.buffer.lineLengthForRow(
        this.bufferPosition.row
      );

      if (this.bufferPosition.row > this.displayLayer.buffer.getLastRow())
        break;
      ...
      if (
        ...
        this.scopeIdsToReopen =
          decorationIterator.seek(this.bufferPosition, endBufferRow) || [];
      }

      var prevCachedScreenLine =
        this.displayLayer.cachedScreenLines[this.screenRow - 1];
      ...
      while (this.bufferPosition.column <= this.bufferLineLength) {
        ...
        var nextCharacter = this.displayLayer.buffer.getCharacterAtPosition(
          this.bufferPosition
        );
        ...
        if (this.emitBuiltInTagBoundary) {
          this.emitOpenTag(
            this.getBuiltInScopeId(this.currentBuiltInClassNameFlags)
          );
        }
  ...
  updateCurrentTokenFlags(nextCharacter) {
    ...
    if (nextCharacter === ' ' || nextCharacter === '\t') {
      ...
      if (this.inLeadingWhitespace)
        this.currentBuiltInClassNameFlags |= LEADING_WHITESPACE;
      if (this.inTrailingWhitespace)
        this.currentBuiltInClassNameFlags |= TRAILING_WHITESPACE;
      ...
      if (nextCharacter === ' ') {
        ...
        if (
          showIndentGuides &&
          this.screenColumn % this.displayLayer.tabLength === 0
        ) {
          this.currentBuiltInClassNameFlags |= INDENT_GUIDE;
        }
    ...
    if (!this.emitBuiltInTagBoundary) {
      this.emitBuiltInTagBoundary =
        this.currentBuiltInClassNameFlags !== previousBuiltInTagFlags;
    }
  }
  ...
  emitFold(nextHunk, decorationIterator, endBufferRow) {
    ...
    this.scopeIdsToReopen = decorationIterator.seek(
      this.bufferPosition,
      endBufferRow
    );
  ...
  emitLineEnding() {
    ...
    if (this.bufferLineLength === 0 && this.displayLayer.showIndentGuides) {
      let whitespaceLength =
        this.displayLayer.leadingWhitespaceLengthForSurroundingLines(
          this.bufferPosition.row
        );
    ...
    // the caller
    if (this.currentScreenLineTags.length === 0)
  ...
  emitHardTab() {
    const distanceToNextTabStop =
      this.displayLayer.tabLength -
      (this.screenColumn % this.displayLayer.tabLength);
    if (this.displayLayer.invisibles.tab) {
  ...
  emitEmptyTokenIfNeeded() {
    const lastTag =
      this.currentScreenLineTags[this.currentScreenLineTags.length - 1];
  ...
  emitCloseTag(scopeId) {
    ...
    while ((containingScopeId = this.containingScopeIds.pop())) {
      this.currentScreenLineTags.push(
        this.displayLayer.closeTagForScopeId(containingScopeId)
      );
  ...
  emitOpenTag(scopeId, reopenTags = true) {
    ...
    if (scopeId > 0) {
      ...
      this.currentScreenLineTags.push(
        this.displayLayer.openTagForScopeId(scopeId)
      );
    }
  }
  ...
  reopenTags() {
    for (let i = 0, n = this.scopeIdsToReopen.length; i < n; i++) {
      ...
      this.currentScreenLineTags.push(
        this.displayLayer.openTagForScopeId(scopeIdToReopen)
      );
    }
    this.scopeIdsToReopen.length = 0;
  }
  ...
  compareBufferPosition(position) {
    ...
    return rowComparison === 0
      ? this.bufferPosition.column - position.column
      : rowComparison;
  }
};
