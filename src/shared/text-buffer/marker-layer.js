...
module.exports = MarkerLayer = class MarkerLayer {
  ...
  static deserializeSnapshot(snapshot) {
    ...
    for (var layerId in snapshot) {
      var markerSnapshots = snapshot[layerId];
      result[layerId] = {};
      for (var markerId in markerSnapshots) {
        var markerSnapshot = markerSnapshots[markerId];
        result[layerId][markerId] = clone(markerSnapshot);
        result[layerId][markerId].range = Range.fromObject(
          markerSnapshot.range
        );
      }
    }
    return result;
  ...
  */
  ...
  constructor(delegate, id, options) {
    ...
    this.maintainHistory =
      (options != null ? options.maintainHistory : undefined) != null
        ? options != null
          ? options.maintainHistory
          : undefined
        : false;
    this.destroyInvalidatedMarkers =
      (options != null ? options.destroyInvalidatedMarkers : undefined) != null
        ? options != null
          ? options.destroyInvalidatedMarkers
          : undefined
        : false;
    ...
    this.persistent =
      (options != null ? options.persistent : undefined) != null
        ? options != null
          ? options.persistent
          : undefined
        : false;
  ...
  // locations.
  copy() {
    const copy = this.delegate.addMarkerLayer({
      maintainHistory: this.maintainHistory,
      role: this.role,
    });
    for (var markerId in this.markersById) {
      var marker = this.markersById[markerId];
      var snapshot = marker.getSnapshot(null);
  ...
  destroy() {
    ...
    this.displayMarkerLayers.forEach((displayMarkerLayer) =>
  ...
  clear() {
    ...
    this.markersById = {};
    this.index = new MarkerIndex();
    this.displayMarkerLayers.forEach((layer) =>
      layer.didClearBufferMarkerLayer()
    );
    ...
    for (var key of Object.keys(params)) {
      ...
      switch (key) {
        ...
        case 'startsInRange':
          ...
          markerIds = filterSet(
            markerIds,
            this.index.findStartingIn(start, end)
          );
          break;
        case 'endsInRange':
        ...
        case 'containsPosition':
          ...
          markerIds = filterSet(
            markerIds,
            this.index.findContaining(position, position)
          );
          break;
        case 'containsRange':
          ({ start, end } = Range.fromObject(value));
          markerIds = filterSet(
            markerIds,
            this.index.findContaining(start, end)
          );
          break;
        case 'intersectsRange':
          ...
          markerIds = filterSet(
            markerIds,
            this.index.findIntersecting(start, end)
          );
          break;
        case 'startRow':
          markerIds = filterSet(
            markerIds,
            this.index.findStartingIn(
              new Point(value, 0),
        ...
        case 'containedInRange':
          ...
          markerIds = filterSet(
            markerIds,
            this.index.findContainedIn(start, end)
          );
          break;
        default:
          continue;
  ...
  */
  ...
  markPosition(position, options = {}) {
    ...
    return this.createMarker(
      this.delegate.clipRange(new Range(position, position)),
      options
    );
  }
  ...
  */
  ...
  splice(start, oldExtent, newExtent) {
    ...
    return invalidated.touch.forEach((id) => {
      ...
      if (
        __guard__(invalidated[marker.getInvalidationStrategy()], (x) =>
          x.has(id)
        )
      ) {
    ...
    for (id of snapshotIds) {
      ...
      if ((marker = this.markersById[id])) {
        ...
        if (marker) {
          ...
          var newMarker = this.createMarker(snapshot.range, snapshot, true);
        }
      }
    }
  ...
  createSnapshot() {
    ...
    for (var id of Object.keys(this.markersById)) {
      var marker = this.markersById[id];
  ...
  emitChangeEvents(snapshot) {
    return this.markersWithChangeListeners.forEach(function (marker) {
      if (!marker.isDestroyed()) {
        ...
        return marker.emitChangeEvent(
          __guard__(
            snapshot != null ? snapshot[marker.id] : undefined,
            (x) => x.range
          ),
          true,
          false
  ...
  serialize() {
    ...
    const ranges = this.index.dump();
    const markersById = {};
    for (id of Object.keys(this.markersById)) {
      var marker = this.markersById[id];
      var snapshot = marker.getSnapshot(Range.fromObject(ranges[id]), false);
  ...
  deserialize(state) {
    ...
    for (var id in state.markersById) {
      var markerState = state.markersById[id];
      var range = Range.fromObject(markerState.range);
  ...
  */
  ...
  createMarker(range, params, suppressMarkerLayerUpdateEvents = false) {
    ...
    marker.trackDestruction =
      this.trackDestructionInOnDidCreateMarkerCallbacks != null
        ? this.trackDestructionInOnDidCreateMarkerCallbacks
        : false;
    if (this.emitCreateMarkerEvents) {
      this.emitter.emit('did-create-marker', marker);
    }
