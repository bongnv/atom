...
class TextBuffer {
  ...
  */
  ...
  constructor(params) {
    ...
    this.maxUndoEntries =
      params.maxUndoEntries != null
        ? params.maxUndoEntries
    ...
    this.defaultMarkerLayer = new MarkerLayer(
      this,
      String(this.nextMarkerLayerId++)
    );
    this.displayLayers = {};
    this.markerLayers = {};
    ...
    this.shouldDestroyOnFileDelete =
      params.shouldDestroyOnFileDelete || (() => false);
  ...
  static async deserialize(params) {
    ...
    for (const layerId in params.displayLayers) {
      ...
      buffer.displayLayers[layerId] = DisplayLayer.deserialize(
        buffer,
        layerState
      );
    }
  ...
  */
    ...
    this.transact(() => {
      ...
      for (let change of lineDiff) {
        ...
        if (change.added) {
          ...
          this.setTextInRange(
            [currentPosition, [endRow, endColumn]],
            '',
            changeOptions
          );
        } else {
  ...
  applyChange(change, pushToHistory = false) {
    ...
    const oldRange = new Range(newStart, traverse(newStart, oldExtent));
    oldRange.freeze();

    const newExtent = traversal(newEnd, newStart);
    const newRange = new Range(newStart, traverse(newStart, newExtent));
    newRange.freeze();
    ...
    if (endRow < lastRow) {
      ...
      if (startRow === 0) {
        ...
        startPoint = new Point(
          startRow - 1,
          this.lineLengthForRow(startRow - 1)
        );
      }
  ...
  */
  ...
  getHistory(maxEntries) {
    ...
    for (let i = snapshot.undoStackChanges.length - 1; i >= 0; i--) {
      ...
      const newRange = new Range(change.newStart, change.newEnd);
  ...
  undo(options) {
    ...
    this.restoreFromMarkerSnapshot(
      pop.markers,
      options && options.selectionsMarkerLayer
    );
  ...
  createCheckpoint(options) {
    return this.historyProvider.createCheckpoint({
      markers: this.createMarkerSnapshot(
        options != null ? options.selectionsMarkerLayer : undefined
      ),
      isBarrier: false,
    });
  }
  ...
  */
    ...
    for (let i = startIndex; i !== endIndex; i += increment) {
      ...
      if (!reverse && argument.replacementText != null) {
        replacementColumnDelta +=
          matchRange.start.column +
          argument.replacementText.length -
  ...
  findWordsWithSubsequence(query, extraWordCharacters, maxCount) {
    return this.buffer.findWordsWithSubsequence(
      query,
      extraWordCharacters,
      maxCount
    );
  ...
  */
  ...
  rangeForRow(row, includeNewline) {
    ...
    if (includeNewline && row < this.getLastRow()) {
      ...
      return new Range(
        new Point(row, 0),
        new Point(row, this.lineLengthForRow(row))
      );
    }
  }
        ...
        if (isWindows) {
          ...
          try {
            await this.buffer.save(
              fsAdmin.createWriteStream(filePath),
              this.getEncoding()
            );
          } catch (_) {
            throw error;
  ...
  */
  ...
  finishLoading(checkpoint, patch, options) {
    if (
      this.isDestroyed() ||
      (this.loaded && checkpoint == null && patch != null)
    ) {
  ...
  destroy() {
    ...
    this.cachedText = null;
    if (typeof this.historyProvider.clear === 'function')
            ...
            if (this.isModified()) {
              const source =
                this.file instanceof File
                  ...
                  : this.file.createReadStream();
              if (
                !(await this.buffer.baseTextMatchesFile(
                  source,
                  this.getEncoding()
                ))
              ) {
  ...
  emitMarkerChangeEvents(snapshot) {
    if (this.transactCallDepth === 0) {
      while (this.markerLayersWithPendingUpdateEvents.size > 0) {
        const updatedMarkerLayers = Array.from(
          this.markerLayersWithPendingUpdateEvents
        );
...
class SearchCallbackArgument {
  ...
  static addContextLines(argument, options) {
    ...
    let row = Math.max(
      0,
      argument.range.start.row - (options.leadingContextLineCount || 0)
    );
