...
class Directory {
  ...
  constructor(directoryPath, symlink = false) {
    ...
    if (directoryPath) {
      ...
      if (
        ...
        directoryPath[directoryPath.length - 1] === path.sep
      ) {
      ...
      return this.getParent()
        ...
        .then(() => {
          return new Promise((resolve, reject) => {
            return fs.mkdir(this.getPath(), mode, (error) => {
              if (error) {
                return reject(error);
              } else {
                return resolve(true);
  ...
  */
  ...
  getEntriesSync() {
    ...
    for (let entryPath of fs.listSync(this.path)) {
      ...
      try {
        ...
        // TODO: bongnv - explain why we need to ignore errors here.
      }
  ...
  getEntries(callback) {
    return fs.list(this.path, (error, entries) => {
      if (error != null) {
      ...
      const directories = [];
      const files = [];
      const addEntry = (entryPath, stat, symlink, callback) => {
        if (stat?.isDirectory()) {
          directories.push(new Directory(entryPath, symlink));
        } else if (stat?.isFile()) {
          files.push(new File(entryPath, symlink));
      ...
      const statEntry = (entryPath, callback) =>
        fs.lstat(entryPath, (error, stat) => {
          if (stat?.isSymbolicLink()) {
            return fs.stat(entryPath, (error, stat) =>
      ...
      return async.eachLimit(entries, 1, statEntry, () =>
  ...
  */
  ...
  isPathPrefixOf(prefix, fullPath) {
    return (
      fullPath.indexOf(prefix) === 0 && fullPath[prefix.length] === path.sep
    );
  }
}
