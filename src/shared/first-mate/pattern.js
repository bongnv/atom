...
module.exports = Pattern = class Pattern {
  constructor(grammar, registry, options = {}) {
    ...
    const { captures, beginCaptures, endCaptures, applyEndPatternLast } =
      options;
    ({
      include: this.include,
      popRule: this.popRule,
      hasBackReferences: this.hasBackReferences,
    } = options);
    ...
    if (match) {
      ...
      const endPattern = this.grammar.createPattern({
        match: end,
    ...
    if (this.captures != null) {
      for (var group in this.captures) {
        var capture = this.captures[group];
        if (
          (capture.patterns != null ? capture.patterns.length : undefined) >
            0 &&
          !capture.rule
        ) {
  ...
  hasAnchor() {
    ...
    let escape = false;
    for (var character of this.regexSource) {
      if (escape && __in__(character, ['A', 'G', 'z'])) {
        return true;
      }
  ...
  resolveBackReferences(line, beginCaptureIndices) {
    ...
    for (var { start, end } of beginCaptureIndices) {
  ...
  ruleForInclude(baseGrammar, name) {
    ...
    if (hashIndex === 0) {
      ...
      return __guard__(
        this.registry.grammarForScopeName(grammarName),
        (x) => x.getRepository()[ruleName]
      );
    } else if (name === '$self') {
      ...
      return __guard__(this.registry.grammarForScopeName(name), (x1) =>
        x1.getInitialRule()
      );
    }
  }
  ...
  resolveScopeName(scopeName, line, captureIndices) {
    ...
    return (resolvedScopeName = scopeName.replace(
      AllCustomCaptureIndicesRegex,
      function (match, index, commandIndex, command) {
        const capture =
          captureIndices[parseInt(index != null ? index : commandIndex)];
        if (capture != null) {
  ...
  handleMatch(stack, line, captureIndices, rule, endPatternMatch) {
    ...
    if (this.captures) {
      tags.push(
        ...this.tagsForCaptureIndices(
          line,
          captureIndices.slice(),
          captureIndices,
          stack
        )
      );
    ...
    if (this.pushRule) {
      ...
      if (contentScopeName) {
        contentScopeName = this.resolveScopeName(
          contentScopeName,
          line,
          captureIndices
        );
  ...
  tagsForCaptureRule(rule, line, captureStart, captureEnd, stack) {
    ...
    const { tags } = rule.grammar.tokenizeLine(
      captureText,
      [...stack, { rule }],
      false,
      true,
  ...
  tagsForCaptureIndices(line, currentCaptureIndices, allCaptureIndices, stack) {
    ...
    if (
      (scope =
        this.captures[parentCapture.index] != null
          ? this.captures[parentCapture.index].name
          : undefined)
    ) {
      parentCaptureScope = this.resolveScopeName(
        scope,
        line,
        allCaptureIndices
      );
    ...
    if (
      (captureRule =
        this.captures[parentCapture.index] != null
          ? this.captures[parentCapture.index].rule
          : undefined)
    ) {
      ...
      while (
        currentCaptureIndices.length &&
        currentCaptureIndices[0].start < parentCapture.end
      ) {
      ...
      while (
        ...
        var childCapture = currentCaptureIndices[0];

        var emptyCapture = childCapture.end - childCapture.start === 0;
        var captureHasNoScope = !this.captures[childCapture.index];
