...
module.exports = GrammarRegistry = class GrammarRegistry {
  constructor(options = {}) {
    this.maxTokensPerLine =
      options.maxTokensPerLine != null ? options.maxTokensPerLine : Infinity;
    this.maxLineLength =
      options.maxLineLength != null ? options.maxLineLength : Infinity;
  ...
  */
  ...
  readGrammarSync(grammarPath) {
    let left;
    const grammar =
      (left = JSON.parse(fs.readFileSync(grammarPath))) != null ? left : {};
  ...
  grammarUpdated(scopeName) {
    for (var grammar of this.grammars) {
  ...
  decodeTokens(lineText, tags, scopeTags = [], fn) {
    ...
    for (let index = 0; index < tags.length; index++) {
      ...
      if (tag >= 0) {
        ...
        scopeTags.pop();
        var expectedScopeName = this.scopeForId(tag + 1);
        var poppedScopeName = scopeNames.pop();
