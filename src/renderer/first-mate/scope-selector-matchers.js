...
class SegmentMatcher {
  ...
  toCssSelector() {
    return this.segment
      .split('.')
      .map((dotFragment) => '.' + dotFragment.replace(/\+/g, '\\+'))
      .join('');
  }
  ...
  toCssSyntaxSelector() {
    return this.segment
      .split('.')
      .map((dotFragment) => '.syntax--' + dotFragment.replace(/\+/g, '\\+'))
      .join('');
  }
}
...
class ScopeMatcher {
  ...
  matches(scope) {
    ...
    let lastDotIndex = 0;
    for (
      matcherSegmentIndex = 0;
      matcherSegmentIndex < this.segments.length;
      matcherSegmentIndex++
    ) {
      var matcherSegment = this.segments[matcherSegmentIndex];
      if (lastDotIndex > scope.length) {
        break;
      }

      var nextDotIndex = scope.indexOf('.', lastDotIndex);
      if (nextDotIndex === -1) {
        nextDotIndex = scope.length;
      }

      var scopeSegment = scope.substring(lastDotIndex, nextDotIndex);
  ...
  toCssSyntaxSelector() {
    return this.segments
      .map((matcher) => matcher.toCssSyntaxSelector())
      .join('');
  }
}
