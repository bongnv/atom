...
const { OnigRegExp, OnigString } = require('napi-oniguruma');
  ...
  Grammar = class Grammar {
    ...
    constructor(registry, options = {}) {
      ...
      ({
        ...
        foldingStopMarker: this.foldingStopMarker,
        maxTokensPerLine: this.maxTokensPerLine,
    ...
    */
    ...
    tokenizeLines(text, compatibilityMode = true) {
      ...
      return (() => {
        ...
        for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
          ...
          ({ tags, ruleStack } = this.tokenizeLine(
            line,
            ruleStack,
            lineNumber === 0,
            compatibilityMode,
    ...
    tokenizeLine(
      ...
      const stringWithNewLine = appendNewLine
        ? new OnigString(line + '\n')
        : string;
      ...
      if (ruleStack != null) {
        ...
        if (compatibilityMode) {
          openScopeTags = [];
          for ({ scopeName, contentScopeName } of ruleStack) {
            if (scopeName) {
              openScopeTags.push(this.registry.startIdForScope(scopeName));
            }
            if (contentScopeName) {
              openScopeTags.push(
                this.registry.startIdForScope(contentScopeName)
              );
            }
          }
        }
      ...
      while (true) {
        var match;
        var previousRuleStackLength = ruleStack.length;
        ...
        if (
          (match = _.last(ruleStack).rule.getNextTags(
            ruleStack,
            string,
            stringWithNewLine,
            position,
          ...
          var { nextTags, tagsStart, tagsEnd } = match;
        ...
        if (position === previousPosition) {
          if (ruleStack.length === previousRuleStackLength) {
            ...
            var popStack;
            var [{ rule: penultimateRule }, { rule: lastRule }] =
              ruleStack.slice(-2);
            ...
            if (
              (lastRule != null ? lastRule.scopeName : undefined) != null &&
            ...
            if (popStack) {
              ruleStack.pop();
              var lastSymbol = _.last(tags);
              if (
                lastSymbol < 0 &&
                lastSymbol === this.startIdForScope(lastRule.scopeName)
              ) {
    ...
    createRepository() {
      const repository = {};
      for (var name in this.rawRepository) {
        var data = this.rawRepository[name];
    ...
    scopesFromStack(stack, rule, endPatternMatch) {
      ...
      const scopes = [];
      for ({ scopeName, contentScopeName } of stack) {
        if (scopeName) {
      ...
      if (
        endPatternMatch &&
        (rule != null ? rule.contentScopeName : undefined) &&
...
class TokenizeLineResult {
  static initClass() {
    Object.defineProperty(this.prototype, 'tokens', {
      get() {
        return this.registry.decodeTokens(
          this.line,
          this.tags,
          this.openScopeTags
        );
