...
class Transaction {
  ...
  shouldGroupWith(previousTransaction) {
    const timeBetweenTransactions =
      this.timestamp - previousTransaction.timestamp;
    return (
...
module.exports = DefaultHistoryProvider = class DefaultHistoryProvider {
  ...
  createCheckpoint(options) {
    const checkpoint = new Checkpoint(
      ...
      options != null ? options.markers : undefined,
      options != null ? options.isBarrier : undefined
    );
  ...
  groupChangesSinceCheckpoint(checkpointId, options) {
    const deleteCheckpoint =
      (options != null ? options.deleteCheckpoint : undefined) != null
        ? options != null
    ...
    for (let i = this.undoStack.length - 1; i >= 0; i--) {
      ...
      switch (entry.constructor) {
        ...
        default:
          throw new Error(
            `Unexpected undo stack entry type: ${entry.constructor.name}`
          );
      }
    }
  ...
  groupLastChanges() {
    ...
    for (let i = this.undoStack.length - 1; i >= 0; i--) {
      ...
      if (patchesSinceCheckpoint.length === 2) {
        var composedPatch = Patch.compose(patchesSinceCheckpoint);
  ...
  enforceUndoStackSizeLimit() {
    if (this.undoStack.length > this.maxUndoEntries) {
      return this.undoStack.splice(
        0,
        this.undoStack.length - this.maxUndoEntries
      );
    }
  }
  ...
  applyGroupingInterval(groupingInterval) {
    ...
    if (
      previousEntry instanceof Transaction &&
      topEntry.shouldGroupWith(previousEntry)
    ) {
      return this.undoStack.splice(
        this.undoStack.length - 2,
        2,
        topEntry.groupWith(previousEntry)
      );
    }
  }
  ...
  undo() {
    ...
    if (spliceIndex != null) {
      this.redoStack.push(
        ...Array.from(this.undoStack.splice(spliceIndex).reverse() || [])
      );
      return {
  ...
  redo() {
    ...
    if (spliceIndex != null) {
      this.undoStack.push(
        ...Array.from(this.redoStack.splice(spliceIndex).reverse() || [])
      );
      return {
        textUpdates: patch.getChanges(),
        markers: snapshotBelow,
  ...
  toString() {
    let output = '';
    for (var entry of this.undoStack) {
      switch (entry.constructor) {
        case Checkpoint:
          output += 'Checkpoint, ';
          break;
  ...
  */
      ...
      for (var entry of stack) {
        switch (entry.constructor) {
          ...
          case Transaction:
            result.push({
              type: 'transaction',
              markerSnapshotBefore: this.serializeSnapshot(
                entry.markerSnapshotBefore,
                options
              ),
              markerSnapshotAfter: this.serializeSnapshot(
                entry.markerSnapshotAfter,
                options
              ),
    ...
    return (() => {
      ...
      for (var entry of stack) {
        switch (entry.type) {
          ...
          default:
            throw new Error(
              `Unexpected undoStack entry type during deserialization: ${entry.type}`
            );
        }
      }
      return result;
  ...
  serializeSnapshot(snapshot, options) {
    ...
    for (var layerId in snapshot) {
      ...
      if (
        !__guard__(this.buffer.getMarkerLayer(layerId), (x) => x.persistent)
      ) {
        continue;
      }
      var serializedMarkerSnapshots = {};
      for (var markerId in layerSnapshot) {
        var markerSnapshot = layerSnapshot[markerId];
        var serializedMarkerSnapshot = Object.assign({}, markerSnapshot);
...
var checkpointFromSnapshot = ({ id, markers }) =>
...
var transactionFromSnapshot = (
  { changes, markersBefore, markersAfter } // TODO: Return raw patch if there's no markersBefore && markersAfter
