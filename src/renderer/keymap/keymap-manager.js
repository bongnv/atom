  ...
  KeymapManager = class KeymapManager {
    ...
    build(
      ...
      for (var selector in keyBindingsBySelector) {
        ...
        for (var keystrokes in keyBindings) {
          ...
          command =
            (left = __guardMethod__(command, 'toString', (o) =>
              o.toString()
            )) != null
              ? left
              : '';
    ...
    findKeyBindings(params = {}) {
      ...
      if (keystrokes != null) {
        bindings = bindings.filter(
          (binding) => binding.keystrokes === keystrokes
        );
      }
      ...
      if (target != null) {
        ...
        while (element != null && element !== document) {
          var matchingBindings = candidateBindings
            .filter((binding) =>
              element.webkitMatchesSelector(binding.selector)
            )
    ...
    filePathMatchesPlatform(filePath) {
      ...
      for (var component of path.basename(filePath).split('.').slice(0, -1)) {
        if (__in__(component, otherPlatforms)) {
          return false;
        }
      }
      return true;
    ...
    handleKeyboardEvent(event, { replay, disabledBindings } = {}) {
      ...
      if (this.pendingPartialMatches != null) {
        const liveMatches = new Set(
          partialMatches.concat(exactMatchCandidates)
        );
        for (binding of this.pendingPartialMatches) {
      ...
      if (exactMatchCandidates.length > 0) {
        ...
        while (
          ...
          for (exactMatchCandidate of exactMatches) {
