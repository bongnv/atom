...
describe('GitHub Flavored Markdown grammar', function () {
  ...
  it('tokenises a ``` code block with an unknown language', function () {
    ...
    expect(tokens[0]).toEqual({
      ...
      scopes: [
        'source.gfm',
        'markup.code.other.gfm',
        'source.embedded.mylanguage',
      ],
    });
  ...
  it('tokenizes a ``` code block with a known language', function () {
    ...
    return expect(ruleStack[1].contentScopeName).toBe(
      'source.embedded.git-config'
    );
  });
  ...
  it('tokenizes [links](links)', function () {
    const { tokens } = grammar.tokenizeLine(
      'please click [this link](website)'
    );
    expect(tokens[0]).toEqual({
      value: 'please click ',
      scopes: ['source.gfm'],
  ...
  it('tokenizes mentions', function () {
    let { tokens } = grammar.tokenizeLine(
      'sentence with no space before@name '
    );
    expect(tokens[0]).toEqual({
    ...
    ({ tokens } = grammar.tokenizeLine(
      "@name '@name' @name's @name. @name, (@name) [@name]"
    ));
    expect(tokens[0]).toEqual({
      value: '@',
    ...
    ({ tokens } = grammar.tokenizeLine(
      'sentence with a space before @name that continues'
    ));
    expect(tokens[0]).toEqual({
    ...
    ({ tokens } = grammar.tokenizeLine(
      '* @name at the start of an unordered list'
    ));
    expect(tokens[0]).toEqual({
      value: '*',
      scopes: ['source.gfm', 'variable.unordered.list.gfm'],
    ...
    ({ tokens } = grammar.tokenizeLine(
      'a username @1337-hubot with numbers, letters and hyphens'
    ));
    expect(tokens[0]).toEqual({ value: 'a username ', scopes: ['source.gfm'] });
    expect(tokens[1]).toEqual({
      value: '@',
  ...
  it('tokenizes issue numbers', function () {
    ...
    ({ tokens } = grammar.tokenizeLine(
      'sentence with a space before #123i and a character after'
    ));
    expect(tokens[0]).toEqual({
    ...
    ({ tokens } = grammar.tokenizeLine(
      'sentence with a space before #123 that continues'
    ));
    expect(tokens[0]).toEqual({
      value: 'sentence with a space before ',
      scopes: ['source.gfm'],
  ...
  it('tokenizes HTML comments', function () {
    ...
    expect(tokens[0]).toEqual({
      ...
      scopes: [
        'source.gfm',
        'comment.block.gfm',
        'punctuation.definition.comment.gfm',
      ],
    });
    expect(tokens[1]).toEqual({
      value: ' a comment ',
