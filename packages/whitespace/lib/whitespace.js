  ...
  handleEvents(editor) {
    ...
    let bufferSavedSubscription = buffer.onWillSave(() => {
      return buffer.transact(() => {
        ...
        if (
          atom.config.get('whitespace.ensureSingleTrailingNewline', {
            scope: scopeDescriptor,
          })
        ) {
    ...
    let editorTextInsertedSubscription = editor.onDidInsertText(function (
      event
    ) {
      if (event.text !== '\n') {
        return;
  ...
  removeTrailingWhitespace(editor, grammarScopeName) {
    ...
    const cursorRows =
      activeEditor && activeEditor.getBuffer() === buffer
        ? new Set(
            activeEditor.getCursors().map((cursor) => cursor.getBufferRow())
          )
        : new Set();

    const ignoreCurrentLine = atom.config.get(
      'whitespace.ignoreWhitespaceOnCurrentLine',
      {
        scope: scopeDescriptor,
      }
    );

    const ignoreWhitespaceOnlyLines = atom.config.get(
      'whitespace.ignoreWhitespaceOnlyLines',
      {
        scope: scopeDescriptor,
      }
    );
    ...
    buffer.transact(() => {
      ...
      if (buffer.findAllSync) {
        ...
        for (let i = 0, n = ranges.length; i < n; i++) {
          ...
          if (ignoreWhitespaceOnlyLines && trailingWhitespaceStart === 0)
            continue;
        ...
        for (
          let row = 0, lineCount = buffer.getLineCount();
          row < lineCount;
          row++
        ) {
          ...
          if (lastCharacter === ' ' || lastCharacter === '\t') {
            const trailingWhitespaceStart = line.search(
            ...
            if (keepMarkdownLineBreakWhitespace) {
              ...
              if (trailingWhitespaceStart > 0 && whitespaceLength >= 2)
                continue;
            }
            buffer.delete(
              Range(
