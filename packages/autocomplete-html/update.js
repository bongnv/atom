...
const AttributesURL =
  'https://raw.githubusercontent.com/adobe/brackets/master/src/extensions/default/HTMLCodeHints/HtmlAttributes.json';
...
const tagsPromise = new Promise((resolve) => {
  request({ json: true, url: TagsURL }, (error, response, tags) => {
    ...
    for (let tag in tags) {
      ...
      if (
        (options.attributes != null ? options.attributes.length : undefined) ===
        0
      ) {
...
const attributesPromise = new Promise((resolve) => {
  return request(
    ...
    (error, response, attributes) => {
      ...
      if (response.statusCode !== 200) {
        console.error(
          `Request for HtmlAttributes.json failed: ${response.statusCode}`
        );
        resolve(null);
      }
...
Promise.all([
  tagsPromise,
  tagDescriptionsPromise,
  attributesPromise,
  globalAttributeDescriptionsPromise,
]).then((values) => {
  const tags = values[0];
