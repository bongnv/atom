...
const mdnJSONAPI =
  'https://developer.mozilla.org/en-US/search.json?topic=html&highlight=false';
const TagsURL =
  'https://raw.githubusercontent.com/adobe/brackets/master/src/extensions/default/HTMLCodeHints/HtmlTags.json';
  ...
  const tagsPromise = new Promise((resolve) => {
    request({ json: true, url: TagsURL }, (error, response, tags) => {
      ...
      if (response.statusCode !== 200) {
        console.error(
          `Request for HtmlTags.json failed: ${response.statusCode}`
        );
        resolve(null);
      }
      ...
      var handleRequest = (tagName, searchResults) => {
        if (searchResults.documents != null) {
          for (let doc of searchResults.documents) {
            ...
            if (
              doc.url === `${mdnHTMLURL}/${tagName}` ||
              (/^h\d$/.test(tagName) &&
                doc.url === `${mdnHTMLURL}/Heading_Elements`)
            ) {
              if (doc.tags.includes('Obsolete')) {
                docs[
                  tagName
                ] = `The ${tagName} element is obsolete. Avoid using it and update existing code if possible.`;
              } else if (doc.tags.includes('Deprecated')) {
                docs[
                  tagName
                ] = `The ${tagName} element is deprecated. Avoid using it and update existing code if possible.`;
              } else {
...
if (require.main === module) {
  fetch().then((docs) => {
    if (docs != null) {
      fs.writeFileSync(
        path.join(__dirname, 'tag-docs.json'),
