...
describe('Clojure grammar', function () {
  ...
  it('tokenizes numerics', function () {
    const numbers = {
      ...
      'constant.numeric.hexadecimal.clojure': [
        '0xDEADBEEF',
        '0XDEADBEEF',
        '0xDEADBEEFN',
        '0x0',
      ...
      for (var scope in numbers) {
        ...
        result.push(
          (() => {
            const result1 = [];
            for (var num of nums) {
              var { tokens } = grammar.tokenizeLine(num);
              result1.push(
                expect(tokens[0]).toEqual({
                  value: num,
        ...
        result.push(
          (() => {
            ...
            for (var bool of bools) {
              var { tokens } = grammar.tokenizeLine(bool);
              result1.push(
                expect(tokens[0]).toEqual({
                  value: bool,
                  scopes: ['source.clojure', scope],
  ...
  it('tokenizes keywords', function () {
    ...
    for (var metaScope in tests) {
      var lines = tests[metaScope];
  ...
  it('tokenizes keyfns (keyword control)', function () {
    ...
    return (() => {
      const result = [];
      for (var keyfn of keyfns) {
        var { tokens } = grammar.tokenizeLine(`(${keyfn})`);
        result.push(
          expect(tokens[1]).toEqual({
            value: keyfn,
            scopes: [
  ...
  it('tokenizes global definitions', function () {
    ...
    return (() => {
      ...
      for (var macro of macros) {
        var { tokens } = grammar.tokenizeLine(`(${macro} foo 'bar)`);
        expect(tokens[1]).toEqual({
          value: macro,
          scopes: [
            'source.clojure',
  ...
  it('tokenizes dynamic variables', function () {
    ...
    return (() => {
      const result = [];
      for (var mutable of mutables) {
        var { tokens } = grammar.tokenizeLine(mutable);
        result.push(
          expect(tokens[0]).toEqual({
            value: mutable,
  ...
  const testMetaSection = function (
    metaScope,
    puncScope,
    startsWith,
    endsWith
    ...
    for (token of mid) {
      expect(token.scopes.slice(0, 2)).toEqual([
        'source.clojure',
        `meta.${metaScope}.clojure`,
      ]);
    }
