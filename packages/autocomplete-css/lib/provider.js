...
module.exports = {
  ...
  isCompletingValue({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    const beforePrefixScopes = editor.scopeDescriptorForBufferPosition(
      beforePrefixBufferPosition
    );
    ...
    return (
      (hasScope(scopes, 'meta.property-list.css') && prefix.trim() === ':') ||
      ...
      (hasScope(scopes, 'source.sass', true) &&
        (hasScope(scopes, 'meta.property-value.sass') ||
          (!hasScope(beforePrefixScopesArray, 'entity.name.tag.css') &&
            prefix.trim() === ':')))
    );
  },
  ...
  isCompletingName({ scopeDescriptor, bufferPosition, prefix, editor }) {
    ...
    if (
      hasScope(previousScopesArray, 'entity.other.attribute-name.class.css') ||
  ...
  isCompletingTagSelector({ editor, scopeDescriptor, bufferPosition }) {
    ...
    if (
      ...
      hasScope(scopes, 'source.css.scss', true) ||
      hasScope(scopes, 'source.css.less', true) ||
      hasScope(scopes, 'source.css.postcss', true)
    ) {
      return (
        !hasScope(previousScopesArray, 'meta.property-value.scss') &&
        !hasScope(previousScopesArray, 'meta.property-value.css') &&
  ...
  isCompletingPseudoSelector({ editor, scopeDescriptor, bufferPosition }) {
    const scopes = scopeDescriptor.getScopesArray();
    let previousBufferPosition = [
      bufferPosition.row,
      Math.max(0, bufferPosition.column - 1),
    ];
    let previousScopes = editor.scopeDescriptorForBufferPosition(
      previousBufferPosition
    );
    ...
    if (
      ...
      if (prefix) {
        ...
        previousScopes = editor.scopeDescriptorForBufferPosition(
          previousBufferPosition
        );
    ...
    if (this.isPropertyValuePrefix(prefix)) {
      for (value of values) {
        if (firstCharsEqual(value, prefix)) {
          completions.push(
            this.buildPropertyValueCompletion(value, property, addSemicolon)
          );
        }
      }
    } else if (
      !hasScope(scopes, 'keyword.other.unit.percentage.css') && // CSS
      !hasScope(scopes, 'keyword.other.unit.scss') && // SCSS (TODO: remove in Atom 1.19.0)
      !hasScope(scopes, 'keyword.other.unit.css')
    ) {
      // Less, Sass (TODO: remove in Atom 1.19.0)
...
var hasScope = (scopesArray, scope, checkEmbedded = false) =>
