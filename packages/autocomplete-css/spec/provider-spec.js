...
Object.keys(packagesToTest).forEach(function (packageLabel) {
  if (
    !atom.packages
      ...
      .includes(packagesToTest[packageLabel].name)
  ) {
    console.warn(
      `Skipping tests for ${packageLabel} because it is not installed`
    );
...
describe('CSS property name and value autocompletions', function () {
  ...
  const getCompletions = function (options = {}) {
    ...
    const request = {
      ...
      prefix,
      activatedManually:
        options.activatedManually != null ? options.activatedManually : true,
    };
  ...
  Object.keys(packagesToTest).forEach((packageLabel) =>
    describe(`${packageLabel} files`, function () {
      beforeEach(function () {
        waitsForPromise(() =>
          atom.packages.activatePackage(packagesToTest[packageLabel].name)
        );
        waitsForPromise(() =>
          atom.workspace.open(packagesToTest[packageLabel].file)
        );
...
`);
        editor.setCursorBufferPosition([1, 12]);
        const completions = getCompletions();
        return completions.forEach((completion) =>
...
`);
      ...
      it('autocompletes more than one inline property value', function () {
        ...
        editor.setText(
          'body { display: block; float: left; cursor: alias; text-decoration: }'
        );
...
`);
          ...
          expect(completions[0].description).toBe(
...
`);
          ...
          expect(completions[0].description.length).toBeGreaterThan(0);
          return expect(
            completions[0].descriptionMoreURL.length
          ).toBeGreaterThan(0);
        });
...
`);
    ...
    return describe('pseudo selectors', () =>
      it('autocompletes without a prefix', function () {
        editor.setText(`\
div:\
`);
