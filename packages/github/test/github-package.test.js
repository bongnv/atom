...
describe('GithubPackage', function () {
  async function buildAtomEnvironmentAndGithubPackage(
  ...
  describe('activate()', function () {
    ...
    beforeEach(async function () {
      ({ atomEnv, githubPackage, project, config, configDirPath, contextPool } =
        await buildAtomEnvironmentAndGithubPackage(
          global.buildAtomEnvironmentAndGithubPackage
        ));
    });
    ...
    describe('with only projects', function () {
      ...
      it("uses the first project's context", function () {
        ...
        assert.strictEqual(
          context1.getRepository(),
          githubPackage.getActiveRepository()
        );
        assert.strictEqual(
          context1.getResolutionProgress(),
          githubPackage.getActiveResolutionProgress()
        );
      });
    ...
    describe('with projects and state', function () {
      ...
      beforeEach(async function () {
        ...
        await contextUpdateAfter(githubPackage, () =>
  ...
  describe('scheduleActiveContextUpdate()', function () {
    ...
    beforeEach(async function () {
      ({ atomEnv, githubPackage, project, contextPool } =
        await buildAtomEnvironmentAndGithubPackage(
          global.buildAtomEnvironmentAndGithubPackage
        ));
    });
    ...
    describe('with existing projects', function () {
      ...
      describe('when changing the active pane item', function () {
        it('follows the active pane item', async function () {
          ...
          assert.strictEqual(githubPackage.getActiveWorkdir(), workdirPath1);
          await contextUpdateAfter(githubPackage, () =>
            atomEnv.workspace.open(itemPath2)
          );
          assert.strictEqual(githubPackage.getActiveWorkdir(), workdirPath2);
        });
        ...
        it('uses a context associated with a project root for paths within that root', async function () {
          ...
          assert.strictEqual(githubPackage.getActiveWorkdir(), workdirPath1);
          await contextUpdateAfter(githubPackage, () =>
            atomEnv.workspace.open(itemPath4)
          );
      ...
      describe('with a locked context', function () {
        ...
        it('may be unlocked', async function () {
          ...
          await contextUpdateAfter(githubPackage, () =>
            atomEnv.workspace.open(itemPath1a)
          );
          assert.strictEqual(githubPackage.getActiveWorkdir(), workdirPath2);
          ...
          await contextUpdateAfter(githubPackage, () =>
            atomEnv.workspace.open(itemPath1b)
          );
          assert.strictEqual(githubPackage.getActiveWorkdir(), workdirPath1);
        });
    ...
    describe('with non-repository, no-conflict, and in-progress merge-conflict projects', function () {
      ...
      beforeEach(async function () {
        ...
        resolutionMergeConflict.reportMarkerCount(
      ...
      describe('when selecting an in-progress merge-conflict project', function () {
        ...
        beforeEach(async function () {
          ...
          resolutionMergeConflict = contextPool
            .getContext(workdirMergeConflict)
            .getResolutionProgress();
        });
      ...
      describe('when opening a no-conflict repository project', function () {
        ...
        beforeEach(async function () {
          ...
          resolutionNoConflict = contextPool
    ...
    describe('with a repository project', function () {
      ...
      describe('when the repository is destroyed', function () {
        beforeEach(function () {
          const repository = contextPool
            .getContext(workdirPath)
            .getRepository();
          repository.destroy();
        });
  ...
  describe('clone()', function () {
    ...
    describe('with an existing project', function () {
      ...
      it('clones into the existing project', async function () {
        assert.strictEqual(
          await githubPackage.workdirCache.find(existingPath),
          existingPath
        );
      });
    });
    ...
    describe('with no projects', function () {
      ...
      beforeEach(async function () {
        await contextUpdateAfter(githubPackage, () =>
          githubPackage.clone(sourcePath, newPath)
        );
      });
  ...
  describe('createCommitPreviewStub()', function () {
    ...
    beforeEach(async function () {
      ({ atomEnv, githubPackage } = await buildAtomEnvironmentAndGithubPackage(
        global.buildAtomEnvironmentAndGithubPackage
      ));
    ...
    describe('when called before the initial render', function () {
      let item;
      beforeEach(function () {
        item = githubPackage.createCommitPreviewStub({
  ...
  describe('createCommitDetailStub()', function () {
    ...
    describe('when called before the initial render', function () {
      let item;
      beforeEach(function () {
        item = githubPackage.createCommitDetailStub({
  ...
  describe('with repository projects', function () {
    ...
    beforeEach(async function () {
      ...
      const watcherPromises = [
        until(() =>
          contextPool.getContext(workdirPath1).getChangeObserver().isStarted()
        ),
        until(() =>
          contextPool.getContext(workdirPath2).getChangeObserver().isStarted()
        ),
      ];
    ...
    beforeEach(function () {
      [atomGitRepository1, atomGitRepository2] =
