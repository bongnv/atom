...
describe('PullRequestChangedFilesContainer', function () {
  ...
  describe('when the patch is fetched successfully', function () {
    it('passes the MultiFilePatch to a MultiFilePatchController', function () {
      ...
      assert.strictEqual(
        subwrapper.find('MultiFilePatchController').prop('multiFilePatch'),
        multiFilePatch
      );
    });
    ...
    it('passes extra props through to MultiFilePatchController', function () {
      ...
      assert.strictEqual(
        subwrapper.find('MultiFilePatchController').prop('extraProp'),
        extraProp
      );
    });
    ...
    it('manages a subscription on the active MultiFilePatch', function () {
      const { multiFilePatch: mfp0 } = multiFilePatchBuilder()
        .addFilePatch()
        .build();

      const wrapper = shallow(buildApp());
      wrapper.find('PullRequestPatchContainer').renderProp('children')(
        null,
        mfp0
      );
      ...
      assert.strictEqual(
        mfp0
          .getFilePatches()[0]
          .emitter.listenerCountForEventName('change-render-status'),
        1
      );
      ...
      const { multiFilePatch: mfp1 } = multiFilePatchBuilder()
        .addFilePatch()
        .build();
      wrapper.find('PullRequestPatchContainer').renderProp('children')(
        null,
  ...
  describe('when the patch load fails', function () {
    it('renders the message in an ErrorView', function () {
      ...
      const subwrapper = wrapper
        .find('PullRequestPatchContainer')
        .renderProp('children')(error, null);

      assert.deepEqual(subwrapper.find('ErrorView').prop('descriptions'), [
        error,
      ]);
    });
  });
