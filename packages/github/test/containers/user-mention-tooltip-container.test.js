...
describe('UserMentionTooltipContainer', function () {
  ...
  it('renders information about a User', function () {
    ...
    assert.strictEqual(
      ...
      'https://i.redd.it/03xcogvbr6v21.jpg'
    );
    assert.isTrue(
      wrapper.find('span').someWhere((s) => s.text() === 'Infinity, Ltd.')
    );
    assert.isTrue(
      wrapper.find('span').someWhere((s) => s.text() === '5 repositories')
    );
  });
  ...
  it('renders information about an Organization', function () {
    ...
    assert.strictEqual(
      wrapper.find('img').prop('src'),
      'https://i.redd.it/eekf8onik0v21.jpg'
    );
    assert.isTrue(
      wrapper.find('span').someWhere((s) => s.text() === '10 members')
    );
    assert.isTrue(
      wrapper.find('span').someWhere((s) => s.text() === '5 repositories')
    );
