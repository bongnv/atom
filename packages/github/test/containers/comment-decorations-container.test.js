...
describe('CommentDecorationsContainer', function () {
  ...
  it('renders nothing if no GitHub remotes exist', async function () {
    ...
    const localRepoData = await wrapper.find(ObserveModel).prop('fetchData')(
      localRepository
    );
    const localRepoWrapper = wrapper.find(ObserveModel).renderProp('children')(
      localRepoData
    ...
    const token = await localRepoWrapper.find(ObserveModel).prop('fetchData')(
      loginModel,
      localRepoData
    );
    assert.isNull(token);

    const tokenWrapper = localRepoWrapper
      .find(ObserveModel)
  ...
  describe('when GitHub remote exists', function () {
    ...
    beforeEach(async function () {
      ...
      localRepoWrapper = wrapper.find(ObserveModel).renderProp('children')(
        repoData
      );
    });
    ...
    it('renders nothing if token is UNAUTHENTICATED', function () {
      const tokenWrapper = localRepoWrapper
        .find(ObserveModel)
        .renderProp('children')(UNAUTHENTICATED);
      assert.isTrue(tokenWrapper.isEmptyRender());
    });
    ...
    it('renders nothing if token is INSUFFICIENT', function () {
      const tokenWrapper = localRepoWrapper
        .find(ObserveModel)
        .renderProp('children')(INSUFFICIENT);
      assert.isTrue(tokenWrapper.isEmptyRender());
    });
    ...
    it('renders nothing if query errors', function () {
      const tokenWrapper = localRepoWrapper
        .find(ObserveModel)
        .renderProp('children')('1234');
      const resultWrapper = tokenWrapper
        .find(QueryRenderer)
        .renderProp('render')({
        error: 'oh noes',
        props: null,
    ...
    it('renders nothing if query result does not include repository', function () {
      const props = queryBuilder(rootQuery).nullRepository().build();
        ...
        commentThreads: [
          {
            thread: { id: 'thread0' },
            comments: [
              { id: 'comment0', path: 'a.txt' },
              { id: 'comment1', path: 'a.txt' },
            ],
          },
        ],
      });
      const patchWrapper = reviewsWrapper
    ...
    it('renders nothing if patch cannot be fetched', function () {
      ...
      const patchWrapper = reviewsWrapper
        .find(PullRequestPatchContainer)
        .renderProp('children')(new Error('oops'), null);
      assert.isTrue(patchWrapper.isEmptyRender());
    });
    ...
    it('renders a CommentPositioningContainer when the patch and reviews arrive', function () {
      ...
      const patchWrapper = reviewsWrapper
        .find(PullRequestPatchContainer)
        .renderProp('children')(null, patch);
    ...
    it('renders nothing while the comment positions are being calculated', function () {
      ...
      const positionedWrapper = patchWrapper
    ...
    it('renders a CommentDecorationsController with all of the results once comment positions arrive', function () {
      ...
      const positionedWrapper = patchWrapper
        .find(CommentPositioningContainer)
        .renderProp('children')(translations);
      ...
      assert.strictEqual(
        controller.prop('pullRequests'),
        props.repository.ref.associatedPullRequests.nodes
      );
    });
  });
});
