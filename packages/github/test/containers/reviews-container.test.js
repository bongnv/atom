...
describe('ReviewsContainer', function () {
  ...
  it('shows a login form if the token is outdated', function () {
    ...
    assert.isTrue(tokenWrapper.exists('GithubLoginView'));
    assert.match(
      tokenWrapper.find('GithubLoginView > p').text(),
      /re-authenticate/
    );
  });
  ...
  it('shows a loading spinner if the patch is being fetched', function () {
    ...
    const patchWrapper = tokenWrapper
      .find('PullRequestPatchContainer')
      .renderProp('children')(null, null);
    const repoWrapper = patchWrapper
      .find('ObserveModel')
      .renderProp('children')(repoData);
  ...
  it('shows an error view if the review comment aggregation fails', function () {
    ...
    const patchWrapper = tokenWrapper
      .find('PullRequestPatchContainer')
      .renderProp('children')(null, multiFilePatch);
    ...
    const reviewsWrapper = relayWrapper
    ...
    assert.deepEqual(
      reviewsWrapper.find('ErrorView').map((w) => w.prop('descriptions')),
      [[e0.stack], [e1.stack]]
    );
  });
  ...
  it('passes the patch to the controller', function () {
    ...
    assert.strictEqual(
      tokenWrapper.find('PullRequestPatchContainer').prop('owner'),
      'secret'
    );
    assert.strictEqual(
      tokenWrapper.find('PullRequestPatchContainer').prop('repo'),
      'squirrel'
    );
    assert.strictEqual(
      tokenWrapper.find('PullRequestPatchContainer').prop('number'),
      123
    );
    assert.strictEqual(
      tokenWrapper.find('PullRequestPatchContainer').prop('endpoint').getHost(),
      'github.enterprise.com'
    );
    assert.strictEqual(
      tokenWrapper.find('PullRequestPatchContainer').prop('token'),
      'shhh'
    );
    const patchWrapper = tokenWrapper
      .find('PullRequestPatchContainer')
    ...
    const positionedWrapper = reviewsWrapper
      .find(CommentPositioningContainer)
      .renderProp('children')(new Map());

    assert.strictEqual(
      positionedWrapper.find(ReviewsController).prop('multiFilePatch'),
      multiFilePatch
    );
  });
  ...
  it('passes loaded repository data to the controller', async function () {
    ...
    assert.strictEqual(
      patchWrapper.find('ObserveModel').prop('model'),
      repository
    );
    assert.deepEqual(
      await patchWrapper.find('ObserveModel').prop('fetchData')(repository),
      repoData
    );
    const repoWrapper = patchWrapper
      .find('ObserveModel')
      .renderProp('children')(extraRepoData);
    ...
    assert.strictEqual(
      positionedWrapper.find(ReviewsController).prop('one'),
      extraRepoData.one
    );
    assert.strictEqual(
      positionedWrapper.find(ReviewsController).prop('two'),
      extraRepoData.two
    );
  });
  ...
  it('passes extra properties to the controller', function () {
    ...
    assert.strictEqual(
      positionedWrapper.find(ReviewsController).prop('extra'),
      extra
    );
  });
  ...
  it('shows an error if the patch cannot be fetched', function () {
    ...
    const patchWrapper = tokenWrapper
      .find('PullRequestPatchContainer')
      .renderProp('children')('[errors intensify]', null);
    assert.deepEqual(patchWrapper.find('ErrorView').prop('descriptions'), [
      '[errors intensify]',
    ]);
  });
  ...
  it('shows an error if the GraphQL query fails', async function () {
    ...
    assert.strictEqual(
      await loginModel.getToken('https://github.enterprise.horse'),
      'different'
    );
