...
describe('ReviewThreadsAccumulator', function () {
  ...
  it('recursively renders a ReviewCommentsAccumulator for each reviewThread', function () {
    ...
    const threadWrapper = wrapper.find('Accumulator').renderProp('children')(
      ...args
    );
  ...
  it('handles the arrival of additional review thread batches', function () {
    ...
    const wrapper = shallow(
      buildApp({ pullRequest: pr0, props: { children } })
    );
    const threadsWrapper0 = wrapper.find('Accumulator').renderProp('children')(
      null,
      batch0,
      true
    );
    const comments0Wrapper0 = threadsWrapper0
    ...
    const threadsWrapper1 = wrapper.find('Accumulator').renderProp('children')(
      null,
      batch1,
      false
    );
    const comments1Wrapper0 = threadsWrapper1
      .find(ReviewCommentsAccumulator)
      .renderProp('children')({
      error: null,
      comments: [1, 1, 1],
    ...
    const comments1Wrapper1 = comments1Wrapper0
      .find(ReviewCommentsAccumulator)
      .renderProp('children')({
      error: null,
      comments: [2, 2, 2],
  ...
  it('handles errors from each ReviewCommentsAccumulator', function () {
    ...
    const threadsWrapper = wrapper.find('Accumulator').renderProp('children')(
      null,
      batch,
      false
    );
    const error0 = new Error('oh shit');
    const commentsWrapper0 = threadsWrapper
      .find(ReviewCommentsAccumulator)
      .renderProp('children')({
      error: error0,
      comments: [],
    ...
    const commentsWrapper1 = commentsWrapper0
      .find(ReviewCommentsAccumulator)
      .renderProp('children')({
      error: error1,
      comments: [],
