...
} from '../../lib/controllers/issueish-list-controller';
...
describe('CurrentPullRequestContainer', function () {
  ...
  it('performs no query without a valid push remote', function () {
    const tracking = Branch.createRemoteTracking(
      'remotes/nope/wat',
      'nope',
      'wat'
    );
  ...
  it('passes an empty result list and an isLoading prop to the controller while loading', function () {
    ...
    assert.isTrue(
      resultWrapper.find(BareIssueishListController).prop('isLoading')
    );
  });
  ...
  it('passes an empty result list and an error prop to the controller when errored', function () {
    ...
    assert.strictEqual(
      resultWrapper.find(BareIssueishListController).prop('error'),
      error
    );
    assert.isFalse(
      resultWrapper.find(BareIssueishListController).prop('isLoading')
  ...
  it('passes a configured pull request creation tile to the controller', function () {
    ...
    const emptyTileWrapper = resultWrapper
      .find(IssueishListController)
      .renderProp('emptyComponent')();
  ...
  it('passes no results if the repository is not found', function () {
    ...
    const props = queryBuilder(currentQuery).nullRepository().build();
