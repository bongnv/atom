...
describe('GitHubTabContainer', function () {
  ...
  describe('refresher', function () {
    ...
    beforeEach(function () {
      ...
      const repoWrapper = wrapper.find('ObserveModel').renderProp('children')(
        defaultRepositoryData
      );
      const tokenWrapper = repoWrapper
        .find('ObserveModel')
        .renderProp('children')('1234');

      retry = sinon.spy();
      const refresher = tokenWrapper
    ...
    it('triggers a refresh when the current repository completes a fetch, push, or pull', function () {
      ...
      simulateOperation(repository, 'push', () =>
        assert.strictEqual(retry.callCount, 1)
      );
      assert.strictEqual(retry.callCount, 2);

      simulateOperation(repository, 'pull', () =>
        assert.strictEqual(retry.callCount, 2)
      );
  ...
  describe('while loading', function () {
    it('passes isLoading to the controller', async function () {
      ...
      const wrapper = shallow(buildApp({ repository: loadingRepo }));
      const repoData = await wrapper.find('ObserveModel').prop('fetchData')(
        loadingRepo
      );
      const repoWrapper = wrapper.find('ObserveModel').renderProp('children')(
        repoData
  ...
  describe('once loaded', function () {
    ...
    it('derives the subset of GitHub remotes', async function () {
      ...
      const tokenWrapper = repoWrapper
        .find('ObserveModel')
        .renderProp('children')('1234');
      const githubRemotes = tokenWrapper
        .find('GitHubTabController')
        .prop('githubRemotes');

      assert.sameMembers(
        Array.from(githubRemotes, (remote) => remote.getName()),
        ['yes0', 'yes1']
      );
    });
    ...
    it('derives the current branch', async function () {
      ...
      const tokenWrapper = repoWrapper
        .find('ObserveModel')
        .renderProp('children')('1234');
      const currentBranch = tokenWrapper
        .find('GitHubTabController')
        .prop('currentBranch');
    ...
    it('identifies the current remote from the config key', async function () {
      ...
      const tokenWrapper = repoWrapper
        .find('ObserveModel')
        .renderProp('children')('1234');
      const currentRemote = tokenWrapper
        .find('GitHubTabController')
        .prop('currentRemote');
