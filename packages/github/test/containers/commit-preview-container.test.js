...
describe('CommitPreviewContainer', function () {
  ...
  it('renders a CommitPreviewController once the file patch is loaded', async function () {
    ...
    await assert.async.isTrue(
      wrapper.update().find('CommitPreviewController').exists()
    );
    assert.strictEqual(
  ...
  it('remembers previously expanded large patches', async function () {
    ...
    await assert.async.isTrue(
      wrapper.update().exists('CommitPreviewController')
    );
    ...
    await assert.async.notStrictEqual(
      wrapper.update().find('CommitPreviewController').prop('multiFilePatch'),
      before
    );
    const after = wrapper
      .find('CommitPreviewController')
