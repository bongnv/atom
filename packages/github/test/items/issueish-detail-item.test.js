...
describe('IssueishDetailItem', function () {
  ...
  it('renders within the workspace center', async function () {
    ...
    assert.include(
      centerPaneItems.map((i) => i.getURI()),
      uri
    );
  ...
  describe('issueish switching', function () {
    ...
    it('reverts to an absent repository when no matching repository is found', async function () {
      ...
      wrapper.update();
      assert.isTrue(
        wrapper.find('IssueishDetailContainer').prop('repository').isAbsent()
      );
    });
  ...
  it('reconstitutes its original URI', async function () {
    ...
    wrapper.update().find('IssueishDetailContainer').prop('switchToIssueish')(
      'you',
      'switched',
      2
    );
  ...
  it('broadcasts title changes', async function () {
    ...
    wrapper.update().find('IssueishDetailContainer').prop('onTitleChange')(
      'SUP'
    );
  ...
  describe('tab navigation', function () {
    ...
    beforeEach(async function () {
      ...
      item = await atomEnv.workspace.open(
        IssueishDetailItem.buildURI({
          host: 'host.com',
          owner: 'dolphin',
          repo: 'fish',
    ...
    it("open files tab if it isn't already opened", async function () {
      await item.openFilesTab({
        changedFilePath: 'dir/file',
        changedFilePosition: 100,
      });

      assert.strictEqual(
        wrapper.find('IssueishDetailItem').state('initChangedFilePath'),
        'dir/file'
      );
      assert.strictEqual(
        wrapper.find('IssueishDetailItem').state('initChangedFilePosition'),
        100
      );
    });
    ...
    it('resets initChangedFilePath & initChangedFilePosition when navigating between tabs', async function () {
      ...
      wrapper.find('IssueishDetailContainer').prop('onTabSelected')(
        IssueishDetailItem.tabs.BUILD_STATUS
      );
      assert.strictEqual(
        wrapper.find('IssueishDetailItem').state('initChangedFilePath'),
        ''
