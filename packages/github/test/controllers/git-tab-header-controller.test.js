...
describe('GitTabHeaderController', function () {
  ...
  it('get currentWorkDirs initializes workdirs state', function () {
    ...
    const wrapper = shallow(
      buildApp({ getCurrentWorkDirs: () => createWorkdirs(paths) })
    );
    assert.strictEqual(
      wrapper.state(['currentWorkDirs']).next().value,
      paths[0]
    );
  });
  ...
  it('calls onDidUpdateRepo on update to setup new listener', function () {
    ...
    const wrapper = shallow(
      buildApp({ onDidUpdateRepo, isRepoDestroyed: () => false })
    );
  ...
  it('updates the committer if the method changes', async function () {
    ...
    await assert.async.strictEqual(
      wrapper.state('committer').getEmail(),
      'upd@te.d'
    );
  });
  ...
  it('handles a lock toggle', async function () {
    ...
    const setContextLock = sinon.stub().returns(
      new Promise((resolve) => {
    ...
    const handlerPromise = wrapper
      .find('GitTabHeaderView')
      .prop('handleLockToggle')();
    wrapper.update();
  ...
  it('handles a workdir selection', async function () {
    ...
    const changeWorkingDirectory = sinon.stub().returns(
      new Promise((resolve) => {
        resolveWorkdirChange = resolve;
      })
    );
    const wrapper = shallow(
      buildApp({ currentWorkDir: 'original', changeWorkingDirectory })
    );

    assert.strictEqual(
      wrapper.find('GitTabHeaderView').prop('workdir'),
      'original'
    );
