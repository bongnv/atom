...
describe('RootController', function () {
  ...
  describe('initialize', function () {
    ...
    it('requests the init dialog with a command', async function () {
      ...
      await wrapper
        .find('Command[command="github:initialize"]')
        .prop('callback')();
    ...
    it('defaults to the project directory containing the open file if there is one', async function () {
      const noRepo0 = await new Promise((resolve, reject) =>
        temp.mkdir({}, (err, p) => (err ? reject(err) : resolve(p)))
      );
      const noRepo1 = await new Promise((resolve, reject) =>
        temp.mkdir({}, (err, p) => (err ? reject(err) : resolve(p)))
      );
  ...
  describe('openIssueishDialog()', function () {
    ...
    it('renders the OpenIssueish dialog', function () {
      const wrapper = shallow(app);
      wrapper
        .find('Command[command="github:open-issue-or-pull-request"]')
        .prop('callback')();
      wrapper.update();

      assert.strictEqual(
        wrapper.find('DialogsController').prop('request').identifier,
        'issueish'
      );
    });
  ...
  describe('openCreateDialog()', function () {
    it('renders the modal create dialog', function () {
      ...
      wrapper
        .find('Command[command="github:create-repository"]')
        .prop('callback')();
      assert.strictEqual(
        wrapper.find('DialogsController').prop('request').identifier,
        'create'
    ...
    it('creates a repository on GitHub on accept', async function () {
      ...
      RelayNetworkLayerManager.getEnvironmentForHost(
      ...
      assert.isTrue(
        clone.calledWith(
          'https://github.com/user0/repo-name',
          localPath,
          'home'
        )
      );
  ...
  describe('openPublishDialog()', function () {
    ...
    it('renders the modal publish dialog', async function () {
      ...
      assert.isTrue(
        payload.remotes.filter((each) => each.isGithubRepo()).isEmpty()
      );
      const inner = observer.renderProp('children')(payload);

      inner
        .find('Command[command="github:publish-repository"]')
        .prop('callback')();
      assert.strictEqual(
        wrapper.find('DialogsController').prop('request').identifier,
        'publish'
    ...
    it('publishes the active repository to GitHub on accept', async function () {
      ...
      const wrapper = shallow(
        React.cloneElement(app, { repository: publishable })
      );
      const inner = wrapper.find('ObserveModel').renderProp('children')({
        isPublishable: true,
      ...
      assert.strictEqual(
        addedRemote.getUrl(),
        'ssh@github.com:user0/repo-name.git'
      );

      assert.isTrue(
        publishable.push.calledWith('master', {
          setUpstream: true,
          remote: addedRemote,
        })
      );
  ...
  describe('context commands trigger event reporting', function () {
    ...
    it('does not send an event when a command not starting with github: is triggered via a context menu', function () {
      commands.dispatch(wrapper.find('CommitView').getDOMNode(), 'core:copy', [
        { contextCommand: true },
      ]);
      assert.isFalse(reporterProxy.addEvent.called);
    });
  ...
  describe('reportRelayError', function () {
    ...
    it('creates a notification for a network error', function () {
      ...
      assert.isTrue(
        notificationManager.addError.calledWith('friendly message', {
          dismissable: true,
