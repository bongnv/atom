...
describe('PullRequestCheckoutController', function () {
  ...
  it('creates a new remote, fetches a PR branch, and checks it out into a new local branch', async function () {
    ...
    sinon
      .stub(localRepository, 'addRemote')
    ...
    assert.isTrue(
      localRepository.addRemote.calledWith('ccc', 'git@github.com:ccc/ddd.git')
    );
    assert.isTrue(
      localRepository.fetch.calledWith('refs/heads/feature', {
        remoteName: 'ccc',
      })
    );
    assert.isTrue(
      localRepository.checkout.calledWith('pr-456/ccc/feature', {
        createNew: true,
        track: true,
  ...
  it('fetches a PR branch from an existing remote and checks it out into a new local branch', async function () {
    ...
    assert.isTrue(
      localRepository.fetch.calledWith('refs/heads/clever-name', {
        remoteName: 'existing',
      })
    );
    assert.isTrue(
      localRepository.checkout.calledWith('pr-789/ccc/clever-name', {
        createNew: true,
        track: true,
  ...
  it('checks out an existing local branch that corresponds to the pull request', async function () {
    ...
    assert.isTrue(
      localRepository.pull.calledWith('refs/heads/yes', {
        remoteName: 'upstream',
        ffOnly: true,
      })
    );
