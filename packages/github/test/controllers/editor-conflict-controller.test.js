...
describe('EditorConflictController', function () {
  ...
  const useFixture = async function (
    fixtureName,
    { isRebase, withEditor } = { isRebase: false }
  ) {
  ...
  it('scrolls the first conflict into view', async function () {
    await useFixture('triple-2way-diff.txt', {
      withEditor(e) {
        sinon.stub(e, 'scrollToBufferPosition');
      },
    });

    assert.isTrue(
      editor.scrollToBufferPosition.calledWith(new Point(4, 0), {
        center: true,
      })
    );
  });
  ...
  describe('on a file with 2-way diff markers', function () {
    ...
    it('renders a ConflictController for each Conflict', function () {
      ...
      conflicts.forEach((conflict) => {
        assert.isTrue(
          conflictControllers.someWhere(
            (cc) => cc.prop('conflict') === conflict
          )
        );
      });
    });
    ...
    it('performs a resolution from the context menu', function () {
      ...
      wrapper.find('ConflictController').at(1).prop('resolveAsSequence')([
        OURS,
      ]);
