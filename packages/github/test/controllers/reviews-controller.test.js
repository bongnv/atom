...
describe('ReviewsController', function () {
  ...
  beforeEach(async function () {
    ...
    relayEnv = RelayNetworkLayerManager.getEnvironmentForHost(
      getEndpoint('github.com'),
      '1234'
    );
  });
  ...
  it('scrolls to and highlight a specific thread on mount', function () {
    ...
    let opWrapper = wrapper
      .find(PullRequestCheckoutController)
      .renderProp('children')(noop);

    assert.include(
      opWrapper.find(ReviewsView).prop('threadIDsOpen'),
      'thread0'
    );
    assert.include(
      opWrapper.find(ReviewsView).prop('highlightedThreadIDs'),
      'thread0'
    );
    assert.strictEqual(
      opWrapper.find(ReviewsView).prop('scrollToThreadID'),
      'thread0'
    );
  ...
  describe('openIssueish', function () {
    it('opens an IssueishDetailItem for a different issueish', async function () {
      ...
      const opWrapper = wrapper
        .find(PullRequestCheckoutController)
        .renderProp('children')(noop);
      await opWrapper.find(ReviewsView).prop('openIssueish')(
        'owner',
        'repo',
        10
      );
  ...
  describe('context lines', function () {
    ...
    it('increases context lines with moreContext', function () {
      const wrapper = shallow(buildApp());
      const opWrapper0 = wrapper
        .find(PullRequestCheckoutController)
        .renderProp('children')(noop);

      opWrapper0.find(ReviewsView).prop('moreContext')();

      const opWrapper1 = wrapper
        .find(PullRequestCheckoutController)
        .renderProp('children')(noop);
  ...
  describe('adding a single comment', function () {
    ...
    it('creates a notification when the review cannot be created', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => {
          return relayResponseBuilder(op).addError('Oh no').build();
        }
      ).resolve();
      ...
      assert.isTrue(
        reportRelayError.calledWith('Unable to submit your comment')
      );
    ...
    it('includes errors from the review deletion', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => {
          return relayResponseBuilder(op).addError('Bam').build();
        }
      ).resolve();
    ...
    it('creates a notification when the review cannot be submitted', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => {
          return relayResponseBuilder(op).addError('Ouch').build();
        }
      ).resolve();
  ...
  describe('resolving threads', function () {
    it('hides the thread, then fires the mutation', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => relayResponseBuilder(op).build()
      ).resolve();
      ...
      assert.isTrue(
        wrapper.find(ReviewsView).prop('threadIDsOpen').has('thread0')
      );

      await wrapper.find(ReviewsView).prop('resolveThread')({
        id: 'thread0',
    ...
    it('re-shows the thread and creates a notification when the thread cannot be resolved', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => relayResponseBuilder(op).addError('boom').build()
      ).resolve();
      ...
      assert.isTrue(
        reportRelayError.calledWith('Unable to resolve the comment thread')
      );
    });
  });
  ...
  describe('unresolving threads', function () {
    it('calls the unresolve mutation', async function () {
      ...
      await wrapper.find(ReviewsView).prop('unresolveThread')({
        id: 'thread1',
    ...
    it('creates a notification if the thread cannot be unresolved', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => relayResponseBuilder(op).addError('ow').build()
      ).resolve();
      ...
      assert.isTrue(
        reportRelayError.calledWith('Unable to unresolve the comment thread')
      );
    });
  });
  ...
  describe('editing review comments', function () {
    it('calls the review comment update mutation and increments a metric', async function () {
      ...
      expectRelayQuery(
        {
          ...
          variables: {
            input: {
              pullRequestReviewCommentId: 'comment-0',
              body: 'new text',
            },
          },
        },
      ...
      await wrapper.find(ReviewsView).prop('updateComment')(
        'comment-0',
        'new text'
      );
      ...
      assert.isFalse(reportRelayError.called);
      assert.isTrue(
        reporterProxy.addEvent.calledWith('update-review-comment', {
          package: 'github',
        })
      );
    });
    ...
    it('creates a notification and and re-throws the error if the comment cannot be updated', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => relayResponseBuilder(op).addError('not right now').build()
      ).resolve();
  ...
  describe('editing review summaries', function () {
    it('calls the review summary update mutation and increments a metric', async function () {
      ...
      await wrapper.find(ReviewsView).prop('updateSummary')(
        'review-0',
        'stuff'
      );
      ...
      assert.isFalse(reportRelayError.called);
      assert.isTrue(
        reporterProxy.addEvent.calledWith('update-review-summary', {
          package: 'github',
        })
      );
    });
    ...
    it('creates a notification and and re-throws the error if the summary cannot be updated', async function () {
      ...
      assert.isTrue(
        reportRelayError.calledWith('Unable to update review summary')
      );
      assert.isFalse(reporterProxy.addEvent.called);
    });
  });
  ...
  describe('action methods', function () {
    ...
    beforeEach(function () {
      ...
      sinon
        .stub(atomEnv.workspace, 'open')
        .resolves({ openFilesTab, onTabSelected });
      sinon.stub(reporterProxy, 'addEvent');
    ...
    it('opens file on disk', async function () {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('reviews-dock-open-file', {
          package: 'github',
        })
      );
    });
    ...
    it('opens diff in PR detail item', async function () {
      ...
      assert.isTrue(
        openFilesTab.calledWith({
          changedFilePath: 'filepath',
          changedFilePosition: 420,
        })
      );
      assert.isTrue(
        reporterProxy.addEvent.calledWith('reviews-dock-open-diff', {
          package: 'github',
          component: 'BareReviewsController',
        })
      );
    });
    ...
    it('opens overview of a PR detail item', async function () {
      ...
      assert.isTrue(
        reporterProxy.addEvent.calledWith('reviews-dock-open-pr', {
          package: 'github',
          component: 'BareReviewsController',
        })
      );
    ...
    it('manages the open/close state of the summary section', async function () {
      assert.isTrue(
        await wrapper.find(ReviewsView).prop('summarySectionOpen'),
        true
      );
    ...
    it('manages the open/close state of the comment section', async function () {
      assert.isTrue(
        await wrapper.find(ReviewsView).prop('commentSectionOpen'),
        true
      );
