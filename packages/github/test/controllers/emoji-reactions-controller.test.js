...
describe('EmojiReactionsController', function () {
  ...
  describe('adding a reaction', function () {
    it('fires the add reaction mutation', async function () {
      ...
      relayEnv
        .getStore()
        .getSource()
        .set('issue0', { ...createRecord('issue0', 'Issue'), ...reactable });
    ...
    it('reports errors encountered', async function () {
      ...
      relayEnv
        .getStore()
        .getSource()
        .set('issue1', { ...createRecord('issue1', 'Issue'), ...reactable });
  ...
  describe('removing a reaction', function () {
    it('fires the remove reaction mutation', async function () {
      ...
      await wrapper.find(EmojiReactionsView).prop('removeReaction')(
        'THUMBS_DOWN'
      );
    ...
    it('reports errors encountered', async function () {
      ...
      expectRelayQuery(
        ...
        (op) => {
          return relayResponseBuilder(op).addError('wtf').build();
        }
      ).resolve();
