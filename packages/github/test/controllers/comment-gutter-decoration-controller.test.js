...
describe('CommentGutterDecorationController', function () {
  ...
  it('decorates the comment gutter', function () {
    ...
    assert.deepEqual(
      marker.prop('bufferRange'),
      new Range([420, 0], [420, Infinity])
    );
    ...
    assert.strictEqual(
      decoration.children('button.icon.icon-comment').length,
      1
    );
  });
  ...
  it('opens review dock and jumps to thread when clicked', async function () {
    ...
    assert.isTrue(
      reporterProxy.addEvent.calledWith('open-review-thread', {
        package: 'github',
