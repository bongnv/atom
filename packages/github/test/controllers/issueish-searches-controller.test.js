...
describe('IssueishSearchesController', function () {
  ...
  it('passes a handler to open an issueish pane and reports an event', async function () {
    ...
    const issueish = new Issueish({
      ...
      author: {
        login: 'smashwilson',
        avatarUrl: 'https://avatars0.githubusercontent.com/u/17565?s=40&v=4',
      },
  ...
  it('passes a handler to open reviews and reports an event', async function () {
    ...
    const issueish = new Issueish({
      ...
      author: {
        login: 'kuychaco',
        avatarUrl: 'https://avatars3.githubusercontent.com/u/7910250?v=4',
      },
      createdAt: '2019-04-01T10:00:00',
      repository: { id: '0' },
      commits: { nodes: [] },
    ...
    assert.isTrue(
      atomEnv.workspace.open.calledWith(
        `atom-github://reviews/github.com/atom/github/2084?workdir=${encodeURIComponent(
          __dirname
        )}`
      )
    );
    assert.isTrue(
      reporterProxy.addEvent.calledWith('open-reviews-tab', {
        package: 'github',
        from: 'IssueishSearchesController',
      })
    );
  });
});
