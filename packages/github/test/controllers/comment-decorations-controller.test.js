...
describe('CommentDecorationsController', function () {
  ...
  function buildApp(override = {}) {
    ...
    const { commentThreads } = aggregatedReviewsBuilder()
      .addReviewThread((t) => {
        t.addComment((c) =>
          c.id('0').path('file0.txt').position(2).bodyHTML('one')
        );
      })
      .addReviewThread((t) => {
        t.addComment((c) =>
          c.id('1').path('file1.txt').position(15).bodyHTML('two')
        );
      })
      .addReviewThread((t) => {
        t.addComment((c) =>
          c.id('2').path('file2.txt').position(7).bodyHTML('three')
        );
      })
      .addReviewThread((t) => {
        t.addComment((c) =>
          c.id('3').path('file2.txt').position(10).bodyHTML('four')
        );
      })
      .build();
  ...
  describe('renders EditorCommentDecorationsController and Gutter', function () {
    ...
    beforeEach(async function () {
      ...
      editor1 = await atomEnv.workspace.open(
        path.join(__dirname, 'another-unrelated-file.txt')
      );
    ...
    it('a pair per matching opened editor', function () {
      assert.strictEqual(
        wrapper.find('EditorCommentDecorationsController').length,
        2
      );
  ...
  it('skips comment thread with only minimized comments', async function () {
    const { commentThreads } = aggregatedReviewsBuilder()
      ...
      .addReviewThread((t) => {
        t.addComment((c) =>
          c.id('1').path('file1.txt').position(15).bodyHTML('three')
        );
      })
      .build();
    await atomEnv.workspace.open(path.join(__dirname, 'file0.txt'));
  ...
  describe('opening the reviews tab with a command', function () {
    it('opens the correct tab', function () {
      ...
      const command = wrapper.find(
        'Command[command="github:open-reviews-tab"]'
      );
