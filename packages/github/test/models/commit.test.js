...
describe('Commit', function () {
  ...
  describe('isEqual()', function () {
    ...
    it('returns false if author differs', function () {
      const a = commitBuilder()
        .addAuthor('Tilde Ann Thurium', 'tthurium@gmail.com')
        .build();

      const b = commitBuilder()
        .addAuthor('Vanessa Yuen', 'vyuen@gmail.com')
        .build();
      assert.isFalse(a.isEqual(b));
    });
    ...
    it('returns false if a co-author differs', function () {
      ...
      const b0 = commitBuilder()
        .addCoAuthor('me@email.com', 'me')
        .addCoAuthor('extra@email.com', 'extra')
        .build();
      assert.isFalse(a.isEqual(b0));

      const b1 = commitBuilder()
        .addCoAuthor('me@email.com', 'different')
        .build();
    ...
    it('returns false if the diff... differs', function () {
      const a = commitBuilder()
        .setMultiFileDiff((mfp) => {
          mfp.addFilePatch((fp) => {
            fp.addHunk((hunk) =>
              hunk.unchanged('-').added('plus').deleted('minus').unchanged('-')
            );
          });
        })
        .build();
