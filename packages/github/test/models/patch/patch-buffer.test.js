...
describe('PatchBuffer', function () {
  ...
  it('creates and finds markers on specified layers', function () {
    const patchMarker = patchBuffer.markRange('patch', [
      [1, 0],
      [2, 4],
    ]);
    const hunkMarker = patchBuffer.markRange('hunk', [
      [2, 0],
      [3, 4],
    ]);
    ...
    assert.sameDeepMembers(patchBuffer.findAllMarkers({}), [
      patchMarker,
      hunkMarker,
    ]);
  });
  ...
  describe('createSubBuffer', function () {
    it('creates a new PatchBuffer containing a subset of the original', function () {
      const m0 = patchBuffer.markRange('patch', [
        [1, 0],
        [3, 0],
      ]); // before
      const m1 = patchBuffer.markRange('hunk', [
        [2, 0],
        [4, 2],
      ]); // before, ending at the extraction point
      const m2 = patchBuffer.markRange('hunk', [
        [4, 2],
        [5, 0],
      ]); // within
      const m3 = patchBuffer.markRange('patch', [
        [6, 0],
        [7, 1],
      ]); // within
      const m4 = patchBuffer.markRange('hunk', [
        [7, 1],
        [9, 0],
      ]); // after, starting at the extraction point
      const m5 = patchBuffer.markRange('patch', [
        [8, 0],
        [10, 0],
      ]); // after

      const { patchBuffer: subPatchBuffer, markerMap } =
        patchBuffer.createSubBuffer([
          [4, 2],
          [7, 1],
        ]);
      ...
      assert.deepEqual(m0.getRange().serialize(), [
        [1, 0],
        [3, 0],
      ]);
      assert.deepEqual(m1.getRange().serialize(), [
        [2, 0],
        [4, 2],
      ]);
      assert.deepEqual(m2.getRange().serialize(), [
        [4, 2],
        [5, 0],
      ]);
      assert.deepEqual(m3.getRange().serialize(), [
        [6, 0],
        [7, 1],
      ]);
      assert.deepEqual(m4.getRange().serialize(), [
        [7, 1],
        [9, 0],
      ]);
      assert.deepEqual(m5.getRange().serialize(), [
        [8, 0],
        [10, 0],
      ]);
      ...
      assert.strictEqual(
        subPatchBuffer.getBuffer().getText(),
        dedent`
        04
        0005
        0006
      ...
      assert.deepEqual(markerMap.get(m2).getRange().serialize(), [
        [0, 0],
        [1, 0],
      ]);
      assert.deepEqual(markerMap.get(m3).getRange().serialize(), [
        [2, 0],
        [3, 1],
      ]);
    });
    ...
    it('includes markers that cross into or across the chosen range', function () {
      ...
      patchBuffer.markRange('hunk', [
        [6, 1],
        [8, 0],
      ]); // within -> after
      patchBuffer.markRange('addition', [
        [1, 0],
        [9, 4],
      ]); // before -> after

      const { patchBuffer: subPatchBuffer } = patchBuffer.createSubBuffer([
        [3, 1],
        [7, 3],
      ]);
      assert.strictEqual(
      ...
      assertMarkerRanges(subPatchBuffer.getLayer('patch'), [
        [0, 0],
        [1, 2],
      ]);
      assertMarkerRanges(subPatchBuffer.getLayer('hunk'), [
        [3, 1],
        [4, 3],
      ]);
      assertMarkerRanges(subPatchBuffer.getLayer('addition'), [
        [0, 0],
        [4, 3],
      ]);
    });
  ...
  describe('extractPatchBuffer', function () {
    it('extracts a subset of the buffer and layers as a new PatchBuffer', function () {
      ...
      const { patchBuffer: subPatchBuffer, markerMap } =
        patchBuffer.extractPatchBuffer([
          [4, 2],
          [7, 1],
        ]);

      assert.strictEqual(
        patchBuffer.getBuffer().getText(),
        dedent`
        0000
        0001
        0002
    ...
    it("does not destroy excluded markers at the extraction range's boundaries", function () {
      const before0 = patchBuffer.markRange('patch', [
        [2, 0],
        [2, 0],
      ]);
      const before1 = patchBuffer.markRange('patch', [
        [2, 0],
        [2, 0],
      ]);
      const within0 = patchBuffer.markRange('patch', [
        [2, 0],
        [2, 1],
      ]);
      const within1 = patchBuffer.markRange('patch', [
        [3, 0],
        [3, 4],
      ]);
      const within2 = patchBuffer.markRange('patch', [
        [4, 0],
        [4, 0],
      ]);
      const after0 = patchBuffer.markRange('patch', [
        [4, 0],
        [4, 0],
      ]);
      const after1 = patchBuffer.markRange('patch', [
        [4, 0],
        [4, 0],
      ]);
  ...
  describe('deleteLastNewline', function () {
    ...
    it('deletes at most one trailing newline', function () {
      ...
      assert.strictEqual(
        endsWithMultiNL.getBuffer().getText(),
        '0\n1\n2\n3\n\n'
      );
    });
  });
  ...
  describe('adopt', function () {
    it('destroys existing content and markers and replaces them with those from the argument', function () {
      ...
      const adopteeMarkers = [
        adoptee.markRange('addition', [
          [2, 0],
          [3, 7],
        ]),
        adoptee.markRange('patch', [
          [1, 0],
          [2, 0],
        ]),
      ];
      ...
      for (const adopteeMarker of adopteeMarkers) {
        ...
        assert.deepEqual(
          adopteeMarker.getRange().serialize(),
  ...
  describe('deferred-marking modifications', function () {
    ...
    it('preserves markers that should be before or after the modification region', function () {
      const invalidBefore = patchBuffer.markRange('patch', [
        [1, 0],
        [1, 0],
      ]);
      const before0 = patchBuffer.markRange(
        'patch',
        [
          [1, 0],
          [4, 0],
      ...
      const before1 = patchBuffer.markRange(
        'hunk',
        [
          [4, 0],
          [4, 0],
      ...
      const before2 = patchBuffer.markRange(
        'addition',
        [
          [3, 0],
          [4, 0],
        ],
        { exclusive: true, reversed: true }
      );
      const before3 = patchBuffer.markRange(
        'nonewline',
        [
          [4, 0],
          [4, 0],
      ...
      const after1 = patchBuffer.markRange(
        'patch',
        [
          [4, 0],
          [4, 3],
      ...
      const after2 = patchBuffer.markRange(
        'deletion',
        [
          [4, 0],
          [5, 0],
      ...
      const after3 = patchBuffer.markRange(
        'nonewline',
        [
          [4, 0],
          [4, 0],
      ...
      const invalidAfter = patchBuffer.markRange('hunk', [
        [5, 0],
        [6, 0],
      ]);
      ...
      assert.deepEqual(before0.getRange().serialize(), [
        [1, 0],
        [4, 0],
      ]);
      assert.deepEqual(before1.getRange().serialize(), [
        [4, 0],
        [4, 0],
      ]);
      assert.deepEqual(before2.getRange().serialize(), [
        [3, 0],
        [4, 0],
      ]);
      assert.deepEqual(before3.getRange().serialize(), [
        [4, 0],
        [4, 0],
      ]);
      assert.deepEqual(marker.getRange().serialize(), [
        [4, 0],
        [9, 0],
      ]);
      assert.deepEqual(after0.getRange().serialize(), [
        [9, 0],
        [9, 0],
      ]);
      assert.deepEqual(after1.getRange().serialize(), [
        [9, 0],
        [9, 3],
      ]);
      assert.deepEqual(after2.getRange().serialize(), [
        [9, 0],
        [10, 0],
      ]);
      assert.deepEqual(after3.getRange().serialize(), [
        [9, 0],
        [9, 0],
      ]);

      assert.deepEqual(invalidBefore.getRange().serialize(), [
        [1, 0],
        [1, 0],
      ]);
      assert.deepEqual(invalidAfter.getRange().serialize(), [
        [10, 0],
        [11, 0],
      ]);
    });
    ...
    it('appends another PatchBuffer at its insertion point', function () {
      ...
      const m1 = subPatchBuffer.markRange('hunk', [
        [0, 0],
        [1, 4],
      ]);
      const m2 = subPatchBuffer.markRange('addition', [
        [1, 2],
        [2, 2],
      ]);

      const mBefore = patchBuffer.markRange('deletion', [
        [0, 0],
        [2, 0],
      ]);
      const mAfter = patchBuffer.markRange('deletion', [
        [7, 0],
        [7, 4],
      ]);
      ...
      assert.deepEqual(mBefore.getRange().serialize(), [
        [0, 0],
        [2, 0],
      ]);
      assert.deepEqual(mAfter.getRange().serialize(), [
        [9, 0],
        [9, 4],
      ]);
      ...
      assert.deepEqual(markerMap.get(m0).getRange().serialize(), [
        [3, 2],
        [3, 2],
      ]);
      assert.deepEqual(markerMap.get(m1).getRange().serialize(), [
        [3, 2],
        [4, 4],
      ]);
      assert.deepEqual(markerMap.get(m2).getRange().serialize(), [
