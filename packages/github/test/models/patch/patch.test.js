...
describe('Patch', function () {
  ...
  it('returns an empty Range at the beginning of its Marker', function () {
    ...
    assert.deepEqual(patch.getStartRange().serialize(), [
      [0, 0],
      [0, 0],
    ]);
  });
  ...
  describe('stage patch generation', function () {
    ...
    it('marks ranges for each change region on the correct marker layer', function () {
      ...
      const layerRanges = [
        ...
        obj[layerName] = stagePatchBuffer
          .findMarkers(layerName, {})
  ...
  describe('unstage patch generation', function () {
    ...
    it('marks ranges for each change region on the correct marker layer', function () {
      ...
      const layerRanges = [
        ...
        obj[layerName] = unstagePatchBuffer
          .findMarkers(layerName, {})
          .map((marker) => marker.getRange().serialize());
        return obj;
      }, {});
  ...
  it('has a stubbed nullPatch counterpart', function () {
    ...
    assert.deepEqual(nullPatch.getMarker().getRange().serialize(), [
      [0, 0],
      [0, 0],
    ]);
    assert.deepEqual(nullPatch.getRange().serialize(), [
      [0, 0],
      [0, 0],
    ]);
    assert.deepEqual(nullPatch.getStartRange().serialize(), [
      [0, 0],
      [0, 0],
    ]);
