...
describe('MultiFilePatch', function () {
  ...
  describe('clone', function () {
    ...
    it('creates a copy with a new PatchBuffer', function () {
      ...
      const dup = original.clone({
        patchBuffer: multiFilePatch.getPatchBuffer(),
      });
      ...
      assert.strictEqual(
        dup.getUnchangedLayer(),
        multiFilePatch.getUnchangedLayer()
      );
      assert.strictEqual(
        dup.getAdditionLayer(),
        multiFilePatch.getAdditionLayer()
      );
      assert.strictEqual(
        dup.getDeletionLayer(),
        multiFilePatch.getDeletionLayer()
      );
      assert.strictEqual(
        dup.getNoNewlineLayer(),
        multiFilePatch.getNoNewlineLayer()
      );
      assert.strictEqual(dup.getFilePatches(), original.getFilePatches());
    });
  ...
  it('locates a Hunk by marker lookup', function () {
    const { multiFilePatch } = multiFilePatchBuilder()
      ...
      .addFilePatch((fp) => {
        fp.addHunk((h) =>
          h.oldRow(5).unchanged('10', '11').added('12').deleted('13')
        );
  ...
  it('adopts a new buffer', function () {
    const { multiFilePatch } = multiFilePatchBuilder()
      .addFilePatch((fp) => {
        ...
        fp.addHunk((h) =>
          h.unchanged('a0').added('a1').deleted('a2').unchanged('a3')
        );
      })
      .addFilePatch((fp) => {
    ...
    assert.strictEqual(
      nextBuffer.getLayer('patch'),
      multiFilePatch.getPatchLayer()
    );
    assert.strictEqual(
      nextBuffer.getLayer('hunk'),
      multiFilePatch.getHunkLayer()
    );
    assert.strictEqual(
      nextBuffer.getLayer('unchanged'),
      multiFilePatch.getUnchangedLayer()
    );
    assert.strictEqual(
      nextBuffer.getLayer('addition'),
      multiFilePatch.getAdditionLayer()
    );
    assert.strictEqual(
      nextBuffer.getLayer('deletion'),
      multiFilePatch.getDeletionLayer()
    );
    assert.strictEqual(
      nextBuffer.getLayer('nonewline'),
      multiFilePatch.getNoNewlineLayer()
    );

    assert.deepEqual(
      nextBuffer.getBuffer().getText(),
      dedent`
      a0
      a1
      a2
    ...
    assert.strictEqual(
      multiFilePatch.getBufferRowForDiffPosition('A0.txt', 1),
      0
    );
    assert.strictEqual(
      multiFilePatch.getBufferRowForDiffPosition('A1.txt', 5),
      7
    );
  });
  ...
  describe('maximum selection index', function () {
    ...
    it('returns the ordinal index of the highest selected change row', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          fp.addHunk((h) =>
            h.unchanged('.').added('0', '1', 'x *').unchanged('.')
          );
          fp.addHunk((h) =>
            h.unchanged('.').deleted('2').added('3').unchanged('.')
          );
        })
        .addFilePatch((fp) => {
          fp.addHunk((h) =>
            h.unchanged('.').deleted('4', '5 *', '6').unchanged('.')
          );
      ...
      assert.strictEqual(
        multiFilePatch.getMaxSelectionIndex(new Set([3, 11])),
        5
      );
    });
  });
  ...
  describe('selection range by change index', function () {
    it('selects the last change row if no longer present', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          fp.addHunk((h) =>
            h.unchanged('.').added('0', '1', '2').unchanged('.')
          );
          fp.addHunk((h) =>
            h.unchanged('.').deleted('3').added('4').unchanged('.')
          );
        })
        .addFilePatch((fp) => {
          fp.addHunk((h) =>
            h.unchanged('.').deleted('5', '6', '7').unchanged('.')
          );
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(9).serialize(),
        [
          [15, 0],
          [15, Infinity],
        ]
    ...
    it('returns the range of the change row by ordinal', function () {
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(0).serialize(),
        [
          [1, 0],
          [1, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(1).serialize(),
        [
          [2, 0],
          [2, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(2).serialize(),
        [
          [3, 0],
          [3, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(3).serialize(),
        [
          [6, 0],
          [6, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(4).serialize(),
        [
          [7, 0],
          [7, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(5).serialize(),
        [
          [10, 0],
          [10, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(6).serialize(),
        [
          [11, 0],
          [11, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(7).serialize(),
        [
          [12, 0],
          [12, Infinity],
        ]
      ...
      assert.deepEqual(
        multiFilePatch.getSelectionRangeForIndex(8).serialize(),
        [
          [15, 0],
          [15, Infinity],
        ]
    ...
    beforeEach(function () {
      multiFilePatch = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('0').added('1').deleted('2', '3').unchanged('4')
          );
        ...
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('8').deleted('9', '10').unchanged('11')
          );
        })
        .build().multiFilePatch;
    });
  ...
  describe('getPreviewPatchBuffer', function () {
    it('returns a PatchBuffer containing nearby rows of the MultiFilePatch', function () {
      ...
      assertMarkerRanges(subPatch.getLayer('patch'), [
        [0, 0],
        [3, 1],
      ]);
      assertMarkerRanges(subPatch.getLayer('hunk'), [
        [0, 0],
        [3, 1],
      ]);
      assertMarkerRanges(subPatch.getLayer('unchanged'), [
        [1, 0],
        [1, 1],
      ]);
  ...
  describe('diff position translation', function () {
    ...
    it('offsets rows by the number of hunks before the diff row', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('10 (13)').deleted('11 (14)').unchanged('12 (15)')
          );
        })
        .build();
    ...
    it('resets the offset at the start of each file patch', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('0 (1)').added('1 (2)', '2 (3)').unchanged('3 (4)')
          ); // Offset +1
          fp.addHunk((h) =>
            h
              .unchanged('4 (6)')
          ...
          fp.addHunk((h) =>
            h.unchanged('9 (12)').deleted('10 (13)').unchanged('11 (14)')
          ); // Offset +3
        })
        .addFilePatch((fp) => {
          fp.setOldFile((f) => f.path('1.txt'));
          fp.addHunk((h) =>
            h.unchanged('12 (1)').added('13 (2)').unchanged('14 (3)')
          ); // Offset +1
          fp.addHunk((h) =>
            h
              .unchanged('15 (5)')
        ...
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('30 (13)').added('31 (14)').unchanged('32 (15)')
          ); // Offset +3
        })
        .build();
    ...
    it('set the offset for diff-gated file patch upon expanding', function () {
      ...
      assert.strictEqual(
        multiFilePatch.getBufferRowForDiffPosition('1.txt', 11),
        9
      );
    });
    ...
    it('does not reset the offset for normally collapsed file patch upon expanding', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          fp.setOldFile((f) => f.path('0.txt'));
          fp.addHunk((h) =>
            h.oldRow(1).unchanged('0-0').deleted('0-1', '0-2').unchanged('0-3')
          );
        })
        .build();
  ...
  describe('collapsing and expanding file patches', function () {
    ...
    describe('when there is a single file patch', function () {
      it('collapses and expands the only file patch', function () {
        ...
        assert.strictEqual(
          multiFilePatch.getBuffer().getText(),
          patchTextForIndexes([0])
        );
        assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 0, start: 0, last: true })
        );
      });
    });
    ...
    describe('when there are multiple file patches', function () {
      ...
      it('collapses and expands the first file patch with all following expanded', function () {
        ...
        assert.strictEqual(
          multiFilePatch.getBuffer().getText(),
          patchTextForIndexes([0, 1, 2, 3])
        );

        assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 0, start: 0 })
        );
        assertInFilePatch(fp1, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 1, start: 4 })
        );
        assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 2, start: 8 })
        );
        assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 3, start: 12, last: true })
        );
      });
      ...
      it('collapses and expands an intermediate file patch while all previous patches are collapsed', function () {
        ...
        assert.strictEqual(
          multiFilePatch.getBuffer().getText(),
          patchTextForIndexes([3])
        );
        assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp1, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 3, start: 0, last: true })
        );
        ...
        assert.strictEqual(
          multiFilePatch.getBuffer().getText(),
          patchTextForIndexes([2, 3])
        );

        assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp1, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 2, start: 0 })
        );
        assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 3, start: 4, last: true })
        );
      });
      ...
      it('collapses and expands an intermediate file patch while all following patches are collapsed', function () {
        ...
        assertInFilePatch(fp1, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 1, start: 4, last: true })
        );
        assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks();
        assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks();
      });
      ...
      it('collapses and expands the final file patch with all previous expanded', function () {
        ...
        assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks(
          hunk({ index: 2, start: 8, last: true })
        );
        assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks();
      ...
      describe('when all patches are collapsed', function () {
        ...
        it('expands a non-first file patch', function () {
          ...
          assert.strictEqual(
            multiFilePatch.getBuffer().getText(),
            patchTextForIndexes([2])
          );

          assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks();
          assertInFilePatch(fp1, multiFilePatch.getBuffer()).hunks();
          assertInFilePatch(fp2, multiFilePatch.getBuffer()).hunks(
            hunk({ index: 2, start: 0, last: true })
          );
          assertInFilePatch(fp3, multiFilePatch.getBuffer()).hunks();
        });
      ...
      it('is deterministic regardless of the order in which collapse and expand operations are performed', function () {
        ...
        function expectVisibleAfter(ops, i) {
          return ops.reduce(
            (visible, op) => (op.index === i ? op.visibleAfter : visible),
            true
          );
        }
        ...
        for (const sequence of operationSequences) {
          ...
          assert.strictEqual(
            multiFilePatch.getBuffer().getText(),
            patchTextForIndexes(visibleIndexes)
          );

          let start = 0;
          for (let i = 0; i < patches.length; i++) {
            const patchAssertions = assertInFilePatch(
              patches[i],
              multiFilePatch.getBuffer()
            );
            if (patches[i].getRenderStatus().isVisible()) {
              patchAssertions.hunks(
                hunk({ index: i, start, last: lastVisibleIndex === i })
              );
              start += 4;
            } else {
              patchAssertions.hunks();
    ...
    describe('when a file patch has no content', function () {
      it('collapses and expands', function () {
        ...
        assert.strictEqual(multiFilePatch.getBuffer().getText(), '');
        assertInFilePatch(fp0, multiFilePatch.getBuffer()).hunks();
        assert.deepEqual(fp0.getMarker().getRange().serialize(), [
          [0, 0],
          [0, 0],
        ]);
      ...
      it('does not insert a trailing newline when expanding a final content-less patch', function () {
        ...
        assert.deepEqual(fp1.getMarker().getRange().serialize(), [
          [2, 1],
          [2, 1],
        ]);
