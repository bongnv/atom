...
describe('Hunk', function () {
  ...
  it('updates markers from a marker map', function () {
    const oMarker = buffer.markRange([
      [0, 0],
      [10, 4],
    ]);
    ...
    const regionUpdateMaps = h
      .getRegions()
      .map((r) => sinon.spy(r, 'updateMarkers'));

    const layer = buffer.addMarkerLayer();
    const nMarker = layer.markRange([
      [0, 0],
      [10, 4],
    ]);
  ...
  it('destroys all of its markers', function () {
    ...
    const allMarkers = [
      h.getMarker(),
      ...h.getRegions().map((r) => r.getMarker()),
    ];
