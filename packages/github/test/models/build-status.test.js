...
describe('BuildStatus', function () {
  ...
  it('interprets an unexpected check conclusion', function () {
    assert.deepEqual(
      buildStatusFromCheckResult({ status: 'COMPLETED', conclusion: 'HUH' }),
      {
        icon: 'unverified',
        classSuffix: 'pending',
      }
    ...
    it('ignores NEUTRAL', function () {
      ...
      for (const neutralStatus of neutrals) {
        for (const otherStatus of all) {
          assert.deepEqual(
            combineBuildStatuses(otherStatus, neutralStatus),
            otherStatus
          );
          assert.deepEqual(
            combineBuildStatuses(neutralStatus, otherStatus),
            otherStatus
          );
        }
      }
