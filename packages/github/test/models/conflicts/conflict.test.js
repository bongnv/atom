...
>>>>>>> master
  ...
  it('accesses a side range that encompasses the banner and content', async function () {
    ...
    assert.deepEqual(conflict.getSide(OURS).getRange().serialize(), [
      [0, 0],
      [2, 0],
    ]);
    assert.deepEqual(conflict.getSide(BASE).getRange().serialize(), [
      [2, 0],
      [4, 0],
    ]);
    assert.deepEqual(conflict.getSide(THEIRS).getRange().serialize(), [
      [5, 0],
      [7, 0],
    ]);
  });
