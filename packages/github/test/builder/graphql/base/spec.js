...
export class FragmentSpec extends Spec {
  ...
  getLinkedSpecCreator(name) {
    const subNodes = [];
    for (const node of this.nodes) {
      const match = node.selections.find(
        (selection) => selection.alias === name || selection.name === name
      );
      if (match) {
        subNodes.push(match);
      }
...
export class QuerySpec extends Spec {
    ...
    const flattenFragments = (selections) => {
      ...
      for (let i = selections.length - 1; i >= 0; i--) {
        if (selections[i].kind === 'InlineFragment') {
          ...
          if (
            selections[i].typeCondition.kind !== 'NamedType' ||
            !typeNameSet.has(selections[i].typeCondition.name.value)
          ) {
            continue;
          }
          ...
          if (!fragment) {
            throw new Error(
              `Reference to unknown fragment: ${selections[i].name.value}`
            );
          }
          if (
            fragment.typeCondition.kind !== 'NamedType' ||
            !typeNameSet.has(fragment.typeCondition.name.value)
          ) {
            continue;
          }
  ...
  getLinkedSpecCreator(name) {
    for (const selection of this.root.selectionSet.selections) {
      ...
      if (fieldName === name) {
        return (typeNameSet) =>
          new QuerySpec(selection, typeNameSet, this.fragmentsByName);
      }
    }
