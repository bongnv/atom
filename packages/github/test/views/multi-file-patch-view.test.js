...
describe('MultiFilePatchView', function () {
  ...
  it('passes the new path of renamed files', function () {
    ...
    assert.strictEqual(
      wrapper.find('FilePatchHeaderView').prop('newPath'),
      'new.txt'
    );
  });
  ...
  describe('file header decoration positioning', function () {
    ...
    function decorationForFileHeader(fileName) {
      return wrapper
        .find('Decoration')
        .filterWhere((dw) =>
          dw.exists(`FilePatchHeaderView[relPath="${fileName}"]`)
        );
    }
    ...
    it('renders visible file headers with position: before', function () {
      ...
      assert.strictEqual(
        decorationForFileHeader('0.txt').prop('position'),
        'before'
      );
      assert.strictEqual(
        decorationForFileHeader('1.txt').prop('position'),
        'before'
      );
      assert.strictEqual(
        decorationForFileHeader('2.txt').prop('position'),
        'before'
      );
    });
    ...
    it('renders a final collapsed file header with position: after', function () {
      ...
      assert.strictEqual(
        decorationForFileHeader('3.txt').prop('position'),
        'before'
      );
      assert.strictEqual(
        decorationForFileHeader('4.txt').prop('position'),
        'after'
      );
    });
    ...
    it('renders a gutter decoration for each review thread if itemType is IssueishDetailItem', function () {
      const { multiFilePatch } = multiFilePatchBuilder()
        .addFilePatch((fp) => {
          ...
          fp.addHunk((h) =>
            h.unchanged('0', '1', '2').added('3').unchanged('4', '5')
          );
        })
        .addFilePatch((fp) => {
      ...
      assert.deepEqual(controller1.prop('extraClasses'), [
  ...
  it('renders the file patch within an editor', function () {
    ...
    assert.strictEqual(
      editor.instance().getModel().getText(),
  ...
  it('selects the next full hunk when a new file patch arrives in hunk selection mode', function () {
    ...
    assert.sameMembers(
      Array.from(selectedRowsChanged.lastCall.args[0]),
      [6, 7]
    );
  ...
  describe('hunk headers', function () {
    ...
    it('pluralizes the toggle and discard button labels', function () {
      ...
      assert.strictEqual(
        wrapper.find('HunkHeaderView').at(0).prop('toggleSelectionLabel'),
        'Stage Selected Line'
      );
      assert.strictEqual(
        wrapper.find('HunkHeaderView').at(0).prop('discardSelectionLabel'),
        'Discard Selected Line'
      );
      assert.strictEqual(
        wrapper.find('HunkHeaderView').at(1).prop('toggleSelectionLabel'),
        'Stage Hunk'
      );
      assert.strictEqual(
        wrapper.find('HunkHeaderView').at(1).prop('discardSelectionLabel'),
        'Discard Hunk'
      );
    ...
    it('handles mousedown as a selection event', function () {
      ...
      wrapper.find('HunkHeaderView').at(1).prop('mouseDown')(
        { button: 0 },
        mfp.getFilePatches()[0].getHunks()[1]
      );
  ...
  describe('custom gutters', function () {
    ...
    it('selects a range of lines on click and drag', function () {
      ...
      instance.didMouseMoveOnLineNumber({
        bufferRow: 2,
        domEvent: { button: 0 },
      });
      assert.deepEqual(
    ...
    describe('shift-click', function () {
      ...
      it("extends to the range's beginning when the selection is reversed", function () {
        ...
        assert.deepEqual(
          editor.getLastSelection().getBufferRange().serialize(),
          [
            [2, 0],
            [6, 4],
          ]
  ...
  describe('registers Atom commands', function () {
    ...
    it('registers file mode and symlink commands depending on the staging status', function () {
      ...
      const wrapper = shallow(
        buildApp({ multiFilePatch, stagingStatus: 'unstaged' })
      );

      assert.isTrue(
        wrapper.exists('Command[command="github:stage-file-mode-change"]')
      );
      assert.isTrue(
        wrapper.exists('Command[command="github:stage-symlink-change"]')
      );
      assert.isFalse(
        wrapper.exists('Command[command="github:unstage-file-mode-change"]')
      );
      assert.isFalse(
        wrapper.exists('Command[command="github:unstage-symlink-change"]')
      );
    ...
    it('does nothing when there is no last discard to undo', function () {
      const undoLastDiscard = sinon.spy();
      const wrapper = mount(
        buildApp({ undoLastDiscard, hasUndoHistory: false })
      );
  ...
  describe('large diff gate', function () {
    ...
    it('displays diff gate when diff exceeds a certain number of lines', function () {
      assert.include(
        wrapper
          .find(
            '.github-FilePatchView-controlBlock .github-FilePatchView-message'
          )
          .first()
          .text(),
      ...
      assert.isTrue(
        wrapper.find('.github-FilePatchView-showDiffButton').exists()
      );
    ...
    it('loads large diff and sends event when show diff button is clicked', function () {
      ...
      wrapper
        .find('.github-FilePatchView-showDiffButton')
        .first()
        .simulate('click');
      wrapper.update();

      assert.isTrue(addEventStub.calledOnce);
      assert.deepEqual(addEventStub.lastCall.args, [
        'expand-file-patch',
        { component: 'MultiFilePatchView', package: 'github' },
      ]);
