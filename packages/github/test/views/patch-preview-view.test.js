...
describe('PatchPreviewView', function () {
  ...
  beforeEach(function () {
    ...
    multiFilePatch = multiFilePatchBuilder()
      .addFilePatch((fp) => {
        ...
        fp.addHunk((h) =>
          h.unchanged('000').added('001', '002').deleted('003', '004')
        );
        fp.addHunk((h) =>
          h.unchanged('005').deleted('006').added('007').unchanged('008')
        );
      })
  ...
  it('builds and renders sub-PatchBuffer content within a TextEditor', function () {
    const wrapper = mount(
      buildApp({ fileName: 'file.txt', diffRow: 5, maxRowCount: 4 })
    );
  ...
  it('retains sub-PatchBuffers, adopting new content if the MultiFilePatch changes', function () {
    const wrapper = mount(
      buildApp({ fileName: 'file.txt', diffRow: 4, maxRowCount: 4 })
    );
    const previewPatchBuffer = wrapper.state('previewPatchBuffer');
    assert.strictEqual(
      previewPatchBuffer.getBuffer().getText(),
      dedent`
      000
      001
      002
  ...
  it('decorates the addition and deletion marker layers', function () {
    ...
    assertMarkerRanges(additionLayer, [
      [0, 0],
      [1, 3],
    ]);
    ...
    assertMarkerRanges(deletionLayer, [
      [2, 0],
      [3, 3],
    ]);
  });
