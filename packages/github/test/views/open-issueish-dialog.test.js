...
import OpenIssueishDialog, {
  openIssueishItem,
} from '../../lib/views/open-issueish-dialog';
...
describe('OpenIssueishDialog', function () {
  ...
  describe('open button enablement', function () {
    ...
    it('enables the open button when issue url box is populated', function () {
      const wrapper = shallow(buildApp());
      wrapper
        .find('.github-OpenIssueish-url')
        .prop('buffer')
        .setText('https://github.com/atom/github/pull/1807');
  ...
  it('calls the acceptance callback with the entered URL', function () {
    ...
    assert.isTrue(
      accept.calledWith('https://github.com/atom/github/pull/1807')
    );
  });
  ...
  describe('openIssueishItem', function () {
    ...
    it('opens an item for a valid PR URL', async function () {
      sinon.stub(atomEnv.workspace, 'open').resolves('item');
      assert.strictEqual(
        await openIssueishItem(
          'https://github.com/smashwilson/az-coordinator/pull/10',
          {
            workspace: atomEnv.workspace,
            workdir: __dirname,
          }
    ...
    it('rejects with an error for an invalid URL', async function () {
      await assert.isRejected(
        openIssueishItem('https://azurefire.net/not-an-issue', {
          workspace: atomEnv.workspace,
          workdir: __dirname,
        }),
