...
describe('PrStatusesView', function () {
  ...
  it('renders nothing if the pull request has no status or checks', function () {
    ...
    const wrapper = shallow(buildApp({ pullRequest }))
      .find(CheckSuitesAccumulator)
      .renderProp('children')(NULL_CHECK_SUITE_RESULT);
  ...
  describe('with displayType: check', function () {
    it('renders a commit status result', function () {
      ...
      assert.isTrue(
        child0
          ...
          .hasClass('github-PrStatuses--pending')
      );
      ...
      assert.isTrue(
        child2
          ...
          .hasClass('github-PrStatuses--failure')
      );
      ...
      const child3 = wrapper
        .find(CheckSuitesAccumulator)
        .renderProp('children')(NULL_CHECK_SUITE_RESULT);
      assert.isTrue(
        child3.find('Octicon[icon="x"]').hasClass('github-PrStatuses--failure')
      );
    });
    ...
    it('renders a combined check suite result', function () {
      ...
      const suite00 = checkSuiteBuilder(checkRunsQuery)
        .checkRuns((conn) => {
          ...
          conn.addEdge((e) =>
            e.node((r) => r.status('COMPLETED').conclusion('SUCCESS'))
          );
        })
        .build();
      ...
      const suite11 = checkSuiteBuilder(checkRunsQuery)
        .checkRuns((conn) => {
          conn.addEdge((e) =>
            e.node((r) => r.status('COMPLETED').conclusion('FAILURE'))
          );
          conn.addEdge((e) =>
            e.node((r) => r.status('COMPLETED').conclusion('SUCCESS'))
          );
      ...
      assert.isTrue(
        child1.find('Octicon[icon="x"]').hasClass('github-PrStatuses--failure')
      );
    });
    ...
    it('combines a commit status and check suite results', function () {
      ...
      assert.isTrue(
        child0.find('Octicon[icon="x"]').hasClass('github-PrStatuses--failure')
      );
    });
  });
  ...
  describe('with displayType: full', function () {
    it('renders a donut chart', function () {
      ...
      const commit0 = commitBuilder(commitQuery)
        .checkSuites((conn) => {
          ...
          conn.addEdge((e) =>
            e.node((s) => s.status('COMPLETED').conclusion('FAILURE'))
          );
        })
        .build();
    ...
    describe('the summary sentence', function () {
      it('reports when all checks are successes', function () {
        ...
        const commit0 = commitBuilder(commitQuery)
          .checkSuites((conn) => {
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion('SUCCESS'))
            );
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion('SUCCESS'))
            );
      ...
      it('reports when all checks have failed', function () {
        ...
        const commit0 = commitBuilder(commitQuery)
          .checkSuites((conn) => {
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion('TIMED_OUT'))
            );
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion('FAILURE'))
            );
        ...
        const checkSuite0 = checkSuiteBuilder(checkRunsQuery)
          .checkRuns((conn) => {
            conn.addEdge((e) =>
              e.node((r) => r.status('COMPLETED').conclusion('TIMED_OUT'))
            );
            conn.addEdge((e) =>
              e.node((r) => r.status('COMPLETED').conclusion('TIMED_OUT'))
            );
        ...
        const checkSuite1 = checkSuiteBuilder(checkRunsQuery)
          .checkRuns((conn) => {
            ...
            conn.addEdge((e) =>
              e.node((r) => r.status('COMPLETED').conclusion('CANCELLED'))
            );
          })
          .build();
      ...
      it('reports counts of mixed results, with proper pluralization', function () {
        ...
        const commit0 = commitBuilder(commitQuery)
          .checkSuites((conn) => {
            ...
            conn.addEdge((e) => e.node((s) => s.status('IN_PROGRESS')));
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion('ERROR'))
            );
          })
          .build();
        const checkSuite0 = checkSuiteBuilder(checkRunsQuery)
        ...
        const checkSuite1 = checkSuiteBuilder(checkRunsQuery)
          .checkRuns((conn) => {
            ...
            conn.addEdge((e) =>
              e.node((r) => r.status('COMPLETED').conclusion('ACTION_REQUIRED'))
            );
          })
          .build();
      ...
      it('omits missing "pending" category', function () {
        ...
        const commit0 = commitBuilder(commitQuery)
          .checkSuites((conn) => {
            ...
            conn.addEdge((e) => e.node((s) => s.status('IN_PROGRESS')));
            conn.addEdge((e) =>
              e.node((s) => s.status('COMPLETED').conclusion(''))
            );
          })
          .build();
        const checkSuite0 = checkSuiteBuilder(checkRunsQuery)
    ...
    it('renders a context view for each status context', function () {
      const pullRequest = pullRequestBuilder(pullRequestQuery)
        ...
        .build();
      const [contexts] = pullRequest.recentCommits.edges.map(
        (e) => e.node.commit.status.contexts
      );

      const wrapper = shallow(buildApp({ pullRequest, displayType: 'full' }));
      const child = wrapper.find(CheckSuitesAccumulator).renderProp('children')(
        NULL_CHECK_SUITE_RESULT
      );

      const contextViews = child.find(PullRequestStatusContextView);
      assert.deepEqual(
        contextViews.map((v) => v.prop('context')),
        contexts
      );
    });
    ...
    describe('the PeriodicRefresher to update status checks', function () {
      it('refetches with the current pull request ID', function () {
        ...
        assert.isTrue(
          refetch.calledWith({ id: 'pr0' }, null, sinon.match.func, {
            force: true,
          })
        );
      ...
      it('is configured with a longer interval once all checks are completed', function () {
        ...
        assert.strictEqual(
          refresher.options.interval(),
          BarePrStatusesView.COMPLETED_REFRESH_TIMEOUT
        );
      });
      ...
      it('changes its interval when the component is re-rendered', function () {
        ...
        const wrapper = shallow(
          buildApp({ pullRequest: pullRequest0, displayType: 'full' })
        );
        const { refresher } = wrapper.instance();
        assert.strictEqual(
          refresher.options.interval(),
