...
describe('PullRequestDetailView', function () {
  ...
  it('renders pull request information', function () {
    ...
    const pullRequest = pullRequestBuilder(pullRequestQuery)
      ...
      .author((a) =>
        a
          .login('author0')
          .avatarUrl('https://avatars3.githubusercontent.com/u/1')
      )
      .build();
    ...
    assert.isDefined(
      wrapper
        .find(PullRequestStatusesView)
        .find('[displayType="check"]')
        .prop('pullRequest')
    );
    ...
    assert.isNotNull(
      wrapper.find(PullRequestTimelineController).prop('pullRequest')
    );
    assert.isNotNull(
      wrapper
        .find(PullRequestStatusesView)
        .find('[displayType="full"]')
        .prop('pullRequest')
    );
  ...
  it('renders ghost user if author is null', function () {
    ...
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-avatar').prop('href'),
      GHOST_USER.url
    );
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-avatarImage').prop('src'),
      GHOST_USER.avatarUrl
    );
    assert.strictEqual(
      wrapper.find('.github-IssueishDetailView-avatarImage').prop('alt'),
      GHOST_USER.login
    );
  });
  ...
  it('tells its tabs when the pull request is not checked out', function () {
    ...
    assert.isFalse(
      wrapper.find(PullRequestTimelineController).prop('onBranch')
    );
