...
describe('IssueishListView', function () {
  ...
  describe('with nonempty results', function () {
    ...
    it('renders a check if all status checks are successful', function () {
      ...
      const accumulated = result
        ...
        .renderProp('children')({ runsBySuite: new Map() });
      assert.strictEqual(
        accumulated
          .find('Octicon.github-IssueishList-item--status')
          .prop('icon'),
        'check'
      );
    });
    ...
    it('calls its `showActionsMenu` handler when the menu icon is clicked', function () {
      ...
      const result = wrapper.find('Accordion').renderProp('children')(mixed);
      const child = result.find(CheckSuitesAccumulator).renderProp('children')({
        runsBySuite: new Map(),
      });
  ...
  it('renders review button only if needed', function () {
    ...
    const wrapper = shallow(
      buildApp({ total: 1, issueishes: [allGreen], openReviews })
    );
    ...
    child1
      .find('.github-IssueishList-openReviewsButton')
