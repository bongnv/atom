...
describe('RepositoryHomeSelectionView', function () {
  ...
  it('translates loaded organizations and the current user as options for the select list', function () {
      ...
      .organizations((conn) => {
        conn.addEdge((edge) =>
          edge.node((o) => {
            ...
            o.login('enabled');
            o.avatarUrl(
              'https://avatars2.githubusercontent.com/u/1089146?s=24&v=4'
            );
        ...
        conn.addEdge((edge) =>
          edge.node((o) => {
            ...
            o.login('disabled');
            o.avatarUrl(
              'https://avatars1.githubusercontent.com/u/1507452?s=24&v=4'
            );
    ...
    assert.deepEqual(wrapper.find(TabbableSelect).prop('options'), [
      ...
      },
      {
        id: 'org0',
        login: 'enabled',
        avatarURL: 'https://avatars2.githubusercontent.com/u/1089146?s=24&v=4',
        disabled: false,
      },
      {
        id: 'org1',
        login: 'disabled',
        avatarURL: 'https://avatars1.githubusercontent.com/u/1507452?s=24&v=4',
        disabled: true,
      },
    ]);
  });
  ...
  it('uses custom renderers for the options and value', function () {
    ...
    assert.strictEqual(
      optionWrapper.find('img').prop('src'),
      'https://avatars2.githubusercontent.com/u/17565?s=24&v=4'
    );
    assert.strictEqual(
      optionWrapper.find('.github-RepositoryHome-ownerName').text(),
      'me'
    );
    assert.isFalse(
      optionWrapper.exists('.github-RepositoryHome-ownerUnwritable')
    );

    const org = user.organizations.edges[0].node;
    const valueWrapper = wrapper
      .find(TabbableSelect)
      .renderProp('valueRenderer')({
      id: org.id,
      login: org.login,
    ...
    assert.strictEqual(
      valueWrapper.find('img').prop('src'),
      'https://avatars2.githubusercontent.com/u/1089146?s=24&v=4'
    );
    assert.strictEqual(
      valueWrapper.find('.github-RepositoryHome-ownerName').text(),
      'atom'
    );
    assert.strictEqual(
      valueWrapper.find('.github-RepositoryHome-ownerUnwritable').text(),
      ...
      .organizations((conn) => {
        ...
        conn.addEdge((edge) =>
          edge.node((o) => {
            ...
            o.login('one');
            o.avatarUrl(
              'https://avatars3.githubusercontent.com/u/13409222?s=24&v=4'
            );
            o.viewerCanCreateRepositories(true);
          })
        );
