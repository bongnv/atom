...
describe('EmojiReactionsView', function () {
  ...
  it('renders reaction groups', function () {
    const reactable = issueBuilder(reactableQuery)
      .addReactionGroup((group) =>
        group.content('THUMBS_UP').users((u) => u.totalCount(10))
      )
      .addReactionGroup((group) =>
        group.content('THUMBS_DOWN').users((u) => u.totalCount(5))
      )
      .addReactionGroup((group) =>
        group.content('ROCKET').users((u) => u.totalCount(42))
      )
      .addReactionGroup((group) =>
        group.content('EYES').users((u) => u.totalCount(13))
      )
      .addReactionGroup((group) =>
        group.content('LAUGH').users((u) => u.totalCount(0))
      )
      .build();
  ...
  it('gracefully skips unknown emoji', function () {
    const reactable = issueBuilder(reactableQuery)
      .addReactionGroup((group) =>
        group.content('AVOCADO').users((u) => u.totalCount(11))
      )
      .build();
  ...
  it("shows which reactions you've personally given", function () {
    const reactable = issueBuilder(reactableQuery)
      .addReactionGroup((group) =>
        group
          .content('ROCKET')
          .users((u) => u.totalCount(5))
          .viewerHasReacted(true)
      )
      .addReactionGroup((group) =>
        group
          .content('EYES')
          .users((u) => u.totalCount(7))
    ...
    assert.isTrue(
      wrapper.find('.github-EmojiReactions-group.rocket').hasClass('selected')
    );
    assert.isFalse(
      wrapper.find('.github-EmojiReactions-group.eyes').hasClass('selected')
    );
  });
  ...
  it('disables the reaction toggle buttons if the viewer cannot react', function () {
    const reactable = issueBuilder(reactableQuery)
      ...
      .addReactionGroup((group) =>
        group.content('THUMBS_UP').users((u) => u.totalCount(2))
      )
      .build();
    ...
    assert.isTrue(
      wrapper.find('.github-EmojiReactions-group.thumbs_up').prop('disabled')
    );
  });
  ...
  it('displays an "add emoji" control if at least one reaction group is empty', function () {
    const reactable = issueBuilder(reactableQuery)
      ...
      .addReactionGroup((group) =>
        group.content('THUMBS_DOWN').users((u) => u.totalCount(0))
      )
      .build();
  ...
  it('does not display the "add emoji" control if all reaction groups are nonempty', function () {
    const reactable = issueBuilder(reactableQuery)
      .addReactionGroup((group) =>
        group.content('THUMBS_UP').users((u) => u.totalCount(1))
      )
      .addReactionGroup((group) =>
        group.content('THUMBS_DOWN').users((u) => u.totalCount(1))
      )
      .addReactionGroup((group) =>
        group.content('ROCKET').users((u) => u.totalCount(1))
      )
      .addReactionGroup((group) =>
        group.content('EYES').users((u) => u.totalCount(1))
      )
      .build();
