...
describe('GitTabView', function () {
  ...
  it('renders the identity panel when requested', async function () {
    ...
    assert.strictEqual(
      wrapper.find('GitIdentityView').prop('usernameBuffer'),
      usernameBuffer
    );
    assert.strictEqual(
      wrapper.find('GitIdentityView').prop('emailBuffer'),
      emailBuffer
    );
  });
  ...
  it('calls changeWorkingDirectory when a project is selected', async function () {
    ...
    wrapper.find('GitTabHeaderController').prop('changeWorkingDirectory')(
      'some-path'
    );
