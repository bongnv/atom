...
describe('ReviewCommentView', function () {
  ...
  describe('review comment data for non-edit mode', function () {
    ...
    it('renders review comment data for non-edit mode', function () {
      ...
      assert.strictEqual(
        wrapper.find('.github-Review-avatar').prop('src'),
        comment.author.avatarUrl
      );
      assert.strictEqual(
        wrapper.find('.github-Review-username').prop('href'),
        comment.author.url
      );
      assert.strictEqual(
        wrapper.find('.github-Review-username').text(),
      ...
      assert.isTrue(
        showActionsMenu.calledWith(sinon.match.any, comment, comment.author)
      );
      ...
      assert.strictEqual(
        wrapper.find(EmojiReactionsController).prop('reactable'),
        comment
      );
    });
    ...
    it('uses a ghost user for comments with no author', function () {
      ...
      wrapper.find('Octicon[icon="ellipses"]').simulate('click');
      assert.isTrue(
        showActionsMenu.calledWith(sinon.match.any, comment, GHOST_USER)
      );
    });
