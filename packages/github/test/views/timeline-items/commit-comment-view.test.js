...
describe('CommitCommentView', function () {
  ...
  it('renders comment data', function () {
    ...
    assert.isTrue(
      wrapper.find('Octicon[icon="comment"]').hasClass('pre-timeline-item-icon')
    );
  ...
  it('shows ghost user when author is null', function () {
    ...
    assert.match(
      ...
      new RegExp(`^${GHOST_USER.login}`)
    );
    assert.strictEqual(
      wrapper.find('.author-avatar').prop('src'),
      GHOST_USER.avatarUrl
    );
    assert.strictEqual(
      wrapper.find('.author-avatar').prop('alt'),
      GHOST_USER.login
    );
  });
