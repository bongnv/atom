...
describe('ReviewsView', function () {
  ...
  it('renders empty state if there is no review', function () {
    ...
    assert.isTrue(
      reporterProxy.addEvent.calledWith('start-pr-review', {
        package: 'github',
  ...
  it('displays ghost user information in review summary if author is null', function () {
    ...
    const summary = wrapper
      .find('.github-ReviewSummary')
      .find('ActionableReviewView')
      .renderProp('render')(showActionMenu);

    // summary
    assert.strictEqual(
      summary.find('.github-ReviewSummary-username').text(),
      GHOST_USER.login
    );
    assert.strictEqual(
      summary.find('.github-ReviewSummary-username').prop('href'),
      GHOST_USER.url
    );
    assert.strictEqual(
      summary.find('.github-ReviewSummary-avatar').prop('src'),
      GHOST_USER.avatarUrl
    );
    assert.strictEqual(
      summary.find('.github-ReviewSummary-avatar').prop('alt'),
      GHOST_USER.login
    );
  });
  ...
  it('displays an author association badge for review summaries', function () {
    const { summaries, commentThreads } = aggregatedReviewsBuilder()
      ...
      .addReviewSummary((r) =>
        r.id(4).authorAssociation('FIRST_TIME_CONTRIBUTOR')
      )
    ...
    assert.strictEqual(
      reviews[0].find('.github-Review-authorAssociationBadge').text(),
      'Member'
    );
    assert.strictEqual(
      reviews[1].find('.github-Review-authorAssociationBadge').text(),
      'Owner'
    );
    assert.strictEqual(
      reviews[2].find('.github-Review-authorAssociationBadge').text(),
      'Collaborator'
    );
    assert.strictEqual(
      reviews[3].find('.github-Review-authorAssociationBadge').text(),
      'Contributor'
    );
    assert.strictEqual(
      reviews[4].find('.github-Review-authorAssociationBadge').text(),
      'First-time contributor'
    );
    assert.strictEqual(
      reviews[5].find('.github-Review-authorAssociationBadge').text(),
      'First-timer'
    );
  ...
  it('calls openIssueish when clicking on an issueish link in a review summary', function () {
    ...
    const { summaries } = aggregatedReviewsBuilder()
      .addReviewSummary((r) => {
        r.bodyHTML(
          'hey look a link <a href="https://github.com/aaa/bbb/pulls/123">#123</a>'
        ).id(0);
      })
      .build();
    ...
    const showActionMenu = () => {};
    const review = wrapper.find('ActionableReviewView').renderProp('render')(
      showActionMenu
    );

    review.find('GithubDotcomMarkdown').prop('switchToIssueish')(
      'aaa',
      'bbb',
      123
    );
  ...
  describe('checkout button', function () {
    it('passes checkoutOp prop through to CheckoutButon', function () {
      ...
      assert.deepEqual(checkoutOpProp.disablement, {
        reason: { name: 'current' },
        message: 'disabled',
      });
    });
  ...
  describe('comment threads', function () {
      ...
      .addReviewThread((t) => {
        ...
        t.addComment((c) =>
          c
            ...
            .path('dir/file0')
            .position(10)
            .bodyHTML('i have a bad windows file path.')
            .author((a) => a.login('user0').avatarUrl('user0.jpg'))
        );
        t.addComment((c) =>
          c
            .id(1)
            ...
            .position(10)
            .bodyHTML('i disagree.')
            .author((a) => a.login('user1').avatarUrl('user1.jpg'))
            .isMinimized(true)
        );
      })
      .addReviewThread((t) => {
        t.addComment((c) =>
          c
            ...
            .path('file1')
            .position(20)
            .bodyHTML('thanks for all the fish')
            .author((a) => a.login('dolphin').avatarUrl('pic-of-dolphin'))
        );
        t.addComment((c) =>
    ...
    beforeEach(function () {
      ...
      commentThreads.forEach((thread) => {
        ...
        commentTranslations.set(path.normalize(rootComment.path), {
          diffToFilePosition,
        });
      });
    ...
    it('groups comments by their resolved status', function () {
      const unresolvedThreads = wrapper
        .find('.github-Reviews-section.comments')
        .find('.github-Review');
      const resolvedThreads = wrapper
        .find('.github-Reviews-section.resolved-comments')
        .find('.github-Review');
    ...
    describe('indication that a comment has been edited', function () {
      it('indicates that a review summary comment has been edited', function () {
        ...
        const commentUrl =
          'https://github.com/atom/github/pull/1995#discussion_r272475592';
        const updatedProps = aggregatedReviewsBuilder()
          .addReviewSummary((r) =>
            r.id(0).lastEditedAt('2018-12-27T17:51:17Z').url(commentUrl)
          )
          .build();
    ...
    describe('each thread', function () {
      it('displays correct data', function () {
        ...
        assert.strictEqual(
          thread.find('.github-Review-file').text(),
          `${path.sep}file0`
        );
      ...
      it('renders a PatchPreviewView per comment thread', function () {
        assert.isTrue(
          wrapper
            .find('details.github-Review')
            .everyWhere(
              (thread) => thread.find('PatchPreviewView').length === 1
            )
        );
      ...
      describe('navigation buttons', function () {
        it('a pair of "Open Diff" and "Jump To File" buttons per thread', function () {
          assert.isTrue(
            wrapper
              ...
              .everyWhere(
                (thread) =>
                  thread.find('.github-Review-navButton.icon-code').length ===
                    1 &&
        ...
        describe('when PR is checked out', function () {
          ...
          beforeEach(function () {
            ...
            openDiff = sinon.spy();
            wrapper = shallow(
              buildApp({ openFile, openDiff, summaries, commentThreads })
            );
          });
        ...
        describe('when PR is not checked out', function () {
          ...
          it('"Jump To File" button is disabled', function () {
            assert.isTrue(
              wrapper
                .find('button.icon-code')
                .everyWhere((button) => button.prop('disabled') === true)
            );
          });
    ...
    it('registers a github:submit-comment command that submits the focused reply comment', async function () {
      ...
      assert.isTrue(
        addSingleComment.calledWith('content', 'abcd', 1, 'file0', 10, {
          didSubmitComment: sinon.match.func,
          didFailComment: sinon.match.func,
        })
      );
    });
    ...
    it('renders progress bar', function () {
      ...
      assert.strictEqual(
        wrapper.find('.github-Reviews-count').text(),
        'Resolved 1 of 3'
      );
      assert.include(
        wrapper.find('progress.github-Reviews-progessBar').props(),
        { value: 1, max: 3 }
      );
    });
  });
