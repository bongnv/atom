...
describe('CheckSuiteView', function () {
  ...
  it('renders the summarized suite information', function () {
    ...
    assert.strictEqual(
      wrapper.find('.github-PrStatuses-list-item-context strong').text(),
      'app'
    );
  });

  it('omits app information when the app is no longer present', function () {
    const checkSuite = checkSuiteBuilder(checkSuiteQuery).nullApp().build();
  ...
  it('renders a CheckRun for each run within the suite', function () {
    ...
    assert.deepEqual(
      wrapper.find(CheckRunView).map((v) => v.prop('checkRun')),
      checkRuns
    );
  });
});
