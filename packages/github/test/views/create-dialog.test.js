...
describe('CreateDialog', function () {
  ...
  it('passes everything to the CreateDialogContainer', function () {
    ...
    assert.strictEqual(
      container.prop('currentWindow'),
      atomEnv.getCurrentWindow()
    );
  ...
  describe('publishRepository', function () {
    ...
    it('successfully publishes an existing local repository', async function () {
      ...
      assert.isTrue(
        repository.addRemote.calledWith(
          'origin',
          'https://github.com/user0/repo-name'
        )
      );
      assert.isTrue(
        repository.push.calledWith('master', {
          remote: CREATED_REMOTE,
          setUpstream: true,
        })
      );
    });
    ...
    it('uses HEAD as the default branch if master is not present', async function () {
      ...
      assert.isTrue(
        repository.push.calledWith('other-branch', {
          remote: CREATED_REMOTE,
          setUpstream: true,
        })
      );
