...
export default class GitHubTabContainer extends React.Component {
  ...
  static getDerivedStateFromProps(props, state) {
    if (props.repository !== state.lastRepository) {
      ...
      const observerSub = remoteOperationObserver.onDidComplete(() =>
        state.refresher.trigger()
      );
  ...
  fetchToken = (loginModel, endpoint) =>
    loginModel.getToken(endpoint.getLoginAccount());
  ...
  renderRepositoryData = (repoData) => {
    ...
    if (repoData) {
      repoData.githubRemotes = repoData.allRemotes.filter((remote) =>
        remote.isGithubRepo()
      );
      repoData.currentBranch = repoData.branches.getHeadBranch();

      repoData.currentRemote = repoData.githubRemotes.withName(
        repoData.selectedRemoteName
      );
      repoData.manyRemotesAvailable = false;
      if (
        !repoData.currentRemote.isPresent() &&
        repoData.githubRemotes.size() === 1
      ) {
      ...
      repoData.endpoint = endpoint =
        repoData.currentRemote.getEndpointOrDotcom();
    }
