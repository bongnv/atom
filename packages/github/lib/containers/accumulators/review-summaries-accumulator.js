...
export class BareReviewSummariesAccumulator extends React.Component {
  ...
  render() {
    const resultBatch = this.props.pullRequest.reviews.edges.map(
      (edge) => edge.node
    );
    ...
    return (
      ...
      >
        {(error, results, loading) => {
          const summaries = results.sort(
            (a, b) =>
              moment(a.submittedAt, moment.ISO_8601) -
              moment(b.submittedAt, moment.ISO_8601)
          );
  ...
  },
  {
    ...
    query: graphql`
      query reviewSummariesAccumulatorQuery(
        ...
        resource(url: $url) {
          ... on PullRequest {
            ...reviewSummariesAccumulator_pullRequest
              @arguments(reviewCount: $reviewCount, reviewCursor: $reviewCursor)
          }
        }
      }
    `,
