...
export class BareCheckRunsAccumulator extends React.Component {
  static propTypes = {
    ...
    checkSuite: PropTypes.shape({
      checkRuns: RelayConnectionPropType(PropTypes.object),
    }),
  ...
  render() {
    const resultBatch = this.props.checkSuite.checkRuns.edges.map(
      (edge) => edge.node
    );
    ...
    return (
      ...
      >
        {(error, checkRuns, loading) =>
          this.props.children({ error, checkRuns, loading })
        }
      </Accumulator>
    );
  }
