...
export default class MultiFilePatchView extends React.Component {
        ...
        this.refEditor.map((editor) => {
          ...
          if (lastSelectionIndex !== null) {
            const nextSelectionRange =
              nextPatch.getSelectionRangeForIndex(lastSelectionIndex);
  ...
  componentDidMount() {
    ...
    this.subs.add(
      this.props.config.onDidChange('github.showDiffIconGutter', () =>
        this.forceUpdate()
      )
    );
    ...
    return this.props.reviewCommentThreads.map(({ comments, thread }) => {
      ...
      return (
        <CommentGutterDecorationController
          ...
          extraClasses={
            isRowSelected ? ['github-FilePatchView-line--selected'] : []
          }
  ...
  renderFilePatchDecorations = (filePatch, index) => {
    ...
    return (
      <Fragment key={filePatch.getPath()}>
        <Marker invalidate="never" bufferRange={filePatch.getStartRange()}>
          ...
          >
            <FilePatchHeaderView
              ...
              openFile={() =>
                this.didOpenFile({ selectedFilePatch: filePatch })
              }
              toggleFile={() => this.props.toggleFile(filePatch)}
              isCollapsed={isCollapsed}
              triggerCollapse={() =>
                this.props.multiFilePatch.collapseFilePatch(filePatch)
              }
              triggerExpand={() =>
        ...
        {isUnavailable &&
          this.renderDiffUnavailable(filePatch, position, index)}
      ...
      <Marker invalidate="never" bufferRange={filePatch.getStartRange()}>
        ...
        >
          <p className="github-FilePatchView-message icon icon-warning">
            This diff is too large to load at all. Use the command-line to view
            it.
          </p>
        </Decoration>
      </Marker>
  ...
  didOpenFile({ selectedFilePatch }) {
    ...
    if (selectedFilePatch && !filePatchesWithCursors.has(selectedFilePatch)) {
      ...
      const cursorRow = firstHunk
        ? firstHunk.getNewStartRow() - 1
  ...
  didChangeSelectedRows() {
    ...
    const nextCursorRows = this.refEditor
      ...
      .getOr([]);
    const hasMultipleFileSelections =
      this.props.multiFilePatch.spansMultipleFiles(nextCursorRows);
  ...
  measurePerformance(action) {
    ...
    if (
      ...
      performance.getEntriesByName(`MultiFilePatchView-${action}-start`)
        .length > 0
    ) {
