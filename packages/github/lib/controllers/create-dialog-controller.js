...
export class BareCreateDialogController extends React.Component {
  ...
  constructor(props) {
    ...
    this.subs = new CompositeDisposable(
      ...
      this.props.config.onDidChange(
        'github.sourceRemoteName',
        this.readSourceRemoteNameSetting
      ),
      this.props.config.onDidChange(
        'github.remoteFetchProtocol',
  ...
  didChangeRepoName = () => {
    ...
    if (!this.modified.localPath) {
      if (this.localPath.getText() === this.projectHome) {
        this.localPath.setText(
          path.join(this.projectHome, this.repoName.getText())
        );
      } else {
  ...
  didChangeOwnerID = (ownerID) =>
    new Promise((resolve) =>
      this.setState({ selectedOwnerID: ownerID }, resolve)
    );
  ...
  didChangeVisibility = (visibility) => {
    return new Promise((resolve) =>
      this.setState({ selectedVisibility: visibility }, resolve)
    );
  };
  ...
  didChangeProtocol = async (protocol) => {
    await new Promise((resolve) =>
      this.setState({ selectedProtocol: protocol }, resolve)
    );
  ...
  writeSourceRemoteNameSetting() {
    if (
      this.props.config.get('github.sourceRemoteName') !==
      this.sourceRemoteName.getText()
    ) {
