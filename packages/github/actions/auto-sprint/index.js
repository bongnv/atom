...
Toolkit.run(
  ...
  `);
    if (
      !teamResponse.organization.team.members.nodes.some(
        (node) => node.login === actorLogin
      )
    ) {
      tools.exit.neutral(
        'User %s is not in the github-package team. Thanks for your contribution!',
        actorLogin
      );
    }
  ...
  `);
    ...
    if (!project) {
      tools.exit.failure(
        'No open project found with a name matching "Release".'
      );
    }
    const column = project.columns.nodes.find(
      (node) => node.name === 'In progress'
    );
    if (!column) {
      tools.exit.failure(
        'No column found in the project %s with a name of exactly "In progress".',
        project.name
      );
    }
