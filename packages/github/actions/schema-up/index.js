...
Toolkit.run(
  async (tools) => {
    ...
    const { code: hasSchemaChanges } = await tools.runInWorkspace(
      'git',
      ['diff', '--quiet', '--', 'graphql/schema.graphql'],
      { reject: false }
    );
  ...
  `,
    ...
    if (openPullRequestsQuery.repository.pullRequests.totalCount > 0) {
      tools.exit.success(
        'One or more schema update pull requests are already open. Please resolve those first.'
      );
    }
    ...
    await tools.runInWorkspace('git', [
      ...
      '--all',
      '--message',
      ':arrow_up: GraphQL schema',
    ]);

    tools.log.info('Re-running the Relay compiler.');
    const { failed: relayFailed, stdout: relayOutput } =
...
The GraphQL schema has been updated and \`relay-compiler\` has been re-run on the package source. `;
    ...
    if (!relayFailed) {
      if (hasRelayChanges !== 0) {
        body +=
          'The modified files have been committed to this branch and pushed. ';
        body += 'If all of the tests pass in CI, merge with confidence :zap:';
      } else {
        body +=
          'The new schema has been committed to this branch and pushed. None of the ';
        body +=
          'generated Relay source has changed as a result, so this should be a trivial merge :shipit: :rocket:';
      }
    } else {
  ...
  `,
    ...
    const createdPullRequest =
      createPullRequestMutation.createPullRequest.pullRequest;
    tools.log.info(
