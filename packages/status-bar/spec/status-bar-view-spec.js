...
describe('StatusBarView', function () {
  ...
  describe('::addLeftTile({item, priority})', function () {
    it('appends the view for the given item to its left side', function () {
      ...
      const tile1 = statusBarView.addLeftTile({
        item: testItem1,
        priority: 10,
      });
      const tile2 = statusBarView.addLeftTile({
        item: testItem2,
        priority: 30,
      });
      const tile3 = statusBarView.addLeftTile({
        item: testItem3,
        priority: 20,
      });
    ...
    return describe('when no priority is given', () =>
  ...
  return describe('::addRightTile({item, priority})', function () {
    it('appends the view for the given item to its right side', function () {
      ...
      const tile1 = statusBarView.addRightTile({
        item: testItem1,
        priority: 10,
      });
      const tile2 = statusBarView.addRightTile({
        item: testItem2,
        priority: 30,
      });
      const tile3 = statusBarView.addRightTile({
        item: testItem3,
        priority: 20,
      });
    ...
    it('allows the view to be removed', function () {
      const testItem = new TestItem(1);
      const disposable = statusBarView.addRightTile({
        item: testItem,
        priority: 10,
      });
      disposable.destroy();
