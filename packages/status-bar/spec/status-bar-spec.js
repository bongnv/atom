...
describe('Status Bar package', function () {
  ...
  describe('@activate()', () =>
    it('appends only one status bar', function () {
      ...
      return expect(
        workspaceElement.querySelectorAll('status-bar').length
      ).toBe(1);
    }));
  ...
  describe('isVisible option', function () {
    ...
    describe('when it is true', function () {
      ...
      return it('shows status bar', () =>
        expect(
          workspaceElement.querySelector('status-bar').parentNode
        ).toBeVisible());
    });
  ...
  describe('full-width setting', function () {
    ...
    it('expects the setting to be enabled by default', function () {
      ...
      return expect(containers.footer.panels).toContain(
        mainModule.statusBarPanel
      );
    });
    ...
    return describe('when setting is changed', function () {
      it("fits status bar to editor's width", function () {
        ...
        return expect(containers.footer.panels).not.toContain(
          mainModule.statusBarPanel
        );
      });
      ...
      return it('restores the status-bar location when re-enabling setting', function () {
        ...
        return expect(containers.bottom.panels).not.toContain(
          mainModule.statusBarPanel
        );
      });
    });
  ...
  return describe("the 'status-bar' service", function () {
    ...
    return it('allows the git info tile to be disabled', function () {
      const getGitInfoTile = () =>
        statusBar
          .getRightTiles()
          .find((tile) => tile.item.matches('.git-view'));
