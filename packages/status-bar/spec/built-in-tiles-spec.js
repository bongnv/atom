...
describe('Built-in Status Bar Tiles', function () {
  ...
  describe('the file info, cursor and selection tiles', function () {
    ...
    beforeEach(function () {
      ...
      return runs(function () {
        let launchMode;
        [launchMode, fileInfo, cursorPosition, selectionCount] = Array.from(
    ...
    describe('when associated with remote file path', function () {
      ...
      it('updates the path in the status bar', function () {
        ...
        expect(fileInfo.currentPath.textContent).toBe(
          'remote://server:123/folder/remote_file.txt'
        );
      ...
      return it('calls relativize with the remote URL on shift-click', function () {
        const spy = spyOn(atom.project, 'relativize').andReturn(
          'remote_file.txt'
        );
      ...
      it('copies the absolute path into the clipboard if available', function () {
        ...
        return runs(function () {
          ...
          return expect(atom.clipboard.read()).toBe(
            fileInfo.getActiveItem().getPath()
          );
        });
      }));
    ...
    describe("when saved buffer's path is clicked", function () {
      it("displays a tooltip containing text 'Copied:' and an absolute native path", function () {
        ...
        return runs(function () {
          fileInfo.currentPath.click();
          return expect(document.querySelector('.tooltip')).toHaveText(
    ...
    describe('when the buffer content has changed from the content on disk', function () {
      it('disables the buffer modified indicator on save', function () {
        ...
        runs(function () {
          ...
          editor.insertText('\n');
          advanceClock(buffer.stoppedChangingDelay);
          return expect(fileInfo.classList.contains('buffer-modified')).toBe(
            true
          );
        });
    ...
    describe("when the associated editor's cursor position changes", function () {
      ...
      return it('does not throw an exception if the cursor is moved as the result of the active pane item changing to a non-editor (regression)', function () {
        waitsForPromise(() =>
          Promise.resolve(atom.packages.deactivatePackage('status-bar'))
        ...
        return runs(function () {
          ...
          atom.workspace
            .getActivePane()
            .activateItem(document.createElement('div'));
    ...
    return describe('the selection count tile', function () {
      ...
      it('updates when the configuration changes', function () {
        ...
        atom.config.set(
          'status-bar.selectionCountFormat',
          'Selection: baz %C quux %L'
        );
        atom.views.performDocumentUpdate();
        return expect(selectionCount.textContent).toBe(
          'Selection: baz 60 quux 2'
        );
      });
  ...
  return describe('the git tile', function () {
    ...
    const hover = function (element, fn) {
      ...
      const hoverDelay =
        (atom.tooltips.defaults.delay != null
    ...
    beforeEach(function () {
      ...
      return (
        ([gitView] = Array.from(
          (ref = statusBar.getRightTiles().map((tile) => tile.getItem()))
        )),
        ref
      );
    });
    ...
    describe('the git ahead/behind count labels', function () {
      ...
      it('shows the number of commits that can be pushed/pulled', function () {
        ...
        waitsForPromise(() =>
          atom.workspace.open(filePath).then(() => repo.refreshStatus())
        );

        return runs(function () {
          const behindElement = document.body.querySelector(
            '.commits-behind-label'
          );
          const aheadElement = document.body.querySelector(
            '.commits-ahead-label'
          );
    ...
    describe('the git branch label', function () {
      ...
      it('displays the current branch for files in repositories', function () {
        ...
        runs(function () {
          const currentBranch = atom.project
            .getRepositories()[0]
            .getShortHead();
      ...
      return it("doesn't display the current branch for a file outside the current project", function () {
        waitsForPromise(() =>
          atom.workspace.open(
            path.join(os.tmpdir(), 'atom-specs', 'not-in-project.txt')
          )
        );
    ...
    return describe('the git status label', function () {
      ...
      it('displays the new icon for a new file', function () {
        waitsForPromise(() =>
          atom.workspace.open(newPath).then(() => repo.refreshStatus())
        );
