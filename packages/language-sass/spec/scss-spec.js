...
describe('SCSS grammar', function () {
  ...
  describe('numbers', function () {
    it('tokenizes them correctly', function () {
      ...
      ({ tokens } = grammar.tokenizeLine(
        '.something { color: rgba(0, 128, 0, 1) }'
      ));
      expect(tokens[10]).toEqual({
        value: '0',
        scopes: [
      ...
      for (var scope in selectors) {
        ...
        it(`tokenizes complex ${scope} selectors`, function () {
          let { tokens } = grammar.tokenizeLine(
            `${selector}legit-#\{$selector}-name\\@sm`
          );
          ...
          ({ tokens } = grammar.tokenizeLine(
            `${selector}legit-#\{component.$selector}-name\\@sm`
          ));
        ...
        it(`tokenizes invalid identifiers in ${scope} selectors`, function () {
          let { tokens } = grammar.tokenizeLine(
            `${selector}legit-#\{$selector}-n}a$me\\@sm`
          );
  ...
  describe('@at-root', () =>
    it('tokenizes it correctly', function () {
      const { tokens } = grammar.tokenizeLine(
        '@at-root (without: media) .btn { color: red; }'
      );
  ...
  describe('@include', function () {
    it('tokenizes it correctly', function () {
      ...
      ({ tokens } = grammar.tokenizeLine(
        '@include media(site.$width: 100px){}'
      ));
    ...
    return it('tokenizes correctly for includes from a module', function () {
      ...
      ({ tokens } = grammar.tokenizeLine(
        '@include media.breakpoint($width: 100px){}'
      ));
...
`);
    ...
    return describe('property values', () =>
...
`);
  ...
  describe('pseudo classes', function () {
    ...
    it('tokenizes nth-* pseudo classes', function () {
      ...
      expect(tokens[4]).toEqual({
        value: 'n',
        scopes: ['source.css.scss', 'constant.other.scss'],
      });
      expect(tokens[5]).toEqual({
        value: ')',
        scopes: [
...
`);
  ...
  describe('variable setting', function () {
    ...
    it('tokenizes maps', function () {
      const { tokens } = grammar.tokenizeLine(
        '$map: (medium: value, header-height: 10px);'
      );
    ...
    it('tokenizes variables in maps', function () {
      const { tokens } = grammar.tokenizeLine(
        '$map: (gutters: $grid-content-gutters)'
      );
...
`);
  ...
  describe('interpolation', function () {
    it('is tokenized within single quotes', function () {
      ...
      ); // escaping CoffeeScript's interpolation
    ...
    it('is tokenized as a keyframe', function () {
      ...
      ({ tokens } = grammar.tokenizeLine(
        '@keyframes anim { #{animations.$keyframe} {} }'
      ));
    ...
    return it('does not tokenize anything after the closing bracket as interpolation', function () {
      ...
      return expect(tokens[3]).not.toEqual({
        value: 'hi',
...
`);
      ...
      return expect(tokens[1][8]).toEqual({
        value: '//url/goes/here',
        scopes: [
          'source.css.scss',
          'meta.property-list.scss',
...
`);
  ...
  return describe('@forward', function () {
    ...
    it('tokenizes @forward with hide correctly', function () {
      const { tokens } = grammar.tokenizeLine(
        "@forward 'module' hide a-mixin $private-variable"
      );
