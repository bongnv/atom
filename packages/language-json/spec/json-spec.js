...
describe('JSON grammar', function () {
  ...
  it('tokenizes arrays', function () {
    ...
    const separatorScopes = [
      ...Array.from(baseScopes),
  ...
  it('identifies trailing commas in arrays', function () {
    ...
    expect(tokens[2]).toEqual({
      ...
      scopes: [
        ...Array.from(baseScopes),
        'invalid.illegal.trailing-array-separator.json',
      ],
    });
    expect(tokens[3]).toEqual({ value: ' ', scopes: baseScopes });
  ...
  it('tokenizes objects', function () {
    ...
    const keyBeginScopes = [
      ...Array.from(keyScopes),
    ...
    const keyValueSeparatorScopes = [
      ...Array.from(valueScopes),
      'punctuation.separator.dictionary.key-value.json',
    ];
    const pairSeparatorScopes = [
      ...Array.from(valueScopes),
      'punctuation.separator.dictionary.pair.json',
    ];
    const stringValueScopes = [
      ...Array.from(valueScopes),
      'string.quoted.double.json',
    ];
    ...
    expect(tokens[6]).toEqual({
      value: '1',
      scopes: [...Array.from(valueScopes), 'constant.numeric.json'],
    });
    ...
    expect(tokens[14]).toEqual({
      value: 'true',
      scopes: [...Array.from(valueScopes), 'constant.language.json'],
    });
  ...
  return it('identifies trailing commas in objects', function () {
    ...
    expect(tokens[15]).toEqual({
      ...
      scopes: [
        ...Array.from(valueScopes),
        'invalid.illegal.trailing-dictionary-separator.json',
      ],
    });
    expect(tokens[16]).toEqual({ value: ' ', scopes: baseScopes });
