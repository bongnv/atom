...
describe('Less grammar', function () {
  ...
  it('parses @media orientation', function () {
    ...
    return expect(tokens[7]).toEqual({
      ...
      scopes: [
        'source.css.less',
        'support.constant.property-value.media-property.media.css',
      ],
    });
  });
...
`);
    expect(lines[0][0]).toEqual({
      value: 'a',
      scopes: ['source.css.less', 'entity.name.tag.css'],
    });
  ...
  it('parses font lists', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { font-family: "Some Font Name", serif; }'
    );
    expect(tokens[5]).toEqual({
      value: 'font-family',
      scopes: [
...
`);
  ...
  it('parses options in import statements', function () {
    ...
    expect(tokens[4]).toEqual({
      ...
      scopes: [
        'source.css.less',
        'meta.at-rule.import.css',
        'keyword.control.import.option.less',
      ],
    });
    expect(tokens[5]).toEqual({
      value: ',',
  ...
  it('parses built-in functions in property values', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { border: 1px solid rgba(0,0,0); }'
    );
    expect(tokens[0]).toEqual({
      value: '.',
      scopes: [
  ...
  it('parses linear-gradient', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { background: linear-gradient(white, black); }'
    );
    expect(tokens[5]).toEqual({
      value: 'background',
      scopes: [
  ...
  it('parses blend modes', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { background-blend-mode: color-dodge; }'
    );
    expect(tokens[5]).toEqual({
  ...
  it('parses non-quoted urls', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { background: url(http://%20/2.png) }'
    );
    expect(tokens[8]).toEqual({
      value: 'url',
      scopes: [
  ...
  it('parses non-quoted relative urls', function () {
    const { tokens } = grammar.tokenizeLine(
      '.foo { background: url(../path/to/image.png) }'
    );
    expect(tokens[8]).toEqual({
      value: 'url',
      scopes: [
