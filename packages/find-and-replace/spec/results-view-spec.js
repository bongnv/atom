...
describe('ResultsView', () => {
  ...
  beforeEach(async () => {
    ...
    const activationPromise = atom.packages
      .activatePackage('find-and-replace')
  ...
  describe('when the result is for a long line', () => {
    it('renders the context around the match', async () => {
      ...
      expect(
        resultsView.refs.listView.element.querySelector('.path-name')
          .textContent
      ).toBe('one-long-line.coffee');
      expect(
        resultsView.refs.listView.element.querySelectorAll('.preview').length
      ).toBe(1);
      expect(
        resultsView.refs.listView.element.querySelector('.preview').textContent
      ).toBe(
        'test test test test test test test test test test test a b c d e f g h i j k l abcdefghijklmnopqrstuvwxyz'
      );
      expect(
        resultsView.refs.listView.element.querySelector('.match').textContent
      ).toBe('ghijkl');
    });
  });
  ...
  describe('when there are multiple project paths', () => {
    ...
    it('includes the basename of the project path that contains the match', async () => {
      ...
      expect(
        resultsView.refs.listView.element.querySelector('.path-name')
          .textContent
      ).toBe(path.join('project', 'one-long-line.coffee'));
    });
  });
  ...
  describe('rendering replacement text', () => {
    ...
    beforeEach(() => {
      projectFindView.findEditor.setText('ghijkl');
      modifiedDelay =
        projectFindView.replaceEditor.getBuffer().stoppedChangingDelay;
    });
    ...
    it('renders the replacement when doing a search and there is a replacement pattern', async () => {
      ...
      expect(
        resultsView.refs.listView.element.querySelector('.replacement')
          .textContent
      ).toBe('cats');
    });
    ...
    it('renders the replacement when changing the text in the replacement field', async () => {
      ...
      expect(
        resultsView.refs.listView.element.querySelector('.match')
      ).toHaveClass('highlight-info');
      expect(
        resultsView.refs.listView.element.querySelector('.replacement')
          .textContent
      ).toBe('');
    ...
    it('renders the captured text when the replace pattern uses captures', async () => {
      ...
      expect(listElement.querySelectorAll('.match')[0].textContent).toBe(
        'function ()'
      );
      expect(listElement.querySelectorAll('.replacement')[0].textContent).toBe(
        '() =>'
      );
      expect(listElement.querySelectorAll('.match')[1].textContent).toBe(
        'function(items)'
      );
      expect(listElement.querySelectorAll('.replacement')[1].textContent).toBe(
        '(items) =>'
      );
    });
  });
  ...
  describe('core:page-up and core:page-down', () => {
    beforeEach(async () => {
      ...
      expect(listView.element.scrollHeight).toBeGreaterThan(
        listView.element.offsetHeight
      );
    });
    ...
    it('selects the first result on the next page when core:page-down is triggered', async () => {
      const { listView } = resultsView.refs;
      expect(
        listView.element.querySelectorAll('.path-row').length
      ).not.toBeGreaterThan(resultsView.model.getPathCount());
      expect(
        listView.element.querySelectorAll('.match-row').length
      ).not.toBeGreaterThan(resultsView.model.getMatchCount());
      expect(
        listView.element.querySelector('.path-row').parentElement
      ...
      await resultsView.pageDown();
      expect(
        _.last(resultsView.element.querySelectorAll('.match-row'))
      ).toHaveClass('selected');
      expect(getSelectedPosition()).toBeGreaterThan(initiallySelectedPosition);
    });
  ...
  describe('core:move-to-top and core:move-to-bottom', () => {
    ...
    it('selects the first/last item when core:move-to-top/move-to-bottom is triggered', async () => {
      ...
      await resultsView.moveToBottom();
      expect(
        _.last(listView.element.querySelectorAll('.match-row'))
      ).toHaveClass('selected');
    ...
    it('selects the path when when core:move-to-bottom is triggered and last item is collapsed', async () => {
      ...
      expect(
        _.last(resultsView.refs.listView.element.querySelectorAll('.path-row'))
          .parentElement
      ).toHaveClass('selected');
    });
    ...
    it('selects the path when when core:move-to-top is triggered and first item is collapsed', async () => {
      ...
      expect(
        resultsView.refs.listView.element.querySelector('.path-row')
          .parentElement
      ).toHaveClass('selected');
    });
  });
  ...
  describe('expanding and collapsing results', () => {
    it('preserves the selected file when collapsing all results', async () => {
      ...
      expect(selectedPath.firstChild.dataset.filePath).toContain(
        'sample.coffee'
      );
      ...
      expect(newSelectedPath.firstChild.dataset.filePath).toContain(
        'sample.js'
      );
      ...
      resultsView.moveDown();
      await resultsView.moveDown();
      expect(resultsView.element.querySelector('.selected')).toHaveClass(
        'match-row'
      );
    ...
    it('preserves the collapsed state of the right files when results are removed', async () => {
      ...
      const matchedPaths = resultsView.refs.listView.element.querySelectorAll(
        '.path.list-nested-item'
      );
    ...
    it('does not contract result when right clicked', async () => {
      ...
      const pathNode =
        resultsView.refs.listView.element.querySelectorAll('.path-row')[0];
      pathNode.dispatchEvent(
        buildMouseEvent('mousedown', { target: pathNode, which: 3 })
      );
      expect(resultsView.element.querySelector('.collapsed')).toBe(null);
    });
  ...
  describe('opening results', () => {
    ...
    function paneItemOpening(pending = null) {
      return new Promise((resolve) => {
        const subscription = atom.workspace.onDidOpen(({ pane, item }) => {
          if (
            pending === null ||
            (pane.getPendingItem() === item) === pending
          ) {
            resolve();
    ...
    it('opens the file containing the result in a non-pending state when the search result is double-clicked', async () => {
      const pathNode =
        resultsView.refs.listView.element.querySelectorAll('.match-row')[0];
      const click1 = buildMouseEvent('mousedown', {
        target: pathNode,
        detail: 1,
      });
      const click2 = buildMouseEvent('mousedown', {
        target: pathNode,
        detail: 2,
      });
      ...
      expect(atom.workspace.getCenter().getActivePane().getPendingItem()).toBe(
        null
      );
    ...
    it('opens the file containing the result in a pending state when the search result is single-clicked', async () => {
      const pathNode =
        resultsView.refs.listView.element.querySelectorAll('.match-row')[0];
      pathNode.dispatchEvent(
        buildMouseEvent('mousedown', { target: pathNode, which: 1 })
      );
      await paneItemOpening();
      const editor = atom.workspace.getCenter().getActiveTextEditor();
      expect(atom.workspace.getCenter().getActivePane().getPendingItem()).toBe(
    ...
    it("opens the file in a new non-active tab on 'find-and-replace:open-in-new-tab'", async () => {
      ...
      atom.commands.dispatch(
        resultsView.element,
        'find-and-replace:open-in-new-tab'
      );
      await paneItemOpening();
    ...
    it('Result view should maintain scroll position', async () => {
      ...
      expect(resultsView.setScrollTop).toHaveBeenCalledWith(
        resultsView.currentScrollTop
      );
    });
    ...
    describe('the `projectSearchResultsPaneSplitDirection` option', () => {
      ...
      it("does not create a split when the option is 'none'", async () => {
        ...
        await paneItemOpening();
        expect(
          atom.workspace.open.mostRecentCall.args[1].split
        ).toBeUndefined();
      });
    ...
    describe('when there are a list of items', () => {
      ...
      describe('when there are collapsed results', () => {
        it('moves to the correct prev/next result when a path is selected', async () => {
          ...
          await resultsView.moveUp();
          expect(
            resultsView.refs.listView.element.querySelectorAll('.path-row')[1]
              .parentElement
          ).toHaveClass('selected');
  ...
  describe('icon services', () => {
    describe('atom.file-icons', () => {
      ...
      it('displays icons for common filetypes', () => {
        expect(DefaultFileIcons.iconClassForPath('README.md')).toBe(
          'icon-book'
        );
        expect(DefaultFileIcons.iconClassForPath('zip.zip')).toBe(
          'icon-file-zip'
        );
        expect(DefaultFileIcons.iconClassForPath('a.gif')).toBe(
          'icon-file-media'
        );
        expect(DefaultFileIcons.iconClassForPath('a.pdf')).toBe(
          'icon-file-pdf'
        );
        expect(DefaultFileIcons.iconClassForPath('an.exe')).toBe(
          'icon-file-binary'
        );
        expect(DefaultFileIcons.iconClassForPath('jg.js')).toBe(
          'icon-file-text'
        );
      });
      ...
      it('allows a service provider to change the handler', async () => {
        ...
        expect(fileIconClasses).toContain(
          'first-icon-class second-icon-class icon'
        );
        expect(fileIconClasses).toContain(
          'third-icon-class fourth-icon-class icon'
        );
        ...
        expect(fileIconClasses).not.toContain(
          'first-icon-class second-icon-class icon'
        );
        expect(fileIconClasses).not.toContain(
          'third-icon-class fourth-icon-class icon'
    ...
    describe('file-icons.element-icons', () => {
      ...
      it('uses the element-icon service if available', () => {
        const iconSelector =
          '.path-row .icon:not([data-name="fake-file-path"])';
        ...
        runs(() => {
          resultsView = getResultsView();
          const iconElements =
            resultsView.element.querySelectorAll(iconSelector);
          expect(iconElements[0].className.trim()).toBe('icon foo bar');
          expect(iconElements[1].className.trim()).toBe('icon baz qlux');
          expect(resultsView.element.querySelector('.icon-file-text')).toBe(
            null
          );
  ...
  describe('updating the search while viewing results', () => {
    it('resets the results message', async () => {
      ...
      atom.commands.dispatch(projectFindView.element, 'core:confirm');
      await etch.update(resultsPane);
      expect(resultsPane.refs.previewCount.textContent).toContain(
  ...
  describe('search result context lines', () => {
    ...
    function getFirstMatchRows(resultsView) {
      ...
      const rowCount = resultsView.resultRowGroups[0].rows.filter(
        (row) =>
          row.data.matchLineNumber ===
          resultsView.resultRows[1].data.matchLineNumber
      ).length;
      return rowNodes.slice(1, 1 + rowCount);
    }

    it('shows no context lines', async () => {
      expect(resultsView.model.getFindOptions().leadingContextLineCount).toBe(
        0
      );
      expect(resultsView.model.getFindOptions().trailingContextLineCount).toBe(
        0
      );
