...
describe('BufferSearch', () => {
  ...
  function getHighlightedRanges() {
    ...
    for (const id in decorations) {
      ...
      if (
        ['find-result', 'current-result'].includes(decoration.properties.class)
      ) {
  ...
  function expectUpdateEvent() {
    ...
    const emittedMarkerRanges = markersListener.mostRecentCall.args[0].map(
      (marker) => marker.getBufferRange().serialize()
    );
  ...
  function scannedRanges() {
    return searchSpy.argsForCall.map((args) =>
      args.find((arg) => arg instanceof Range)
    );
  }
  ...
  describe('replacing a search result', () => {
    ...
    it('replaces the marked text with the given string', () => {
      ...
      expect(
        currentResultListener.mostRecentCall.args[0].getBufferRange()
      ).toEqual(rangeToSelect);
      expect(currentResultListener.mostRecentCall.args[0].isDestroyed()).toBe(
        false
      );
    });
  ...
  describe('.prototype.resultsMarkerLayerForTextEditor(editor)', () =>
    it('creates or retrieves the results marker layer for the given editor', () => {
      ...
      for (const marker of layer1.findMarkers()) {
        expect(editor.getTextInBufferRange(marker.getBufferRange())).toMatch(
          /a+/
        );
      }
