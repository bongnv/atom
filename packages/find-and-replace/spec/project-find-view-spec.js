  ...
  describe(`ProjectFindView (ripgrep=${ripgrep})`, () => {
    ...
    function waitForSearchResults() {
      return conditionPromise(() =>
        projectFindView.refs.descriptionLabel.textContent.includes(
          'results found'
        )
      );
    }
    ...
    beforeEach(() => {
      ...
      activationPromise = atom.packages
        ...
        .then(function ({ mainModule }) {
          ...
          const spy = spyOn(projectFindView, 'search').andCallFake(
            (options) => {
              return (searchPromise = spy.originalValue.call(
                projectFindView,
                options
              ));
            }
    ...
    describe('when project-find:show is triggered', () => {
      ...
      it('honors config settings for find options', async () => {
        ...
        expect(projectFindView.refs.wholeWordOptionButton).toHaveClass(
          'selected'
        );
      });
    });
    ...
    describe('when project-find:show-in-current-directory is triggered', () => {
      ...
      beforeEach(() => {
        ...
        <div>
          <span class='name' data-path='${escapePath(
            projectPath
          )}'>${projectPath}</span>
          <ul class='files'>
            <li class='file' data-path='${escapePath(
              path.join(projectPath, 'one.js')
            )}'>
              <span class='name'>one.js</span>
            </li>
            <li class='file' data-path='${escapePath(
              path.join(projectPath, 'two.js')
            )}'>
            ...
            <div class='directory'>
              <div>
                <span class='name' data-path='${escapePath(
                  path.join(projectPath, 'nested')
                )}'>nested</span>
                <ul class='file'>
                  <li class='file' data-path='${escapePath(
                    path.join(projectPath, 'three.js')
                  )}'>
    ...
    describe('finding', () => {
      ...
      describe('when the find string contains an escaped char', () => {
        beforeEach(() => {
          ...
          fs.writeFileSync(
            path.join(projectPath, 'tabs.txt'),
            '\t\n\\\t\n\\\\t'
          );
          atom.project.setPaths([projectPath]);
          atom.commands.dispatch(workspaceElement, 'project-find:show');
        ...
        describe('when regex search is disabled', () => {
          ...
          it('finds a backslash', async () => {
            ...
            expect(
              resultsView.refs.listView.element.querySelectorAll(
                '.match.highlight-info'
              )
            ).toHaveLength(3);
          });
      ...
      describe('splitting into a second pane', () => {
        ...
        it('splits when option is right', async () => {
          ...
          expect(atom.workspace.getCenter().getActivePane()).not.toBe(
            initialPane
          );
        });
        ...
        it('can be duplicated on the right', async () => {
          ...
          const resultsPaneView2 = atom.views.getView(
            pane2.itemForURI(ResultsPaneView.URI)
          );
          ...
          const resultCount =
            resultsPaneView1.querySelectorAll('.match-row').length;
          expect(resultCount).toBeGreaterThan(0);
          expect(resultsPaneView2.querySelectorAll('.match-row')).toHaveLength(
            resultCount
          );
          expect(
            resultsPaneView2.querySelector('.preview-count').innerHTML
          ).toEqual(resultsPaneView1.querySelector('.preview-count').innerHTML);
        });
      ...
      describe('serialization', () => {
        it('serializes if the case, regex and whole word options', async () => {
          atom.commands.dispatch(editorElement, 'project-find:show');
          expect(projectFindView.refs.caseOptionButton).not.toHaveClass(
            'selected'
          );
          projectFindView.refs.caseOptionButton.click();
          expect(projectFindView.refs.caseOptionButton).toHaveClass('selected');

          expect(projectFindView.refs.regexOptionButton).not.toHaveClass(
            'selected'
          );
          projectFindView.refs.regexOptionButton.click();
          expect(projectFindView.refs.regexOptionButton).toHaveClass(
            'selected'
          );

          expect(projectFindView.refs.wholeWordOptionButton).not.toHaveClass(
            'selected'
          );
      ...
      describe('description label', () => {
        ...
        it('shows an error when the pattern is invalid and clears when no error', async () => {
          ...
          expect(projectFindView.refs.descriptionLabel).toHaveClass(
            'text-error'
          );
          expect(projectFindView.refs.descriptionLabel.textContent).toContain(
          ...
          expect(projectFindView.refs.descriptionLabel).not.toHaveClass(
            'text-error'
          );
          expect(projectFindView.refs.descriptionLabel.textContent).toContain(
            'Find in Project'
      ...
      describe('regex', () => {
        ...
        it('escapes regex patterns by default', async () => {
          ...
          expect(atom.workspace.scan.argsForCall[0][0]).toEqual(
            /i\(\\w\)ems\+/gim
          );
        });
        ...
        describe('when search has been run', () => {
          ...
          it('toggles regex option via an event and finds files matching the pattern', async () => {
            ...
            expect(atom.workspace.scan.mostRecentCall.args[0]).toEqual(
              /i(\w)ems+/gim
            );
          });
      ...
      describe('when core:confirm is triggered', () => {
        ...
        describe('when results exist', () => {
          ...
          it('only searches paths matching text in the path filter', async () => {
            ...
            expect(atom.workspace.scan.argsForCall[0][1].paths).toEqual([
              '*.js',
            ]);
          });
          ...
          it('updates the results list when a buffer changes', async () => {
            ...
            expect(
              listView.element.querySelectorAll('.match-row')
            ).toHaveLength(11);
            expect(
              listView.element.querySelectorAll('.match.highlight-info')
            ).toHaveLength(13);
            expect(resultsPaneView.refs.previewCount.textContent).toBe(
              '13 results found in 2 files for items'
            );
            ...
            expect(
              listView.element.querySelectorAll('.path-row')[1].parentElement
            ...
            await waitForSearchResults();
            await conditionPromise(() => editor.getSelectedText() === 'items');
          ...
          it("doesn't update the results list when a buffer outside the project changes", async () => {
            ...
            expect(
              resultsView.refs.listView.element.querySelectorAll('.list-item')
            ).toHaveLength(13);
            expect(resultsPaneView.refs.previewCount.textContent).toBe(
              '13 results found in 2 files for items'
            );
      ...
      describe('history', () => {
        beforeEach(() => {
          ...
          atom.commands.dispatch(
            projectFindView.findEditor.getElement(),
            'core:confirm'
          );
        ...
        it('can navigate the entire history stack', () => {
          ...
          atom.commands.dispatch(
            projectFindView.pathsEditor.element,
            'core:move-up'
          );
      ...
      describe('when there is an error searching', () => {
        it('displays the errors in the results pane', async () => {
          ...
          expect(errorList.querySelectorAll('li')[1].textContent).toBe(
            'Broken'
          );
        });
      });
      ...
      describe('buffer search sharing of the find options', () => {
        function getResultDecorations(clazz) {
          const result = [];
          const decorations = editor.decorationsStateForScreenRowRange(
            0,
            editor.getLineCount()
          );
        ...
        it('setting the find text does not interfere with the project replace state', async () => {
          ...
          advanceClock(
            projectFindView.findEditor.getBuffer().stoppedChangingDelay + 1
          );
          spyOn(atom.workspace, 'scan');
          ...
          await projectFindView.search({
            onlyRunIfActive: false,
        ...
        it('shares the buffers and history cyclers between both buffer and project views', () => {
          ...
          expect(projectFindView.findEditor.getText()).toBe('');
          atom.commands.dispatch(
            projectFindView.findEditor.element,
            'core:move-up'
          );
        ...
        it('highlights the search results in the selected file', async () => {
          ...
          await new Promise((resolve) =>
            editor.onDidChangeSelectionRange(resolve)
          );
          ...
          expect(editor.getSelectedBufferRange()).not.toEqual(
    ...
    describe('replacing', () => {
      ...
      describe('replace all button enablement', () => {
        ...
        it('is disabled when a search returns no results', async () => {
          ...
          expect(projectFindView.refs.replaceAllButton).not.toHaveClass(
            'disabled'
          );
      ...
      describe('when the replace button is pressed', () => {
        ...
        describe('when there are search results after a replace', () => {
          it('runs the search after the replace', async () => {
            ...
            expect(projectFindView.errorMessages).not.toBeVisible();
            expect(
              getExistingResultsPane().refs.previewCount.textContent
            ).toContain('13 results found in 2 files for items');
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
            ...
            expect(
              projectFindView.refs.descriptionLabel.textContent
            ).not.toContain('Replaced items');
            expect(projectFindView.refs.descriptionLabel.textContent).toContain(
              '13 results found in 2 files for items'
            );
          });
      ...
      describe('when the project-find:replace-all is triggered', () => {
        ...
        describe('when a search with no results has been run', () => {
          ...
          it('doesnt replace anything', () => {
            ...
            expect(atom.beep).toHaveBeenCalled();
            expect(
              projectFindView.refs.descriptionLabel.textContent.replace(
                /( {2})/g,
                ' '
              )
        ...
        describe('when a search with results has been run', () => {
          ...
          it('messages the user when the search text has changed since that last search', () => {
            ...
            expect(atom.confirm.mostRecentCall.args[0].message).toContain(
              'was changed to'
            );
          });
    ...
    describe('panel opening', () => {
      describe('when a panel is already open on the right', () => {
        ...
        it("doesn't open another panel even if the active pane is vertically split", async () => {
          ...
          expect(
            workspaceElement.querySelectorAll('.preview-pane').length
          ).toBe(1);
        });
      });
    ...
    describe('when language-javascript is active', () => {
      ...
      it('uses the regexp grammar when regex-mode is loaded from configuration', async () => {
        ...
        expect(projectFindView.findEditor.getGrammar().scopeName).toBe(
          'source.js.regexp'
        );
      ...
      describe('when panel is active', () => {
        ...
        it('does not use regexp grammar when in non-regex mode', () => {
          expect(projectFindView.model.getFindOptions().useRegex).not.toBe(
            true
          );
          expect(projectFindView.findEditor.getGrammar().scopeName).toBe(
