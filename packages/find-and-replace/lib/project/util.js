...
const getReplacementResultsMessage = function ({
  findPattern,
  replacePattern,
  replacedPathCount,
  replacementCount,
  ...
  if (replacedPathCount) {
    return `<span class=\"text-highlight\">Replaced <span class=\"highlight-error\">${sanitizePattern(
      ...
      replacePattern
    )}</span> ${_.pluralize(replacementCount, 'time')} in ${_.pluralize(
      replacedPathCount,
      'file'
    )}</span>`;
  } else {
...
const getSearchResultsMessage = function (results) {
  if ((results != null ? results.findPattern : undefined) != null) {
    ...
    if (matchCount) {
      return `${_.pluralize(matchCount, 'result')} found in ${_.pluralize(
        pathCount,
        'file'
...
const titleize = (str) =>
  str.toLowerCase().replace(/(?:^|\s)\S/g, (capital) => capital.toUpperCase());
