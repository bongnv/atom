...
module.exports = class ProviderConfig {
  ...
  scopeDescriptorToType(scopeDescriptor) {
    ...
    for (const type of Object.keys(config)) {
      ...
      if (
        typePriority > highestTypePriority &&
  ...
  addLegacyConfigEntry(suggestions) {
    ...
    this.config.builtin.suggestions =
      this.config.builtin.suggestions.concat(suggestions);
  ...
  addConfigEntry(config) {
    for (const type in config) {
      ...
      this.config[type].typePriority =
        options.typePriority != null ? options.typePriority : 1;

      const suggestions = this.sanitizeSuggestionsFromConfig(
        options.suggestions,
        type
      );
