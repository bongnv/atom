...
module.exports = class ProviderManager {
  ...
  applicableProviders(labels, scopeDescriptor) {
    ...
    providers = this.filterProvidersByScopeDescriptor(
      providers,
      scopeDescriptor
    );
  ...
  sortProviders(providers, scopeDescriptor) {
    ...
    return stableSort(providers, (providerA, providerB) => {
      const priorityA =
        providerA.provider.suggestionPriority != null
          ? providerA.provider.suggestionPriority
          : 1;
      const priorityB =
        providerB.provider.suggestionPriority != null
          ? providerB.provider.suggestionPriority
          : 1;
  ...
  apiVersionForProvider(provider) {
    if (
      this.metadataForProvider(provider) &&
      this.metadataForProvider(provider).apiVersion
    ) {
  ...
  See https://github.com/atom/autocomplete-plus/wiki/Provider-API.`);
    ...
    if (!this.isValidProvider(provider, apiVersion)) {
      console.warn(
        `Provider ${provider.constructor.name} is not valid`,
        provider
      );
