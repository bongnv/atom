...
module.exports = class AutocompleteManager {
  constructor() {
    ...
    this.getSuggestionsFromProviders =
      this.getSuggestionsFromProviders.bind(this);
  ...
  updateCurrentEditor(currentEditor, labels) {
    ...
    this.editorSubscriptions.add(
      new Disposable(() => {
        if (this.editorView) {
          this.editorView.removeEventListener(
            'compositionstart',
            compositionStart
          );
  ...
  handleEvents() {
    ...
    this.subscriptions.add(
      this.suggestionList.onDidCancel(this.hideSuggestionList)
    );
    this.subscriptions.add(
    ...
    return this.subscriptions.add(
      atom.commands.add('atom-text-editor', {
        ...
        'autocomplete-plus:navigate-to-description-more-link': () => {
          ...
          let descriptionContainer = suggestionListView.querySelector(
  ...
  getSuggestionsFromProviders(options) {
    ...
    providers.forEach((provider) => {
      ...
      return providerPromises.push(
        Promise.resolve(getSuggestions(upgradedOptions)).then(
          (providerSuggestions) => {
            ...
            if (apiVersion > 1 && providerSuggestions.length) {
              hasDeprecations = this.deprecateForSuggestion(
                provider,
            ...
            if (hasDeprecations || apiVersion === 1) {
              providerSuggestions = providerSuggestions.map((suggestion) => {
                const newSuggestion = {
                  text:
                    suggestion.text != null ? suggestion.text : suggestion.word,
                ...
                if (
                  newSuggestion.rightLabelHTML == null &&
                  suggestion.renderLabelAsHtml
                ) {
                  newSuggestion.rightLabelHTML = suggestion.label;
                }
                if (
                  newSuggestion.rightLabel == null &&
                  !suggestion.renderLabelAsHtml
                ) {
            ...
            for (let i = 0; i < providerSuggestions.length; i++) {
              ...
              if (
                suggestion.replacementPrefix == null ||
                !!suggestion.isPrefixModified
              ) {
                if (apiVersion < 4) {
                  suggestion.replacementPrefix = this.wordPrefixRegex.test(
                    options.prefix
                  )
                    ? options.prefix
                    : '';
            ...
            if (provider.filterSuggestions) {
              providerSuggestions = this.filterSuggestions(
                providerSuggestions,
                options
              );
            }
...
See https://github.com/atom/autocomplete-plus/wiki/Provider-API`);
  ...
  getPrefix(editor, position, scopeDescriptor) {
    ...
    const line = editor
      .getBuffer()
      .getTextInRange([[position.row, 0], position]);
  ...
  getWordCharacterRegex(scopeDescriptor) {
    ...
    if (!regex) {
      regex = new RegExp(
        `[${UnicodeLetters}${additionalWordChars.replace(']', '\\]')}\\d]`
      );
  ...
  getLegacyPrefix(editor, bufferPosition) {
    ...
    if (!prefix || !prefix[2] || prefix[2].length === MAX_LEGACY_PREFIX_LENGTH)
      return '';
    return prefix[2];
  }
  ...
  confirm(suggestion) {
    ...
    const apiVersion = this.providerManager.apiVersionForProvider(
      suggestion.provider
    );
  ...
  getDetailsOnSelect(suggestion) {
    if (
      ...
      Promise.resolve(
        suggestion.provider.getSuggestionDetailsOnSelect(suggestion)
      ...
      for (let i = 0; i < cursors.length; i++) {
        ...
        if (
          ...
          cursor.selection.selectLeft(
            suggestion.replacementPrefix.length + suffix.length
          );
  ...
  getSuffix(editor, bufferPosition, suggestion) {
    ...
    let suffix =
      suggestion.snippet != null ? suggestion.snippet : suggestion.text;
    const endPosition = [
      bufferPosition.row,
      bufferPosition.column + suffix.length,
    ];
    const endOfLineText = editor.getTextInBufferRange([
      bufferPosition,
      endPosition,
    ]);
    const nonWordCharacters = new Set(
      atom.config.get('editor.nonWordCharacters').split('')
    );
    while (suffix) {
      if (
        endOfLineText.startsWith(suffix) &&
        !nonWordCharacters.has(suffix[0])
      ) {
        break;
      }
  ...
  isCurrentFileBlackListed() {
    ...
    const fileName = path.basename(
      (left = this.buffer.getPath()) != null ? left : ''
    );
        ...
        if (this.autoActivationEnabled || this.suggestionList.isActive()) {
          if (newText.length > 0) {
            ...
            if (
              ...
              (newText.length === 2 &&
                this.bracketMatcherPairs.includes(newText))
            )
              return true;
          } else if (
            oldText.length > 0 &&
            (this.backspaceTriggersAutocomplete ||
              this.suggestionList.isActive())
          ) {
            ...
            if (
              ...
              (oldText.length === 2 &&
                this.bracketMatcherPairs.includes(oldText))
            )
              return true;
          }
        }
