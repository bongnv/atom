...
module.exports = class SubsequenceProvider {
    ...
    return this.configSuggestionsBuffer
      ...
      .then((matches) => {
        for (let k = 0; k < matches.length; k++) {
          matches[k].configSuggestion =
            suggestions[matches[k].positions[0].row];
        }
        return matches;
      });
  }
  ...
  */
  ...
  getSuggestions({ editor, bufferPosition, prefix, scopeDescriptor }) {
    ...
    const additionalWordCharacters =
      getAdditionalWordCharacters(scopeDescriptor);

    const configSuggestions =
      this.providerConfig.getSuggestionsForScopeDescriptor(scopeDescriptor);
    ...
    return Promise.all(
      buffers
        .map(
          this.bufferToSubsequenceMatches.bind(
            this,
            prefix,
            additionalWordCharacters
          )
