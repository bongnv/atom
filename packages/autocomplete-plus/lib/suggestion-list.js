...
module.exports = class SuggestionList {
  constructor() {
    ...
    this.confirmSelectionIfNonDefault =
  ...
  addBindings(editor) {
    ...
    const completionKey =
      atom.config.get('autocomplete-plus.confirmCompletion') || '';
    ...
    const useCoreMovementCommands = atom.config.get(
      'autocomplete-plus.useCoreMovementCommands'
    );
    const commandNamespace = useCoreMovementCommands
      ? 'core'
      : 'autocomplete-plus';
  ...
  */
  ...
  show(editor, options) {
    if (
      atom.config.get('autocomplete-plus.suggestionListFollows') === 'Cursor'
    ) {
    ...
    if (this.activeEditor === editor) {
      if (!bufferPosition.isEqual(this.displayBufferPosition)) {
        ...
        if (this.suggestionMarker) {
          this.suggestionMarker.setBufferRange([
            bufferPosition,
            bufferPosition,
          ]);
        }
