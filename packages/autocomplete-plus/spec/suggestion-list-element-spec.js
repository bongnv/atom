...
describe('Suggestion List Element', () => {
  ...
  describe('renderItem', () => {
    ...
    it('HTML escapes displayText', () => {
      ...
      expect(suggestionListElement.selectedLi.innerHTML).toContain(
        'Animal&lt;Cat&gt;'
      );
    ...
    it('HTML escapes labels', () => {
      ...
      suggestionListElement.renderItem(suggestion);
      expect(
        suggestionListElement.selectedLi.querySelector('.left-label').innerHTML
      ).toContain('Animal&lt;Cat&gt;');
      return expect(
  ...
  describe('itemChanged', () => {
    ...
    it('updates the list item', async () => {
      ...
      suggestionListElement.itemChanged({
      ...
      expect(
        suggestionListElement.element.querySelector(
  ...
  describe('getDisplayHTML', () => {
    it('uses displayText over text or snippet', () => {
      ...
      expect(fragmentToHtml(html)).toBe(
        '<span class="character-match">a</span>cd'
      );
    });
    ...
    it('outputs correct html when there are no snippets in the snippet field', () => {
      ...
      expect(fragmentToHtml(html)).toBe(
        '<span class="character-match">a</span>bc(d, e)f'
      );
    });
    ...
    it('replaces a snippet with escaped multiple right braces', () => {
      let text = '';
      let snippet =
        'abc(${1:d}, ${2:something{ok\\}}, ${3:e})f ${4:interface{\\}}';
      let replacementPrefix = 'a';
      let display = suggestionListElement.getDisplayFragment(
        text,
        snippet,
  ...
  describe('findCharacterMatches', () => {
    let assertMatches = (text, replacementPrefix, truthyIndices) => {
      ...
      let matches = suggestionListElement.findCharacterMatchIndices(
        text,
        replacementPrefix
      );
  ...
  describe('removeEmptySnippets', () => {
    ...
    it('removes an empty snippet group with surrounding text', () => {
      expect(suggestionListElement.removeEmptySnippets('hello$0')).toBe(
        'hello'
      );
      expect(suggestionListElement.removeEmptySnippets('$0hello')).toBe(
        'hello'
      );
      expect(suggestionListElement.removeEmptySnippets('hello$0hello')).toBe(
        'hellohello'
      );
      expect(suggestionListElement.removeEmptySnippets('hello$1000hello')).toBe(
        'hellohello'
      );
    });
    ...
    it('removes an empty snippet group with braces with surrounding text', () => {
      ...
      expect(suggestionListElement.removeEmptySnippets('${0}hello')).toBe(
        'hello'
      );
      expect(suggestionListElement.removeEmptySnippets('hello${0}hello')).toBe(
        'hellohello'
      );
      expect(
        suggestionListElement.removeEmptySnippets('hello${1000}hello')
      ).toBe('hellohello');
    });
  ...
  describe('moveSelectionUp', () => {
    ...
    it('cycles to the last element in the suggestion list when the current selection is at the start of the list', () => {
      spyOn(suggestionListElement, 'visibleItems').and.returnValue([
        'a',
        'b',
        'c',
        'd',
