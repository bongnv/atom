...
describe('Provider Manager', () => {
  ...
  describe('when no providers have been registered, and enableBuiltinProvider is true', () => {
    ...
    it('registers the default provider for all scopes', () => {
      expect(
        providerManager.applicableProviders(['workspace-center'], '*').length
      ).toBe(1);
      expect(
        providerManager.applicableProviders(['workspace-center'], '*')[0]
      ).toBe(providerManager.defaultProvider);
    });

    it('adds providers', () => {
      expect(providerManager.isProviderRegistered(testProvider)).toEqual(false);
      expect(hasDisposable(providerManager.subscriptions, testProvider)).toBe(
        false
      );
    ...
    it('registers a valid provider', () => {
      expect(
        providerManager.applicableProviders(['workspace-center'], '.source.js')
          .length
      ).toEqual(1);
      expect(
        providerManager
  ...
  describe('when providers have been registered', () => {
    ...
    beforeEach(() => {
      ...
      testProvider2 = {
        ...
        providerblacklist: {
          'autocomplete-plus-fuzzyprovider':
            '.source.js .variable.js .comment3',
        },
        getSuggestions(options) {
          return [
            {
    ...
    it('does not return providers if the scopeChain exactly matches a global blacklist item', () => {
      expect(
        providerManager.applicableProviders(
          ['workspace-center'],
          '.source.js .comment'
        )
      ).toHaveLength(4);
      atom.config.set('autocomplete-plus.scopeBlacklist', [
        '.source.js .comment',
      ]);
      expect(
        providerManager.applicableProviders(
