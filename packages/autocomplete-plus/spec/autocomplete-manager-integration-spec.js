...
describe('Autocomplete Manager', () => {
  ...
  describe('when an external provider is registered', () => {
    ...
    beforeEach(async () => {
      ...
      mainModule = (await atom.packages.activatePackage('autocomplete-plus'))
        .mainModule;
    ...
    it('does not display empty suggestions', async () => {
      ...
      expect(editorView.querySelector('.autocomplete-plus')).toExist();
      expect(editorView.querySelectorAll('.autocomplete-plus li')).toHaveLength(
        2
      );
    });
    ...
    describe('when filterSuggestions option is true', () => {
      ...
      it('does not display empty suggestions', async () => {
        ...
        expect(editorView.querySelector('.autocomplete-plus')).toExist();
        expect(
          editorView.querySelectorAll('.autocomplete-plus li')
        ).toHaveLength(2);
      });
    });
    ...
    describe('when the type option has a space in it', () =>
      it('does not display empty suggestions', async () => {
        ...
        expect(items[0].querySelector('.icon').className).toBe(
          'icon local function'
        );
        expect(items[1].querySelector('.icon').className).toBe(
          'icon another ~ function'
        );
      }));
      ...
      describe('when a suggestion description is not specified', () => {
        ...
        it('scrolls the list always showing the selected item', async () => {
          ...
          let itemHeight = parseInt(
            getComputedStyle(editorView.querySelector('.autocomplete-plus li'))
              .height
          );
          ...
          let scroller = suggestionList.querySelector(
          ...
          expect(
            editorView.querySelectorAll('.autocomplete-plus li')[1]
          ).toHaveClass('selected');
          expect(scroller.scrollTop).toBe(0);

          atom.commands.dispatch(suggestionList, 'core:move-down');
          expect(
            editorView.querySelectorAll('.autocomplete-plus li')[2]
          ).toHaveClass('selected');
          expect(scroller.scrollTop).toBe(itemHeight);

          atom.commands.dispatch(suggestionList, 'core:move-down');
          expect(
            editorView.querySelectorAll('.autocomplete-plus li')[3]
          ).toHaveClass('selected');
          expect(scroller.scrollTop).toBe(itemHeight * 2);

          atom.commands.dispatch(suggestionList, 'core:move-down');
          expect(
            editorView.querySelectorAll('.autocomplete-plus li')[0]
          ).toHaveClass('selected');
          expect(scroller.scrollTop).toBe(0);
        ...
        it('only shows the maxVisibleSuggestions in the suggestion popup', () => {
          ...
          expect(
            suggestionList.querySelector('.suggestion-list-scroller').style[
              'max-height'
            ]
          ...
          let descriptionHeight = parseInt(
            getComputedStyle(
              editorView.querySelector(
                '.autocomplete-plus .suggestion-description'
              )
            ).height
          );
          expect(suggestionList.offsetHeight).toBe(
            2 * itemHeight + descriptionHeight
          );
          expect(
            suggestionList.querySelector('.suggestion-list-scroller').style[
              'max-height'
        ...
        it('parses markdown in the description', async () => {
          spyOn(provider, 'getSuggestions').and.callFake(({ prefix }) => {
            let list = [
              {
                text: 'ab',
                descriptionMarkdown: '**mmmmmmmmmmmmmmmmmmmmmmmmmm**',
              },
              {
                text: 'abc',
                descriptionMarkdown: '**mmmmmmmmmmmmmmmmmmmmmm**',
              },
              { text: 'abcd', descriptionMarkdown: '**mmmmmmmmmmmmmmmmmm**' },
              { text: 'abcde', descriptionMarkdown: '**mmmmmmmmmmmmmm**' },
            ];
            return list
              .filter((item) => item.text.startsWith(prefix))
              .map((item) => item);
          });
          ...
          expect(
            editorView.querySelector(
              '.autocomplete-plus .suggestion-description strong'
            ).textContent
    ...
    describe('when match.snippet is used', () => {
      ...
      describe('when the snippets package is enabled', () => {
        ...
        it('displays the snippet without the `${1:}` in its own class', async () => {
          ...
          let wordElement = editorView.querySelector(
            '.autocomplete-plus span.word'
          );
          expect(wordElement.textContent).toBe('method(something)');
          expect(
            wordElement.querySelector('.snippet-completion').textContent
          ).toBe('something');

          let wordElements = editorView.querySelectorAll(
            '.autocomplete-plus span.word'
          );
        ...
        it('escapes \\ in list to match snippet behavior', async () => {
          ...
          expect(wordElements).toHaveLength(4);
          expect(wordElements[3].textContent).toBe(
            'namespace\\method4(something)'
          );
    ...
    describe('when the matched prefix is highlighted', () => {
      ...
      describe('when the prefix does not match the word', () => {
        it('does not render any character-match spans', async () => {
          ...
          let text = editorView.querySelector(
            '.autocomplete-plus li span.word'
          ).textContent;
        ...
        describe('when the snippets package is enabled', () => {
          ...
          it('does not highlight the snippet html; ref issue 301', async () => {
            ...
            let word = editorView.querySelector(
              '.autocomplete-plus li span.word'
            );
    ...
    describe("when autocomplete-plus.suggestionListFollows is 'Cursor'", () => {
      ...
      it('places the suggestion list at the cursor', async () => {
        ...
        expect(overlayElement.style.left).toBe(
          pixelLeftForBufferPosition([0, 10])
        );
    ...
    describe("when autocomplete-plus.suggestionListFollows is 'Word'", () => {
      ...
      it('opens to the correct position, and correctly closes on cancel', async () => {
        ...
        expect(overlayElement).toExist();
        expect(overlayElement.style.left).toBe(
          pixelLeftForBufferPosition([0, 12])
        );
      });
      ...
      it('displays the suggestion list taking into account the passed back replacementPrefix', async () => {
        ...
        expect(overlayElement).toExist();
        expect(overlayElement.style.left).toBe(
          pixelLeftForBufferPosition([0, 14])
        );
      });
      ...
      it('when broken by a non-word character, the suggestion list is positioned at the beginning of the new word', async () => {
        ...
        expect(overlayElement.style.left).toBe(
          pixelLeftForBufferPosition([0, 17])
        );
    ...
    describe('when auto-activation is disabled', () => {
      ...
      it('does not accept the suggestion if auto-confirm single suggestion is disabled', async () => {
        ...
        atom.config.set(
          'autocomplete-plus.enableAutoConfirmSingleSuggestion',
          false
        );
        expect(editorView.querySelector('.autocomplete-plus')).not.toExist();
        atom.commands.dispatch(editorView, 'autocomplete-plus:activate');
...
defm`);
    ...
    describe('label rendering', () => {
      describe('when no labels are specified', () => {
        ...
        it('displays the text in the suggestion', async () => {
          triggerAutocompletion(editor);
          await waitForAutocomplete(editor);
          let iconContainer = editorView.querySelector(
            '.autocomplete-plus li .icon-container'
          );
          let leftLabel = editorView.querySelector(
            '.autocomplete-plus li .right-label'
          );
          let rightLabel = editorView.querySelector(
            '.autocomplete-plus li .right-label'
          );
      ...
      describe('when `type` is specified', () => {
        ...
        it('displays an icon in the icon-container', async () => {
          ...
          let icon = editorView.querySelector(
            '.autocomplete-plus li .icon-container .icon'
          );
      ...
      describe('when the `type` specified has a default icon', () => {
        ...
        it('displays the default icon in the icon-container', async () => {
          ...
          let icon = editorView.querySelector(
            '.autocomplete-plus li .icon-container .icon i'
          );
      ...
      describe('when `iconHTML` is specified', () => {
        ...
        it('displays an icon in the icon-container', async () => {
          ...
          let icon = editorView.querySelector(
            '.autocomplete-plus li .icon-container .icon .omg'
          );
          expect(icon).toExist();
        });
      });
      ...
      describe('when `rightLabel` is specified', () => {
        ...
        it('displays the text in the suggestion', async () => {
          ...
          let label = editorView.querySelector(
            '.autocomplete-plus li .right-label'
          );
      ...
      describe('when `rightLabelHTML` is specified', () => {
        ...
        it('displays the text in the suggestion', async () => {
          ...
          let label = editorView.querySelector(
            '.autocomplete-plus li .right-label .something'
          );
      ...
      describe('when `leftLabel` is specified', () => {
        ...
        it('displays the text in the suggestion', async () => {
          ...
          let label = editorView.querySelector(
            '.autocomplete-plus li .left-label'
          );
          expect(label).toHaveText('<i class="something">sometext</i>');
        });
      });
      ...
      describe('when `leftLabelHTML` is specified', () => {
        ...
        it('displays the text in the suggestion', async () => {
          ...
          let label = editorView.querySelector(
            '.autocomplete-plus li .left-label .something'
          );
          expect(label).toHaveText('sometext');
        });
      });
  ...
  describe('when opening a javascript file', () => {
    beforeEach(async () => {
      ...
      await timeoutPromise(
        autocompleteManager.providerManager.defaultProvider
          .deferBuildWordListInterval + 100
      );
    });
    ...
    describe('when the buffer changes', () => {
      ...
      it('shows the suggestion list on backspace if allowed', async () => {
        atom.config.set(
          'autocomplete-plus.backspaceTriggersAutocomplete',
          true
        );
        expect(editorView.querySelector('.autocomplete-plus')).not.toExist();
      ...
      it('does not update the suggestion list while composition is in progress', async () => {
        ...
        spyOn(
          autocompleteManager.suggestionList,
          'changeItems'
        ).and.callThrough();
        ...
        expect(
          autocompleteManager.suggestionList.changeItems
        ).toHaveBeenCalled();
        expect(
          autocompleteManager.suggestionList.changeItems.calls.mostRecent()
            .args[0][0].text
        ).toBe('quicksort');
        autocompleteManager.suggestionList.changeItems.calls.reset();

        activeElement.dispatchEvent(
          buildIMECompositionEvent('compositionstart', {
            data: 'i',
        ...
        activeElement.dispatchEvent(
          buildIMECompositionEvent('compositionupdate', {
            data: 'ï',
            target: activeElement,
          })
        );
        ...
        const initialCallCount =
          autocompleteManager.suggestionList.changeItems.calls.count();
        await conditionPromise(
          () =>
            autocompleteManager.suggestionList.changeItems.calls.count() >
            initialCallCount
  ...
  describe('when a long completion exists', () => {
    ...
    it('sets the width of the view to be wide enough to contain the longest completion without scrolling', async () => {
      ...
      let suggestionListView =
        autocompleteManager.suggestionList.suggestionListElement;
      expect(suggestionListView.scrollWidth).toBe(
        suggestionListView.offsetWidth
      );
    });
  });
  ...
  describe('when watching an external text editor for a given set of provider labels', () => {
    ...
    beforeEach(async () => {
      ...
      autocompleteDisposable = autocompleteManager.watchEditor(bottomEditor, [
        'bottom-label',
      ]);
    });
    ...
    it('provides appropriate autocompletions when each editor is focused.', async () => {
      ...
      expect(
        bottomEditorView.querySelector('.autocomplete-plus')
      ).not.toExist();
