...
module.exports = class DirectoryView {
  constructor(directory) {
    ...
    this.subscriptions.add(
      this.directory.onDidDestroy(() => this.subscriptions.dispose())
    );
    ...
    if (this.directory.squashedNames != null) {
      ...
      this.directoryName.appendChild(
        document.createTextNode(this.directory.squashedNames[1])
      );
    } else {
    ...
    this.subscriptions.add(
      this.directory.onDidStatusChange(() => this.updateStatus())
    );
    this.updateStatus();
        ...
        for (let entry of addedEntries) {
          ...
          if (insertionIndex < numberOfEntries) {
            this.entries.insertBefore(
              view.element,
              this.entries.children[insertionIndex]
            );
          } else {
