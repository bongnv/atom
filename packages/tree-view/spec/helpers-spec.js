...
describe('Helpers', () => {
  describe('repoForPath', () => {
    ...
    beforeEach(async () => {
      ...
      fixturesRepo = await atom.project.repositoryForDirectory(
    ...
    it('returns the project repository for a subpath', () => {
      expect(
        helpers.repoForPath(
          path.join(fixturesPath, 'root-dir1', 'tree-view.txt')
        )
      ).toEqual(fixturesRepo);
    });
  ...
  describe('getFullExtension', () => {
    ...
    it('returns the extension for a path', () => {
      expect(
        helpers.getFullExtension(path.join('path', 'to', 'filename.txt'))
      ).toBe('.txt');
    });
    ...
    it('returns the full extension for a filename with more than one extension', () => {
      ...
      expect(helpers.getFullExtension('archive.tar.gz.bak')).toBe(
        '.tar.gz.bak'
      );
    });
    ...
    it('returns no extension when the filename begins with a period', () => {
      ...
      expect(
        helpers.getFullExtension(path.join('path', 'to', '.gitconfig'))
      ).toBe('');
    });
  });
});
