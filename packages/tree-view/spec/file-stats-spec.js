...
describe('FileStats', function () {
  describe('provision of filesystem stats', function () {
    let [file1Data, file2Data, timeStarted, treeView] = Array.from([
    ...
    it('passes stats to File instances', function () {
      const { stats } = treeView.roots[0].directory.entries.get('file1.txt');
    ...
    it('passes stats to Directory instances', function () {
      const { stats } = treeView.roots[0].directory.entries.get('subdir');
      expect(stats).toBeDefined();
    ...
    it('passes stats to File instances in subdirectories', function () {
      ...
      const { stats } = subdir.entries.get('file2.txt');
      expect(stats).toBeDefined();
  ...
  return describe('virtual filepaths', function () {
    ...
    return it("doesn't throw an exception when accessing virtual filepaths", () =>
