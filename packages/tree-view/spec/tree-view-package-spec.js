...
const isFilesystemCaseSensitive = function () {
  if (isCaseSensitive === null) {
    ...
    if (fs.existsSync(path.join(tempPath, 'CASE-SENSITIVE-CHECK.txt'))) {
...
const waitForWorkspaceOpenEvent = (causeFileToOpen) =>
...
describe('TreeView', function () {
  ...
  const selectEntry = (pathToSelect) =>
    treeView.selectEntryForPath(
      atom.project.getDirectories()[0].resolve(pathToSelect)
    );
  ...
  beforeEach(function () {
    ...
    return runs(function () {
      const moduleInstance = atom.packages
  ...
  afterEach(function () {
    ...
    if (
      (treeViewOpenPromise = __guard__(
        atom.packages.getActivePackage('tree-view'),
        (x) => x.mainModule.treeViewOpenPromise
      ))
    ) {
  ...
  describe('on package activation', function () {
    it('renders the root directories of the project and their contents alphabetically with subdirectories first, in a collapsed state', function () {
      expect(root1.querySelector('.header .disclosure-arrow')).not.toHaveClass(
        'expanded'
      );
    ...
    return describe('when the project has no paths', function () {
      ...
      it('displays a view to add projects', function () {
        ...
        return expect(
          treeView.element.querySelector('.tree-view-root')
        ).not.toExist();
      });
      ...
      describe("when clicking on 'Add projects'", function () {
        ...
        it('opens up a folder picker', function () {
          ...
          addProjectsButton.dispatchEvent(
      ...
      describe("when clicking on 'Reopen Projects'", function () {
        ...
        beforeEach(
          () =>
            (reopenProjectsButton = treeView.element.querySelectorAll(
              '#add-projects-view .btn'
            )[1])
        );
        ...
        return it('opens a modal to choose an old project', function () {
          ...
          atom.commands.onDidDispatch(function (event) {
            if (event.type === 'application:reopen-project') {
              return (done = true);
            }
          });

          reopenProjectsButton.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
      ...
      return describe('when the project is assigned a path', () =>
        it('creates a root directory view', function () {
          const projectPath = temp.mkdirSync('atom-project');

          atom.project.setPaths([projectPath]);

          expect(treeView.roots).toHaveLength(1);
          expect(fs.absolute(treeView.roots[0].getPath())).toBe(
            fs.absolute(projectPath)
          );

          expect(treeView.element.querySelector('.tree-view-root')).toExist();
          return expect(
            treeView.element.querySelector('#add-projects-view')
          ).not.toExist();
        }));
    });
  });
    ...
    return it('waits for the Tree View to open before destroying it', function () {
      ...
      runs(() =>
        spyOn(atom.workspace, 'open').andReturn(
          new Promise((resolve) => (resolveOpenPromise = resolve))
        )
      );
      ...
      return runs(function () {
        atom.packages
          .deactivatePackage('tree-view')
          .then(() => expect(opened).toBe(true));
  ...
  describe('when tree-view:toggle is triggered on the root view', function () {
    ...
    return describe('when the tree view is hidden', () =>
      it('shows and focuses the tree view', function () {
  ...
  describe('when tree-view:toggle-focus is triggered on the root view', function () {
    ...
    return describe('when the tree view is shown', function () {
      ...
      return describe('when the tree view is focused', () =>
        it('unfocuses the tree view', function () {
          waitsForPromise(() => atom.workspace.open()); // When we call focus below, we want an editor to become focused
  ...
  describe('when the tree-view is destroyed', () =>
    it('can correctly re-create the tree-view', function () {
      treeView = atom.workspace.getLeftDock().getActivePaneItem();
      const entryCountBeforeRecreatingView =
        treeView.element.getElementsByClassName('entry').length;
      ...
      return runs(function () {
        ...
        const entryCountAfterRecreatingView =
          treeView2.element.getElementsByClassName('entry').length;
        return expect(entryCountAfterRecreatingView).toBe(
          entryCountBeforeRecreatingView
        );
      });
    }));
  ...
  describe('when tree-view:reveal-active-file is triggered', function () {
    ...
    describe('if the current file has a path', function () {
      describe('if the tree-view.focusOnReveal config option is true', () =>
        it('shows and focuses the tree view and selects the file', function () {
      ...
      describe('if the tree-view.focusOnReveal config option is false', () =>
        it('shows the tree view and selects the file, but does not change the focus', function () {
      ...
      return describe('if the file is located under collapsed folders', () =>
        it('expands all the folders and selects the file', function () {
          waitsForPromise(() =>
            atom.workspace.open(path.join(path1, 'dir1', 'file1'))
          );
          ...
          return runs(function () {
            expect(
              treeView.entryForPath(path1).classList.contains('expanded')
            ).toBe(true);
            expect(
              treeView
                .entryForPath(path.join(path1, 'dir1'))
                .classList.contains('expanded')
            ).toBe(true);
            expect(treeView.selectedEntry()).toBeTruthy();
            return expect(treeView.selectedEntry().getPath()).toBe(
              path.join(path1, 'dir1', 'file1')
            );
          });
        }));
    });
    ...
    describe('if the current file has no path', () =>
      it('shows and focuses the tree view, but does not attempt to select a specific file', function () {
    ...
    return describe('if there are more items than can be visible in the viewport', function () {
      ...
      return it('scrolls the selected file into the visible view', function () {
        // Open file at bottom
        waitsForPromise(() =>
          atom.workspace.open(path.join(rootDirPath, 'file-20.txt'))
        );
        waitsForPromise(() => treeView.revealActiveFile());
        runs(function () {
          expect(treeView.scrollTop()).toBeGreaterThan(400);
          const entries = treeView.element.querySelectorAll('.entry');
          let { scrollTop } = treeView.element;
          return (() => {
            const result = [];
            for (
              let i = 0, end = entries.length, asc = 0 <= end;
              asc ? i < end : i > end;
              asc ? i++ : i--
            ) {
              atom.commands.dispatch(treeView.element, 'core:move-up');
              expect(treeView.element.scrollTop - scrollTop).toBeLessThan(
                entries[i].clientHeight
              );
        ...
        waitsForPromise(() =>
          atom.workspace.open(path.join(rootDirPath, 'file-10.txt'))
        );
        waitsForPromise(() => treeView.revealActiveFile());
        runs(function () {
        ...
        // Open file at top
        waitsForPromise(() =>
          atom.workspace.open(path.join(rootDirPath, 'file-1.txt'))
        );
        waitsForPromise(() => treeView.revealActiveFile());
  ...
  describe('when tree-view:unfocus is triggered on the tree view', () =>
    it('surrenders focus to the workspace but remains open', function () {
      ...
      return runs(function () {
        ...
        return expect(
          atom.workspace.getCenter().getActivePane().isActive()
        ).toBe(true);
      });
    }));
  ...
  describe('copy path commands', function () {
    ...
    beforeEach(function () {
      pathToSelect = path.join(
        treeView.roots[0].directory.path,
        'dir1',
        'file1'
      );
    ...
    return describe('when tree-view:copy-project-path is triggered on the tree view', function () {
      it('copies the relativized selected path to the clipboard', function () {
        ...
        return expect(atom.clipboard.write).toHaveBeenCalledWith(
          relativizedPath
        );
      });
  ...
  describe("when a directory's disclosure arrow is clicked", function () {
    ...
    it('restores the expansion state of descendant directories', function () {
      ...
      expect(
        root1.querySelectorAll('.entries > li > .entries > li > .entries')
          .length
      ).toBe(1);

      // collapsed descendants remain collapsed
      return expect(
        root1.querySelectorAll('.entries > li')[1].querySelector('.entries')
  ...
  describe('when the package first activates and there is a file open (regression)', function () {
    ...
    describe('when the file is permanent', function () {
      beforeEach(() =>
        waitForWorkspaceOpenEvent(() => atom.workspace.open('tree-view.js'))
      );
    ...
    return describe('when the file is pending', function () {
      ...
      beforeEach(() =>
        waitsForPromise(() =>
          atom.workspace
            .open('tree-view.js', { pending: true })
            .then((o) => (editor = o))
        )
      );
      ...
      return it('marks the pending file as permanent', function () {
        runs(function () {
          expect(
            atom.workspace.getCenter().getActivePane().getActiveItem()
          ).toBe(editor);
          expect(
            atom.workspace.getCenter().getActivePane().getPendingItem()
          ).toBe(editor);
          sampleJs.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
        ...
        return waitsFor(
          () =>
            atom.workspace.getCenter().getActivePane().getPendingItem() === null
        );
      });
    });
  });
  ...
  describe('when files are clicked', function () {
    ...
    describe('when a file is single-clicked', function () {
      describe('when core.allowPendingPaneItems is set to true (default)', function () {
        ...
        beforeEach(function () {
          ...
          waitForWorkspaceOpenEvent(function () {
            let r;
            return (r = sampleJs.dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 1 })
            ));
          });
        ...
        return it('opens the file in a pending state', function () {
          expect(activePaneItem.getPath()).toBe(
            atom.project.getDirectories()[0].resolve('tree-view.js')
          );
          return expect(
            atom.workspace.getCenter().getActivePane().getPendingItem()
          ).toEqual(activePaneItem);
        });
      });
      ...
      describe('when core.allowPendingPaneItems is set to false', function () {
        ...
        return it('does not open the file', () =>
          expect(atom.workspace.open).not.toHaveBeenCalled());
      });

      return describe('when it is immediately opened with `::openSelectedEntry` afterward', () =>
        it('does not open a duplicate file', function () {
          ...
          return runs(() =>
            expect(
              atom.workspace.getCenter().getActivePane().getItems().length
            ).toBe(1)
          );
        }));
    });
  ...
  describe('when a directory is double-clicked', () =>
    it('toggles the directory expansion state and does not change the focus to the editor', function () {
      jasmine.attachToDOM(workspaceElement);
  ...
  describe('when an directory is alt-clicked', function () {
    describe('when the directory is collapsed', () =>
      it('recursively expands the directory', function () {
        root1.dispatchEvent(
          new MouseEvent('click', { bubbles: true, detail: 1 })
        );
        treeView.roots[0].collapse();
        ...
        expect(treeView.roots[0]).not.toHaveClass('expanded');
        root1.dispatchEvent(
          new MouseEvent('click', { bubbles: true, detail: 1, altKey: true })
        );
        expect(treeView.roots[0]).toHaveClass('expanded');

        const children = root1.querySelectorAll('.directory');
        expect(children.length).toBeGreaterThan(0);
        return children.map((child) => expect(child).toHaveClass('expanded'));
      }));
    ...
    return describe('when the directory is expanded', function () {
      ...
      beforeEach(function () {
        ...
        return children.map((child) => child.expand());
      });
    ...
    describe('when the item has a path', function () {
      it('selects the entry with that path in the tree view if it is visible', function () {
        ...
        return runs(function () {
          ...
          return expect(
            treeView.element.querySelectorAll('.selected').length
          ).toBe(1);
        });
      });
      ...
      return describe('when the tree-view.autoReveal config setting is true', function () {
        ...
        return it("selects the active item's entry in the tree view, expanding parent directories if needed", function () {
          ...
          waitsFor(
            () => treeView.getSelectedEntries()[0].textContent === 'sub-file1'
          );
  ...
  describe('keyboard navigation', function () {
    ...
    describe('core:move-down', function () {
      describe('when a collapsed directory is selected', () =>
        it('skips to the next directory', function () {
          root1
          ...
          return expect(root1.querySelectorAll('.directory')[1]).toHaveClass(
            'selected'
          );
        }));

      describe('when an expanded directory is selected', () =>
        it('selects the first entry of the directory', function () {
      ...
      describe('when the last entry of an expanded directory is selected', () =>
        it('selects the entry after its parent directory', function () {
          const subdir1 = root1.querySelectorAll('.directory')[1];
          subdir1.expand();
          waitForWorkspaceOpenEvent(function () {
            const entries = subdir1.querySelectorAll('.entry');
            return entries[entries.length - 1].dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 1 })
            );
          });
          ...
          return runs(function () {
            atom.commands.dispatch(treeView.element, 'core:move-down');
            return expect(root1.querySelectorAll('.directory')[2]).toHaveClass(
              'selected'
            );
          });
        }));
      ...
      describe('when the last directory of another last directory is selected', function () {
        ...
        beforeEach(function () {
          ...
          expect(nested.querySelector('.header').textContent).toContain(
            'nested'
          );
          nested.expand();
          const entries = nested.querySelectorAll('.entry');
          nested2 = entries[entries.length - 1];
          nested2.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
        ...
        return describe('when the directory is expanded', () =>
          it('selects the entry after its grandparent directory', function () {
            nested2.expand();
    ...
    describe('core:move-up', function () {
      describe('when there is an expanded directory before the currently selected entry', function () {
        ...
        beforeEach(function () {
          ...
          lastDir.expand();
          return waitForWorkspaceOpenEvent(() =>
        ...
        return describe('when the expanded directory has no children', () =>
      ...
      describe('when there is an entry before the currently selected entry', () =>
        it('selects the previous entry', function () {
      ...
      describe('when there is no entry before the currently selected entry, but there is a parent directory', () =>
        it('selects the parent directory', function () {
          const subdir = root1.querySelector('.directory');
          subdir.expand();
          subdir
            .querySelector('.entries .entry')
            .dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 1 })
            );
      ...
      return describe('when the tree view is empty', () =>
        it('does nothing', function () {
          atom.commands.dispatch(
            treeView.roots[0].querySelector('.header'),
    ...
    describe('core:move-to-top', function () {
      it('scrolls to the top', function () {
        ...
        for (element of treeView.element.querySelectorAll('.directory')) {
          element.expand();
        }
    ...
    describe('core:page-up', () =>
      it('scrolls up a page', function () {
        ...
        const { scrollTop } = treeView.element;
        expect(scrollTop).toBeGreaterThan(0);

        atom.commands.dispatch(treeView.element, 'core:page-up');
        return expect(treeView.element.scrollTop).toBe(
          scrollTop - treeView.element.offsetHeight
        );
      }));

    describe('core:page-down', () =>
      it('scrolls down a page', function () {
        let element;
        treeView.element.style.height = '5px';
        jasmine.attachToDOM(treeView.element);
        for (element of treeView.element.querySelectorAll('.directory')) {
    ...
    describe('movement outside of viewable region', () =>
      it('scrolls the tree view to the selected item', function () {
        ...
        const entryCount = treeView.element.querySelectorAll('.entry').length;
        const entryHeight =
          treeView.element.querySelector('.file').offsetHeight;

        _.times(entryCount, () =>
          atom.commands.dispatch(treeView.element, 'core:move-down')
        );
        expect(
          treeView.element.scrollTop + treeView.element.offsetHeight
        ).toBeGreaterThan(entryCount * entryHeight - 1);
    ...
    describe('tree-view:expand-directory', function () {
        ...
        return describe('when the directory is already expanded', function () {
          describe('when the directory is empty', () =>
            xit('does nothing', function () {
              const rootDirPath = fs.absolute(
                temp.mkdirSync('tree-view-root1')
              );
          ...
          return describe('when the directory has entries', () =>
            it('moves the cursor down to the first sub-entry', function () {
              ...
              atom.commands.dispatch(treeView.element, 'tree-view:expand-item');
              return expect(subdir.querySelector('.entry')).toHaveClass(
                'selected'
              );
            }));
        });
      ...
      return describe('when a file entry is selected', () =>
        it('does nothing', function () {
          waitForWorkspaceOpenEvent(() =>
            root1
              .querySelector('.file')
    ...
    describe('tree-view:recursive-expand-directory', function () {
      describe('when an collapsed root is recursively expanded', () =>
        it('expands the root and all subdirectories', function () {
          root1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          treeView.roots[0].collapse();
      ...
      return describe('when a file is selected and ordered to recursively expand', () =>
        it("recursively expands the selected file's parent directory", function () {
    ...
    describe('tree-view:collapse-directory', function () {
      ...
      describe('when an expanded directory is selected', () =>
        it('collapses the selected directory', function () {
          subdir.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          subdir.expand();
      ...
      describe('when a collapsed directory is selected', () =>
        it("collapses and selects the selected directory's parent directory", function () {
      ...
      return describe('when a file is selected', () =>
        it("collapses and selects the selected file's parent directory", function () {
          waitForWorkspaceOpenEvent(() =>
            subdir
              .querySelector('.file')
              .dispatchEvent(
    ...
    describe('tree-view:recursive-collapse-directory', function () {
      ...
      return describe('when an expanded directory is recursively collapsed', () =>
        it('collapses the directory and all its child directories', function () {
          let child;
          parent.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
    ...
    describe('tree-view:collapse-all', function () {
      const expandAll = () =>
        (() => {
          const result = [];
          for (var root of treeView.roots) {
            root.expand(true);
    ...
    describe('tree-view:open-selected-entry', function () {
      ...
      describe('when a directory is selected', () =>
        it('expands or collapses the directory', function () {
          const subdir = root1.querySelector('.directory');
          subdir.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
      ...
      return describe('when nothing is selected', () =>
        it('does nothing', function () {
          atom.commands.dispatch(
            treeView.element,
            'tree-view:open-selected-entry'
        ...
        return describe(command, function () {
          describe('when a file is selected', function () {
            ...
            return it('opens the file in the new split pane and focuses it', function () {
              ...
              const splitPaneItem = atom.workspace
                .getCenter()
                .getActivePaneItem();
              expect(previousPane).not.toBe(splitPane);
              expect(splitPaneItem.getPath()).toBe(
                atom.project.getDirectories()[0].resolve('tree-view.txt')
              );
  ...
  describe('opening in existing split panes', function () {
    beforeEach(function () {
      jasmine.attachToDOM(workspaceElement);
      return [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach(() =>
        waitForWorkspaceOpenEvent(function () {
      ...
      return describe(command, () =>
        describe('when a file is selected', function () {
          ...
          return it(`opens the file in pane ${paneNumber} and focuses it`, function () {
            ...
            return expect(item.getPath()).toBe(
              atom.project.getDirectories()[0].resolve('tree-view.txt')
            );
          });
        })
  ...
  describe('opening existing opened files in existing split panes', function () {
    beforeEach(function () {
      ...
      jasmine.attachToDOM(workspaceElement);
      global.debug = true;
      return [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach((index) =>
        waitForWorkspaceOpenEvent(function () {
    ...
    return [0, 1, 2, 3, 4, 5, 6, 7, 8].forEach(function (index) {
      ...
      return describe(command, () =>
        [1, 2, 3, 4, 5, 6, 7, 8, 9].forEach(function (fileIndex) {
          ...
          return describe(`when a file is selected that is already open in pane ${fileIndex}`, function () {
            ...
            return it(`opens the file in pane ${paneNumber} and focuses it`, function () {
              ...
              expect(atom.views.getView(pane)).toHaveFocus();
              return expect(item.getPath()).toBe(
                atom.project.getDirectories()[0].resolve(fileName)
              );
            });
          });
        })
  ...
  describe('removing a project folder', function () {
    describe('when the project folder is selected', () =>
      it('removes the folder from the project', function () {
    ...
    describe('when an entry is selected', () =>
      it('removes the project folder containing the entry', function () {
        treeView.selectEntry(
          treeView.roots[1].querySelector('.entries').querySelector('li')
        );
        atom.commands.dispatch(
          treeView.element,
          'tree-view:remove-project-folder'
    ...
    describe('when nothing is selected and there is only one project folder', () =>
      it('removes the project folder', function () {
    ...
    return describe('when nothing is selected and there are multiple project folders', () =>
      it('does nothing', function () {
  ...
  describe('file modification', function () {
    ...
    beforeEach(function () {
      ...
      dirView3.expand();
      [fileView, fileView2, fileView3] = Array.from(
        root1.querySelectorAll('.file')
      );
    ...
    describe('tree-view:copy', function () {
      ...
      return describe('when multiple files are selected', () =>
    ...
    describe('tree-view:paste', function () {
      ...
      describe('when attempting to paste a directory into itself', () =>
        it('shows a warning notification and does not paste', function () {
          ...
          return expect(
            atom.notifications.getNotifications()[0].getMessage()
          ).toContain('Cannot copy a folder into itself');
        }));
      ...
      describe('when attempting to paste a directory into a sibling directory that starts with the same letter', () =>
        it('allows the paste to occur', function () {
          ...
          return expect(
            atom.notifications.getNotifications()[0]
          ).toBeUndefined();
        }));
      ...
      describe("when pasting entries which don't exist anymore", () =>
        it("skips the entry which doesn't exist", function () {
          const filePathDoesntExist1 = path.join(
            dirPath2,
            'test-file-doesnt-exist1.txt'
          );
          const filePathDoesntExist2 = path.join(
            dirPath2,
            'test-file-doesnt-exist2.txt'
          );

          LocalStorage['tree-view:copyPath'] = JSON.stringify([
            filePath2,
            filePathDoesntExist1,
            filePath3,
            filePathDoesntExist2,
          ]);
          ...
          expect(
            fs.existsSync(path.join(dirPath, path.basename(filePath2)))
          ).toBeTruthy();
          expect(
            fs.existsSync(path.join(dirPath, path.basename(filePath3)))
          ).toBeTruthy();
          expect(
            fs.existsSync(
              path.join(dirPath, path.basename(filePathDoesntExist1))
            )
          ).toBeFalsy();
          expect(
            fs.existsSync(
              path.join(dirPath, path.basename(filePathDoesntExist2))
            )
          ).toBeFalsy();
      ...
      describe('when a file has been copied', function () {
        describe('when a file is selected', function () {
          ...
          return describe('when the target already exists', () =>
            it('appends a number to the destination name', function () {
              ...
              expect(
                fs.existsSync(
                  path.join(path.dirname(filePath), 'test-file0.txt')
                )
              ).toBeTruthy();
              expect(
                fs.existsSync(
                  path.join(path.dirname(filePath), 'test-file1.txt')
                )
              ).toBeTruthy();
              return expect(fs.existsSync(filePath)).toBeTruthy();
            }));
        ...
        describe('when a file containing two or more periods has been copied', () =>
          describe('when a file is selected', function () {
            it("creates a copy of the original file in the selected file's parent directory", function () {
              ...
              atom.commands.dispatch(treeView.element, 'tree-view:paste');
              expect(
                fs.existsSync(path.join(dirPath, path.basename(dotFilePath)))
              ).toBeTruthy();
            ...
            return describe('when the target already exists', () =>
              it('appends a number to the destination name', function () {
                ...
                expect(
                  fs.existsSync(path.join(dirPath, 'test0.file.txt'))
                ).toBeTruthy();
                expect(
                  fs.existsSync(path.join(dirPath, 'test1.file.txt'))
                ).toBeTruthy();
                return expect(fs.existsSync(dotFilePath)).toBeTruthy();
              }));
          }));
        ...
        describe('when a directory is selected', function () {
          it('creates a copy of the original file in the selected directory', function () {
            ...
            expect(
              fs.existsSync(path.join(dirPath2, path.basename(filePath)))
            ).toBeTruthy();
            return expect(fs.existsSync(filePath)).toBeTruthy();
          });
        ...
        describe('when a directory with a period is selected', function () {
          ...
          it('creates a copy of the original file in the selected directory', function () {
            ...
            const directories =
              treeView.roots[0].entries.querySelectorAll('.directory');
            const dotDirView = directories[directories.length - 1];
            dotDirView.dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 1 })
            );
            atom.commands.dispatch(treeView.element, 'tree-view:paste');

            expect(
              fs.existsSync(path.join(dotDirPath, path.basename(filePath)))
            ).toBeTruthy();
            return expect(fs.existsSync(filePath)).toBeTruthy();
          });
          ...
          return describe('when the target already exists', () =>
            it('appends a number to the destination file name', function () {
              ...
              expect(
                fs.existsSync(path.join(dotDirPath, 'test0.file.txt'))
              ).toBeTruthy();
              expect(
                fs.existsSync(path.join(dotDirPath, 'test1.file.txt'))
              ).toBeTruthy();
              return expect(fs.existsSync(dotFilePath)).toBeTruthy();
            }));
        });
        ...
        describe('when pasting into a different root directory', () =>
          it('creates the file', function () {
            ...
            atom.commands.dispatch(treeView.element, 'tree-view:paste');
            return expect(
              fs.existsSync(path.join(dirPath2, path.basename(filePath4)))
            ).toBeTruthy();
          }));

        return describe("when pasting a file with an asterisk char '*' in to different directory", () =>
          it('should successfully move the file', function () {
            ...
            return expect(
              fs.existsSync(
                path.join(dirPath2, path.basename(asteriskFilePath))
              )
            ).toBeTruthy();
          }));
      });
      ...
      describe('when multiple files have been copied', () =>
        describe('when a file is selected', function () {
          ...
          return describe('when the target destination file exists', () =>
            it('appends a number to the duplicate destination target names', function () {
              ...
              expect(
                fs.existsSync(path.join(dirPath, 'test-file20.txt'))
              ).toBeTruthy();
              return expect(
                fs.existsSync(path.join(dirPath, 'test-file30.txt'))
              ).toBeTruthy();
            }));
        }));
      ...
      describe('when a file has been cut', function () {
        ...
        describe('when a file is selected', function () {
          ...
          describe('when the target destination file exists', function () {
            ...
            describe('when selecting the replace option', () =>
              it('replaces the existing file', function () {
            ...
            return describe('when cancelling the dialog', () =>
              it('does not replace the existing file', function () {
        ...
        return describe('when a directory is selected', () =>
      ...
      describe('when multiple files have been cut', function () {
        describe('when a file is selected', function () {
          beforeEach(
            () =>
              (LocalStorage['tree-view:cutPath'] = JSON.stringify([
                filePath2,
                filePath3,
              ]))
          );
          ...
          it('emits events', function () {
            ...
            expect(callback).toHaveBeenCalledWith({
              initialPath: filePath2,
              newPath: newPath2,
            });
            return expect(callback).toHaveBeenCalledWith({
              initialPath: filePath3,
              newPath: newPath3,
            });
          });
          ...
          return describe('when the target destination file exists', function () {
            ...
            beforeEach(function () {
              ...
              return (LocalStorage['tree-view:cutPath'] = JSON.stringify([
                filePath5,
                filePath2,
                filePath3,
              ]));
      ...
      return describe('when pasting the file fails due to a filesystem error', () =>
        it('shows a notification', function () {
          ...
          expect(
            atom.notifications.getNotifications()[0].getMessage()
          ).toContain('Failed to copy entry');
          return expect(
            atom.notifications.getNotifications()[0].getDetail()
    ...
    describe('tree-view:add-file', function () {
      ...
      describe('when a file is selected', function () {
        it("opens an add dialog with the file's current directory path populated", function () {
          ...
          expect(addDialog.miniEditor.getText()).toBe(
            atom.project.relativize(dirPath) + path.sep
          );
          expect(addDialog.miniEditor.getCursorBufferPosition().column).toBe(
            addDialog.miniEditor.getText().length
          );
          return expect(addDialog.miniEditor.element).toHaveFocus();
        });

        describe('when the parent directory of the selected file changes', () =>
          it('still shows the active file as selected', function () {
            dirView.directory.emitter.emit(
              'did-remove-entries',
              new Map().set('deleted.txt', {})
            );
            return expect(
              treeView.element.querySelector('.selected').textContent
        ...
        describe(`when the path without a trailing '${path.sep}' is changed and confirmed`, function () {
          ...
          describe('when a file already exists at that location', () =>
            it('shows an error message and does not close the dialog', function () {
              ...
              expect(addDialog.errorMessage.textContent).toContain(
                'already exists'
              );
          ...
          return describe('when the project has no path', () =>
            it('adds a file and closes the dialog', function () {
              ...
              const newPath = path.join(
                fs.realpathSync(temp.mkdirSync()),
                'a-file'
              );
        ...
        describe("when the add dialog's editor loses focus", () =>
          it('removes the dialog and focuses root view', function () {
            ...
            expect(atom.workspace.getModalPanels().length).toBe(0);
            return expect(
              atom.views.getView(atom.workspace.getCenter().getActivePane())
            ).toHaveFocus();
          }));

        return describe('when the path ends with whitespace', () =>
          it('removes the trailing whitespace before creating the file', function () {
            const newPath = path.join(dirPath, 'new-test-file.txt');
            addDialog.miniEditor.insertText(path.basename(newPath) + '  ');
      ...
      describe('when a directory is selected', () =>
        it("opens an add dialog with the directory's path populated", function () {
          addDialog.cancel();
          dirView.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
      ...
      describe('when the root directory is selected', () =>
        it('opens an add dialog with no path populated', function () {
          addDialog.cancel();
          root1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
      ...
      return describe("when the project doesn't have a root directory", () =>
        it('shows an error', function () {
          ...
          atom.commands.dispatch(addDialog.element, 'core:confirm');
          return expect(addDialog.element.textContent).toContain(
        ...
        describe(`when the path without a trailing '${path.sep}' is changed and confirmed`, () =>
          describe('when no directory exists at the given path', () =>
            it('adds a directory and closes the dialog', function () {
              ...
              expect(document.activeElement).toBe(treeView.element);
              expect(
                dirView.querySelector('.directory.selected').textContent
              ).toBe('new');
              return expect(callback).toHaveBeenCalledWith({ path: newPath });
            })));
        ...
        return describe(`when the path with a trailing '${path.sep}' is changed and confirmed`, function () {
          describe('when no directory exists at the given path', function () {
            ...
            it('selects the created directory and does not change the expansion state of existing directories', function () {
              ...
              expect(
                treeView.entryForPath(expandedPath).isExpanded
              ).toBeTruthy();
              return expect(callback).toHaveBeenCalledWith({ path: newPath });
            });
          ...
          return describe('when a directory already exists at the given path', () =>
            it('shows an error message and does not close the dialog', function () {
              const newPath = path.join(dirPath, 'new-dir');
    ...
    describe('tree-view:move', function () {
      describe('when a file is selected', function () {
        ...
        it("opens a move dialog with the file's current path (excluding extension) populated", function () {
          ...
          expect(moveDialog.promptText.textContent).toBe(
            'Enter the new path for the file.'
          );
          expect(moveDialog.miniEditor.getText()).toBe(
            atom.project.relativize(filePath)
          );
          expect(moveDialog.miniEditor.getSelectedText()).toBe(
        ...
        describe('when the path is changed and confirmed', function () {
            ...
            it('moves the file, updates the tree view, closes the dialog, and emits an event', function () {
              ...
              waitsFor('tree view to update', function () {
                const files = Array.from(
                  root1.querySelectorAll('.entries .file')
                );
                return (
                  files.filter((f) => f.textContent === 'renamed-test-file.txt')
                    .length > 0
                );
              });
          ...
          describe("when the directories along the new path don't exist", () =>
            it('creates the target directory before moving the file', function () {
              const newPath = path.join(
                rootDirPath,
                'new',
                'directory',
                'renamed-test-file.txt'
              );
              ...
              waitsFor('tree view to update', function () {
                const directories = Array.from(
                  root1.querySelectorAll('.entries .directory')
                );
                return (
                  directories.filter((f) => f.textContent === 'new').length > 0
                );
              });
          ...
          describe('when a file or directory already exists at the target path', () =>
            it('shows an error message and does not close the dialog', function () {
              ...
              expect(moveDialog.errorMessage.textContent).toContain(
                'already exists'
              );
        ...
        return describe("when the move dialog's editor loses focus", () =>
          it('removes the dialog and focuses root view', function () {
            workspaceElement.focus();
            expect(atom.workspace.getModalPanels().length).toBe(0);
            return expect(
      ...
      describe("when a file is selected that's name starts with a '.'", function () {
        ...
        return it('selects the entire file name', function () {
          expect(moveDialog.element).toExist();
          expect(moveDialog.miniEditor.getText()).toBe(
            atom.project.relativize(dotFilePath)
          );
          return expect(moveDialog.miniEditor.getSelectedText()).toBe(
            '.dotfile'
          );
        });
      });
      ...
      describe('when a subdirectory is selected', function () {
        ...
        beforeEach(function () {
          ...
          return waitsForPromise(function () {
            ...
            return treeView.toggleFocus().then(function () {
              atom.commands.dispatch(treeView.element, 'tree-view:move');
              return (moveDialog = atom.workspace
                .getModalPanels()[0]
                .getItem());
            });
          });
        ...
        it("opens a move dialog with the folder's current path populated", function () {
          ...
          expect(moveDialog.miniEditor.getText()).toBe(
            atom.project.relativize(dirPath)
          );
          return expect(moveDialog.miniEditor.element).toHaveFocus();
        });
        ...
        return describe('when the path is changed and confirmed', function () {
          it('updates text editor paths accordingly', function () {
            ...
            return expect(editor.getPath()).toBe(
              filePath.replace('test-dir', 'renamed-dir')
            );
          });
      ...
      return describe('when the project is selected', () =>
        it("doesn't display the move dialog", function () {
          treeView.roots[0].dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          atom.commands.dispatch(treeView.element, 'tree-view:move');
    ...
    describe('tree-view:duplicate', function () {
      describe('when a file is selected', function () {
        ...
        it("opens a copy dialog to duplicate with the file's current path populated", function () {
          ...
          expect(copyDialog.promptText.textContent).toBe(
            'Enter the new path for the duplicate.'
          );
          expect(copyDialog.miniEditor.getText()).toBe(
            atom.project.relativize(filePath)
          );
          expect(copyDialog.miniEditor.getSelectedText()).toBe(
            fileNameWithoutExtension
          );
        ...
        describe('when the path is changed and confirmed', function () {
          describe('when all the directories along the new path exist', () =>
            it('duplicates the file, updates the tree view, opens the new file and closes the dialog', function () {
              const newPath = path.join(
                rootDirPath,
                'duplicated-test-file.txt'
              );
          ...
          describe("when the directories along the new path don't exist", () =>
            it('duplicates the tree and opens the new file', function () {
              const newPath = path.join(
                rootDirPath,
                'new',
                'directory',
          ...
          describe('when a file or directory already exists at the target path', () =>
            it('shows an error message and does not close the dialog', () =>
              runs(function () {
                ...
                expect(copyDialog.errorMessage.textContent).toContain(
                  'already exists'
                );
          ...
          return describe('when trying to duplicate a file with the same name but different case', () =>
            it('shows an error message and does not close the dialog', () =>
              runs(function () {
        ...
        return describe("when the duplicate dialog's editor loses focus", () =>
          it('removes the dialog and focuses root view', function () {
            workspaceElement.focus();
            expect(atom.workspace.getModalPanels().length).toBe(0);
            return expect(
      ...
      describe("when a file is selected that's name starts with a '.'", function () {
        ...
        return it('selects the entire file name', function () {
          ...
          return expect(copyDialog.miniEditor.getSelectedText()).toBe(
            '.dotfile'
          );
        });
      });
      ...
      describe('when the project is selected', () =>
        it("doesn't display the copy dialog", function () {
          treeView.roots[0].dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          atom.commands.dispatch(treeView.element, 'tree-view:duplicate');
      ...
      describe('when the editor has focus', function () {
        ...
        return it('duplicates the current file', () =>
    ...
    return describe('tree-view:remove', function () {
      ...
      it('shows a notification on failure', function () {
        ...
        waitsFor(
          'onDeleteEntryFailed to be called',
          () => callback.callCount === 1
        );

        return runs(function () {
          const notificationsNumber =
            atom.notifications.getNotifications().length;
      ...
      describe('when a directory is removed', function () {
        it('closes editors with filepaths belonging to the removed folder', function () {
          ...
          runs(function () {
            const openFilePaths = atom.workspace
              .getTextEditors()
      ...
      describe('when multiple files and folders are deleted', function () {
        it('does not error when the selected entries form a parent/child relationship', function () {
          ...
          waitsFor(
            'dir1/file1 to attempt to be deleted',
            () => fs.existsSync.mostRecentCall.args[0] === filePath
          );
        ...
        return it("focuses the first selected entry's parent folder", function () {
          ...
          waitsFor(
            'all selected entries to be deleted',
      ...
      return describe("when the entry is deleted before 'Move to Trash' is selected", () =>
  ...
  describe('file system events', function () {
    ...
    beforeEach(function () {
      ...
      return (temporaryFilePath = path.join(
        atom.project.getPaths()[0],
        'temporary'
      ));
    });

    return describe('when a file is added or removed in an expanded directory', () =>
      it("updates the directory view to display the directory's new contents", function () {
        let entriesCountBefore = null;

        runs(function () {
          expect(fs.existsSync(temporaryFilePath)).toBeFalsy();
          entriesCountBefore =
            treeView.roots[0].querySelectorAll('.entry').length;
  ...
  describe('project changes', function () {
    ...
    return describe('when a root folder is added', function () {
      it('maintains expanded folders', function () {
        ...
        return expect(
          treeView.roots[0].querySelector('.directory')
  ...
  describe('the hideVcsIgnoredFiles config option', function () {
    describe("when the project's path is the repository's working directory", function () {
      ...
      it('hides git-ignored files if the option is set, but otherwise shows them', function () {
        expect(
          Array.from(treeView.element.querySelectorAll('.file')).map(
            (f) => f.textContent
          )
        ).toEqual(['.gitignore', 'ignored.txt']);
    ...
    return describe("when the project's path is a subfolder of the repository's working directory", function () {
      ...
      return it('does not hide git ignored files', () =>
        expect(
          Array.from(treeView.element.querySelectorAll('.file')).map(
            (f) => f.textContent
          )
        ).toEqual(['.gitignore', 'tree-view.js', 'tree-view.txt']));
    });
  });

  describe('the hideIgnoredNames config option', () =>
    it('hides ignored files if the option is set, but otherwise shows them', function () {
      ...
      expect(
        Array.from(treeView.roots[0].querySelectorAll('.entry')).map(
          (e) => e.textContent
        )
      ).toEqual(['.git', 'test.js', 'test.txt']);

      // Add a unique class to the names even when showing them
      expect(
        Array.from(
          treeView.roots[0].querySelectorAll('.status-ignored-name')
        ).map((e) => e.textContent)
      ).toEqual(['test.js']);
  ...
  describe('the squashedDirectoryName config option', function () {
    ...
    return describe('when enabled', function () {
      ...
      it('does not squash root directories', function () {
        ...
        const zetaDirPath = findDirectoryContainingText(
          treeView.roots[0],
          'zeta'
        ).getPath();
      ...
      it('does not squash a file in to a DirectoryViews', function () {
        ...
        zetaDir.expand();
        const zetaEntries = [].slice
          .call(zetaDir.children[1].children)
      ...
      it('squashes two dir names when the first only contains a single dir', function () {
        const betaDir = findDirectoryContainingText(
          treeView.roots[0],
          `alpha${path.sep}beta`
        );
        betaDir.expand();
        const betaEntries = [].slice
          .call(betaDir.children[1].children)
          .map((element) => element.innerText);
      ...
      it('squashes three dir names when the first and second only contain single dirs', function () {
        const epsilonDir = findDirectoryContainingText(
          treeView.roots[0],
          `gamma${path.sep}delta${path.sep}epsilon`
        );
        epsilonDir.expand();
        const epsilonEntries = [].slice
          .call(epsilonDir.children[1].children)
          .map((element) => element.innerText);
      ...
      it('does not squash a dir name when there are two child dirs ', function () {
        const lambdaDir = findDirectoryContainingText(
          treeView.roots[0],
          'lambda'
        );
        lambdaDir.expand();
        const lambdaEntries = [].slice
          .call(lambdaDir.children[1].children)
          .map((element) => element.innerText);
      ...
      describe('when a squashed directory is deleted', () =>
        it('un-squashes the directories', function () {
          ...
          const piDir = findDirectoryContainingText(
            treeView.roots[0],
            `omicron${path.sep}pi`
          );
          treeView.focus();
          ...
          return runs(function () {
            const omicronDir = findDirectoryContainingText(
              treeView.roots[0],
              'omicron'
            );
      ...
      describe('when a file is created within a directory with another squashed directory', () =>
        it('un-squashes the directories', function () {
          jasmine.attachToDOM(workspaceElement);
          let piDir = findDirectoryContainingText(
            treeView.roots[0],
            `omicron${path.sep}pi`
          );
      ...
      return describe('when a directory is reloaded', () =>
        it('squashes the directory names the last of which is same as an unsquashed directory', function () {
          ...
          muDir.expand();
          let muEntries = Array.from(muDir.children[1].children).map(
          ...
          muDir.expand();
          muDir.reload();
          muEntries = Array.from(muDir.children[1].children).map(
            (element) => element.innerText
          );
  ...
  describe('Git status decorations', function () {
    ...
    beforeEach(function () {
      ...
      const workingDirFixture = path.join(
        __dirname,
        'fixtures',
        'git',
        'working-dir'
      ...
      treeView.updateRoots();
      return treeView.roots[0].entries
        .querySelectorAll('.directory')[1]
        .expand();
    });

    describe('when the project is the repository root', () =>
      it('adds a custom style', () =>
        expect(treeView.element.querySelectorAll('.icon-repo').length).toBe(
          1
        )));
    ...
    describe('when a file is modified', () =>
      it('adds a custom style', () =>
        expect(
          treeView.element.querySelector('.project-root .file.status-modified')
        ).toHaveText('b.txt')));

    describe('when a file is modified', () =>
      it('adds a custom style to the project root', () =>
        expect(treeView.element.querySelector('.project-root')).toHaveClass(
          'status-modified'
        )));
      ...
      it('adds a custom style', () =>
        expect(
          treeView.element.querySelector(
            '.project-root .directory.status-modified'
          ).header
        ).toHaveText('dir')));
    ...
    describe('when a file is new', () =>
      it('adds a custom style', function () {
        treeView.roots[0].entries.querySelectorAll('.directory')[2].expand();
        return expect(
          treeView.element.querySelector('.project-root .file.status-added')
        ).toHaveText('new2');
      }));
    ...
    describe('when a directory is new', () =>
      it('adds a custom style', () =>
        expect(
          treeView.element.querySelector(
            '.project-root .directory.status-added'
          ).header
        ).toHaveText('dir2')));
    ...
    describe('when a file is ignored', () =>
      it('adds a custom style', () =>
        expect(
          treeView.element.querySelector('.project-root .file.status-ignored')
        ).toHaveText('ignored.txt')));
    ...
    describe('when a file is selected in a directory', function () {
      beforeEach(function () {
        ...
        return fileView.dispatchEvent(
          new MouseEvent('click', { bubbles: true, detail: 1 })
        );
      });

      return describe('when the file is deleted', () =>
        it('updates the style of the directory', function () {
          const callback = jasmine.createSpy('onEntryDeleted');
          treeView.onEntryDeleted(callback);

          const pathToDelete = treeView.selectedEntry().getPath();
          expect(treeView.selectedEntry().getPath()).toContain(
            path.join('dir2', 'new2')
          );
          const dirView = findDirectoryContainingText(
            treeView.roots[0],
            'dir2'
          );
    ...
    return describe('on #darwin, when the project is a symbolic link to the repository root', function () {
      beforeEach(function () {
        ...
        return waitsFor(function (done) {
          let disposable;
          return (disposable = atom.project
            .getRepositories()[0]
            .onDidChangeStatuses(function () {
              disposable.dispose();
              return done();
            }));
        });
      ...
      describe('when a file is modified', () =>
        it("updates its and its parent directories' styles", function () {
          expect(
            treeView.element.querySelector(
              '.project-root .file.status-modified'
            )
          ).toHaveText('b.txt');
          expect(
            treeView.element.querySelector(
          ...
          return expect(
            treeView.element.querySelector('.project-root')
          ).toHaveClass('status-modified');
        }));

      return describe('when a file loses its modified status', () =>
        it("updates its and its parent directories' styles", function () {
          ...
          return expect(
            treeView.element.querySelector('.project-root.status-modified')
          ).not.toExist();
        }));
    });
  });
  ...
  describe('selecting items', function () {
    ...
    beforeEach(function () {
      ...
      dirView.expand();
      return (
        ([fileView1, fileView2, fileView3, fileView4, fileView5] = Array.from(
          (ref = dirView.querySelectorAll('.file'))
        )),
        ref
      );
    });
    ...
    return describe('selecting multiple items', function () {
      ...
      describe('using the shift key', () =>
        it('selects the items between the already selected item and the shift clicked item', function () {
          fileView1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          fileView3.dispatchEvent(
            new MouseEvent('mousedown', { bubbles: true, shiftKey: true })
          );
      ...
      describe('using the metakey(cmd) key', () =>
        it('selects the cmd-clicked item in addition to the original selected item', function () {
          fileView1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          fileView3.dispatchEvent(
      ...
      describe('using the metakey(cmd) key on already selected item', () =>
        it('deselects just the cmd-clicked item', function () {
          fileView1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          fileView3.dispatchEvent(
          ...
          fileView1.dispatchEvent(
            new MouseEvent('mouseup', { bubbles: true, metaKey: true })
          );
      ...
      describe('using the shift and metakey(cmd) keys', () =>
        it('selects the items between the last cmd-clicked item and the clicked item', function () {
          fileView1.dispatchEvent(
            new MouseEvent('click', { bubbles: true, detail: 1 })
          );
          fileView3.dispatchEvent(
      ...
      describe('non-darwin platform', function () {
        ...
        return describe('using the ctrl key', () =>
          it('selects the ctrl-clicked item in addition to the original selected item', function () {
            fileView1.dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 1 })
            );
            fileView3.dispatchEvent(
      ...
      return describe('darwin platform', function () {
        ...
        return describe('right-clicking', function () {
          describe('when multiple items are selected', () =>
            it('displays the multi-select context menu', function () {
              fileView1.dispatchEvent(
                new MouseEvent('click', { bubbles: true, detail: 1 })
              );
              fileView3.dispatchEvent(
  ...
  describe('the sortFoldersBeforeFiles config option', function () {
    ...
    it('lists folders first if the option is set', function () {
      ...
      const topLevelEntries = [].slice
        .call(treeView.roots[0].entries.children)
        .map((element) => element.innerText);
      ...
      alphaDir.expand();
      const alphaEntries = [].slice
        .call(alphaDir.children[1].children)
        .map((element) => element.innerText);
      ...
      gammaDir.expand();
      const gammaEntries = [].slice
        .call(gammaDir.children[1].children)
        .map((element) => element.innerText);

      return expect(gammaEntries).toEqual([
        'theta',
        'delta.txt',
        'epsilon.txt',
      ]);
  ...
  describe('when reloading a directory with deletions and additions', () =>
    it('does not throw an error (regression)', function () {
      const projectPath = temp.mkdirSync('atom-project');
      ...
      treeView.roots[0].reload();
      return expect(
        treeView.roots[0].directory.serializeExpansionState()
      ).toEqual({
        isExpanded: true,
  ...
  describe('Dragging and dropping files', function () {
    ...
    describe("when dragging a FileView onto a DirectoryView's header", () =>
      it('should add the selected class to the DirectoryView', function () {
        // Dragging delta.txt onto alphaDir
        const alphaDir = findDirectoryContainingText(
          treeView.roots[0],
          'alpha'
        );
        alphaDir.expand();

        const gammaDir = findDirectoryContainingText(
          treeView.roots[0],
          'gamma'
        );
        gammaDir.expand();
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] = Array.from(
    ...
    describe("when dropping a FileView onto a DirectoryView's header", function () {
      it('should move the file to the hovered directory', function () {
        ...
        const alphaDirContents = findDirectoryContainingText(
          treeView.roots[0],
          'alpha'
        ).querySelectorAll('.entry').length;
        const gammaDirContents = findDirectoryContainingText(
          treeView.roots[0],
          'gamma'
        ).querySelectorAll('.entry').length;
        ...
        waitsFor(
          ...
          () =>
            ...
            findDirectoryContainingText(
              treeView.roots[0],
              'gamma'
            ).querySelectorAll('.entry').length < gammaDirContents
        );
        ...
        return runs(function () {
          ...
          ).toBe(alphaDirContents + 1);
          return expect(
            findDirectoryContainingText(
              treeView.roots[0],
              'gamma'
            ).querySelectorAll('.entry').length
          ).toBe(gammaDirContents - 1);
        });
      });

      describe('when the ctrl/cmd modifier key is pressed', () =>
        it('should copy the file to the hovered directory', function () {
          // Dragging delta.txt onto alphaDir
          const alphaDir = findDirectoryContainingText(
            treeView.roots[0],
            'alpha'
      ...
      return it("shouldn't update editors with similar file paths", function () {
        ...
        return runs(function () {
          ...
          const editors = atom.workspace.getTextEditors();
          expect(editors[0].getPath()).toBe(
            deltaFilePath.replace('gamma', 'alpha')
          );
    ...
    describe("when dropping multiple FileViews onto a DirectoryView's header", () =>
      it('should move the files to the hovered directory', function () {
        ...
        return runs(function () {
          ...
          ).toBe(alphaDirContents + 2);
          return expect(
            findDirectoryContainingText(
              treeView.roots[0],
              'gamma'
            ).querySelectorAll('.entry').length
          ).toBe(gammaDirContents - 2);
        });
      }));

    describe("when dropping a DirectoryView and FileViews onto a DirectoryView's header", () =>
      it('should move the files and directory to the hovered directory', function () {
        // Dragging alpha.txt and alphaDir into thetaDir
        const alphaFile = Array.from(treeView.roots[0].entries.children).find(
          (element) => element.getPath() === alphaFilePath
        );
        ...
        gammaDir.expand();
        const thetaDir = findDirectoryContainingText(
          treeView.roots[0],
          'theta'
        );
        thetaDir.expand();
        ...
        const thetaDirContents = findDirectoryContainingText(
          treeView.roots[0],
          'theta'
        ).querySelectorAll('.entry').length;
        ...
        waitsFor(
          ...
          () =>
            findDirectoryContainingText(
              treeView.roots[0],
              'theta'
            ).querySelectorAll('.entry').length > thetaDirContents
        );

        return runs(function () {
          thetaDir.expand();
          expect(thetaDir.querySelectorAll('.entry').length).toBe(
            thetaDirContents + 2
          );
          // alpha dir still has all its entries
          alphaDir = findDirectoryContainingText(thetaDir.entries, 'alpha');
          alphaDir.expand();
          return expect(alphaDir.querySelectorAll('.entry').length).toBe(
            alphaDirContents
          );
        });
      }));
    ...
    describe("when dropping a DirectoryView onto a DirectoryView's header", function () {
      ...
      it('should move the directory to the hovered directory', function () {
        ...
        return runs(function () {
          ...
          return expect(editor.getPath()).toBe(
            thetaFilePath.replace('gamma', 'alpha')
          );
        });
      });
      ...
      it('shows a warning notification and does not move the directory if it would result in recursive copying', function () {
        ...
        return expect(
          atom.notifications.getNotifications()[0].getMessage()
        ).toContain('Cannot move a folder into itself');
      });
      ...
      it('shows a warning notification and does not move the directory if it would result in recursive copying (symlink)', function () {
        ...
        const [dragStartEvent, dragEnterEvent, dropEvent] = Array.from(
          eventHelpers.buildInternalDragEvents(
            [alphaDir],
            symlinkDir.querySelector('.header'),
            symlinkDir,
            treeView
          )
        );
      ...
      it('moves successfully when dragging a directory onto a sibling directory that starts with the same letter', function () {
        ...
        const alpha2Dir = findDirectoryContainingText(
          treeView.roots[0],
          'alpha2'
        );
        const [dragStartEvent, dragEnterEvent, dropEvent] = Array.from(
          eventHelpers.buildInternalDragEvents(
            [alphaDir],
            alpha2Dir.querySelector('.header'),
            alpha2Dir,
            treeView
          )
        );
    ...
    describe('when dropping a DirectoryView and FileViews in the same parent DirectoryView', () =>
      describe('when the ctrl/cmd modifier key is pressed', () =>
        it('should copy the files and directory', function () {
          ...
          return runs(function () {
            ...
            expect(fs.existsSync(path.join(alphaDirPath, 'beta0.txt'))).toBe(
              true
            );
            return expect(fs.existsSync(path.join(alphaDirPath, 'eta0'))).toBe(
              true
            );
          });
        })));
    ...
    describe("when dragging a file from the OS onto a DirectoryView's header", function () {
      ...
      return describe('when the ctrl/cmd modifier key is pressed', () =>
        it('should copy the file to the hovered directory', function () {
          // Dragging delta.txt from OS file explorer onto alphaDir
          const alphaDir = findDirectoryContainingText(
            treeView.roots[0],
    ...
    describe("when dragging a file and directory from the OS onto a DirectoryView's header", () =>
      it('should move the file and directory to the hovered directory', function () {
    ...
    describe('when dragging a directory from the OS onto a blank section of the Tree View', () =>
      it('should create a new project folder', function () {
        ...
        waitsFor(
          'project folder to be added',
          () => treeView.roots.length === 2
        );

        return runs(() =>
          expect(treeView.roots[1].querySelector('.header .name')).toHaveText(
            'gamma'
          )
        );
      }));
    ...
    describe('when dragging a file from the OS onto a blank section of the Tree View', () =>
      it("should create a new project folder using the file's parent directory", function () {
        ...
        const dropEvent = eventHelpers.buildExternalDropEvent(
          [
            deltaFilePath,
            epsilonFilePath, // directly under gammaDir
            alphaDirPath,
            betaFilePath,
            etaDirPath, // betaFile and etaDir directly under alphaDir
          ],
          treeView.element
        );
        treeView.onDrop(dropEvent);
        ...
        waitsFor(
          'project folder to be added',
          () => treeView.roots.length === 4
        );

        return runs(function () {
          // Adding project folders is async - don't rely on a specific order
          const names = treeView.roots.map(
            (root) => root.querySelector('.header .name').innerText
          );
    ...
    describe('when dragging entries that already exist', function () {
      ...
      describe('when dragging a single file', function () {
        ...
        beforeEach(function () {
          ...
          gammaDir.expand();
          const deltaFile = findFileContainingText(
            treeView.roots[0],
            'delta.txt'
          );

          return (
            ([dragStartEvent, dragEnterEvent, dropEvent] = Array.from(
              (ref = eventHelpers.buildInternalDragEvents(
                [deltaFile],
                alphaDir.querySelector('.header'),
                alphaDir,
                treeView
        ...
        describe('when selecting the replace option', () =>
          it('replaces the existing file', function () {
            ...
            treeView.onDragStart(dragStartEvent);
            treeView.onDrop(dropEvent);
            return expect(fs.readFileSync(deltaAlphaFilePath, 'utf8')).toBe(
              "doesn't matter"
            );
          }));
      ...
      describe('when dragging multiple files', function () {
        ...
        beforeEach(function () {
          ...
          return (dropEvent = eventHelpers.buildExternalDropEvent(
      ...
      return describe('when dragging directories', function () {
        ...
        it('recursively walks the directory structure and prompts to replace each conflicting file', function () {
          ...
          return expect(atom.confirm.calls[3].args[0].message).toContain(
            'nested.txt'
          );
        });
        ...
        return it('moves everything as expected', function () {
          ...
          expect(
            fs.readFileSync(
              path.join(alphaDirPath, 'new', 'no-conflict.txt'),
              'utf8'
            )
          ).toBe('neither');
    ...
    return describe('when the event does not originate from the Tree View', () =>
      it('does nothing', function () {
        const alphaDir = findDirectoryContainingText(
          treeView.roots[0],
          'alpha'
  ...
  describe('the alwaysOpenExisting config option', function () {
    ...
    describe('when a file is single-clicked', function () {
      ...
      return it('selects the files and opens it in the active editor, without changing focus', function () {
        ...
        runs(function () {
          expect(sampleJs).toHaveClass('selected');
          expect(atom.workspace.getCenter().getActivePaneItem().getPath()).toBe(
            atom.project.getDirectories()[0].resolve('tree-view.js')
          );
          return expect(treeView.element).toHaveFocus();
        });
      ...
      describe('when a file that is already open in other pane is single-clicked', function () {
        ...
        return describe('when core.allowPendingPaneItems is set to true (default)', function () {
          ...
          return it("doesn't open the file in the active pane", function () {
            ...
            return expect(activePaneItem.getPath()).toBe(
              atom.project.getDirectories()[0].resolve('tree-view.js')
            );
          });
        });
      ...
      return describe('when a file is double-clicked', function () {
        ...
        beforeEach(function () {
          ...
          waitForWorkspaceOpenEvent(function () {
            ...
            return sampleTxt.dispatchEvent(
              new MouseEvent('click', { bubbles: true, detail: 2 })
            );
          });
        ...
        return it('opens the file and focuses it', function () {
          ...
          return expect(
            atom.views.getView(atom.workspace.getCenter().getPanes()[1])
          ).toHaveFocus();
        });
      });
    });
  ...
  describe('Dragging and dropping root folders', function () {
    ...
    afterEach(function () {
      let ref;
      return (
        ([alphaDirPath, gammaDirPath, thetaDirPath, etaDirPath] = Array.from(
          (ref = [])
        )),
        ref
      );
    ...
    describe("when dragging a project root's header onto a different project root", function () {
      describe('when dragging on the top part of the root', () =>
        it('should add the placeholder above the directory', function () {
          ...
          const [dragStartEvent, dragOverEvents, dragEndEvent] = Array.from(
      ...
      return describe('when below all entries', () =>
    ...
    describe("when dropping a project root's header onto a different project root", function () {
      describe('when dropping on the top part of the header', () =>
        it('should add the placeholder above the directory', function () {
          ...
          const alphaDir = treeView.roots[0];
          const gammaDir = treeView.roots[1];
          const [dragStartEvent, dragDropEvents] = Array.from(
            eventHelpers.buildPositionalDragEvents(
              gammaDir.querySelector('.project-root-header'),
              alphaDir,
              '.tree-view'
      ...
      return describe('when dropping on the bottom part of the header', () =>
        it('should add the placeholder below the directory', function () {
          ...
          const [dragStartEvent, dragDropEvents] = Array.from(
            eventHelpers.buildPositionalDragEvents(
              thetaDir.querySelector('.project-root-header'),
              alphaDir,
              '.tree-view'
            )
          );
    ...
    describe('when a root folder is dragged out of application', () =>
      it("should carry the folder's information", function () {
        ...
        expect(dragStartEvent.dataTransfer.getData('text/plain')).toEqual(
          gammaDirPath
        );
        if (['darwin', 'linux'].includes(process.platform)) {
          return expect(
            dragStartEvent.dataTransfer.getData('text/uri-list')
          ).toEqual(`file://${gammaDirPath}`);
        }
      }));

    describe('when a root folder is dropped from another Atom window', () =>
      it('adds the root folder to the window', function () {
        let dragDropEvents;
        const alphaDir = treeView.roots[0];
        [_, dragDropEvents] = Array.from(
          eventHelpers.buildPositionalDragEvents(
            null,
            alphaDir.querySelector('.project-root-header'),
            '.tree-view'
        ...
        dropEvent.dataTransfer.setData(
          'from-window-id',
          treeView.rootDragAndDrop.getWindowId() + 1
        );
    ...
    describe('when a root folder is dropped to another Atom window', () =>
      it('removes the root folder from the first window', function () {
        ...
        treeView.rootDragAndDrop.onDragStart(dragStartEvent);
        treeView.rootDragAndDrop.onDropOnOtherWindow(
          {},
          Array.from(gammaDir.parentElement.children).indexOf(gammaDir)
        );
  ...
  describe('when the active file path does not exist in the project', () =>
    it('deselects all entries', function () {
    ...
    it('converts legacy expansion serialization Objects to Maps', function () {
      ...
      const convertedState = {
        ...
        entries: new Map().set('a', { isExpanded: true }).set('tree-view', {
          isExpanded: false,
          entries: new Map().set('sub-folder', { isExpanded: true }),
        }),
      };
      ...
      const directory = new Directory({
        ...
        fullPath: 'path',
        symlink: false,
        expansionState: legacyState,
      });
      expect(directory.expansionState.entries instanceof Map).toBe(true);

      var assertEntriesDeepEqual = (expansionEntries, convertedEntries) =>
        expansionEntries.forEach(function (entry, name) {
          if (
            entry.entries != null ||
            convertedEntries.get(name).entries != null
          ) {
            assertEntriesDeepEqual(
              entry.entries,
              convertedEntries.get(name).entries
            );
          }
          return expect(entry).toEqual(convertedEntries.get(name));
        });

      return assertEntriesDeepEqual(
        directory.expansionState.entries,
        convertedState.entries
      );
    }));

  var findDirectoryContainingText = function (element, text) {
    const directories = Array.from(
      element.querySelectorAll('.entries .directory')
    );
    return directories.find(
      (directory) => directory.header.textContent === text
    );
  };
...
describe('Service provider', function () {
  ...
  beforeEach(function () {
    ...
    return runs(function () {
      treeView = atom.workspace.getLeftDock().getActivePaneItem();
      return (treeViewService = atom.packages
        .getActivePackage('tree-view')
        .mainModule.provideTreeView());
    });
  });

  it('provides the `selectedPaths` method which should return the selected paths in the Tree View', () =>
    expect(treeViewService.selectedPaths()).toEqual([
      atom.project.getPaths()[0],
    ]));
  ...
  return it('provides the `entryForPath` method which should return the Tree View entry for a given path', function () {
    ...
    return expect(treeViewService.entryForPath(root)).toEqual(
      treeView.roots[0]
    );
  });
});
...
describe('Icon class handling', () =>
  it('allows multiple classes to be passed', function () {
    ...
    const providerDisposable = atom.packages.serviceHub.provide(
      'atom.file-icons',
      '1.0.0',
      {
    ...
    return runs(function () {
      ...
      expect(files[0].fileName.className).toBe(
        'name icon first-icon-class second-icon-class'
      );
      expect(files[1].fileName.className).toBe(
        'name icon third-icon-class fourth-icon-class'
      );

      providerDisposable.dispose();

      files = workspaceElement.querySelectorAll('li[is="tree-view-file"]');
      return expect(files[0].fileName.className).toBe(
        'name icon icon-file-text'
      );
    });
  }));
...
describe('Hidden on startup', function () {
  ...
  return describe('When set to false', () =>
    it('allows the pane to show up as normal', function () {
      waitsForPromise(() =>
        // First deactivate the package so that we can start from scratch
        atom.packages.deactivatePackage('tree-view')
      );
