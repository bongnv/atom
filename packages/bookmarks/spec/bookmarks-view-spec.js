describe('Bookmarks package', () => {
  ...
  const bookmarkedRangesForEditor = (editor) => {
    const decorationsById = editor.decorationsStateForScreenRowRange(
      0,
      editor.getLastScreenRow()
    );
    const decorations = Object.keys(decorationsById).map(
      (key) => decorationsById[key]
    );
    return decorations
  ...
  describe('toggling bookmarks', () => {
    ...
    it('toggles proper classes on proper gutter, line row and highlight on point bookmark', () => {
      ...
      expect(
        editorElement.querySelectorAll('.highlight.bookmarked').length
      ).toBe(0);
      expect(editorElement.querySelectorAll('.line.bookmarked').length).toBe(1);
      expect(
        editorElement.querySelectorAll('.line-number.bookmarked').length
      ).toBe(1);
  ...
  describe('browsing bookmarks', () => {
    it('displays a select list of all bookmarks', async () => {
      ...
      expect(bookmarks[0].querySelector('.primary-line').textContent).toBe(
        'sample.js:1'
      );
      expect(bookmarks[0].querySelector('.secondary-line').textContent).toBe(
        'var quicksort = function () {'
      );
      expect(bookmarks[1].querySelector('.primary-line').textContent).toBe(
        'sample.js:3'
      );
      expect(bookmarks[1].querySelector('.secondary-line').textContent).toBe(
        'if (items.length <= 1) return items;'
      );
      expect(bookmarks[2].querySelector('.primary-line').textContent).toBe(
        'sample.js:5'
      );
    ...
    describe('when a bookmark is selected', () => {
      ...
      it('sets the cursor to the location of the bookmark and activates the right editor', async () => {
        ...
        const bookmarkElement = workspaceElement.querySelector(
