...
module.exports = class Bookmarks {
  ...
  constructor(editor, markerLayer) {
    this.editor = editor;
    this.markerLayer =
      markerLayer || this.editor.addMarkerLayer({ persistent: true });
    ...
    this.disposables.add(
      atom.commands.add(atom.views.getView(this.editor), {
        ...
        'bookmarks:jump-to-previous-bookmark':
          this.jumpToPreviousBookmark.bind(this),
        'bookmarks:select-to-next-bookmark':
          this.selectToNextBookmark.bind(this),
        'bookmarks:select-to-previous-bookmark':
          this.selectToPreviousBookmark.bind(this),
        'bookmarks:clear-bookmarks': this.clearBookmarks.bind(this),
      })
    );
    this.decorationLayerLine = this.editor.decorateMarkerLayer(
      this.markerLayer,
      { type: 'line', class: 'bookmarked' }
    );
    this.decorationLayerHighlight = this.editor.decorateMarkerLayer(
      this.markerLayer,
      { type: 'highlight', class: 'bookmarked' }
    );
  ...
  jumpToNextBookmark() {
    if (this.markerLayer.getMarkerCount() > 0) {
      ...
      const bookmarkMarker =
        markers.find(
          (marker) => marker.getBufferRange().start.row > bufferRow
        ) || markers[0];
    ...
    if (this.markerLayer.getMarkerCount() > 0) {
      ...
      if (!bookmarkMarker) {
        ...
        this.editor.setSelectedBufferRange(
          [
            bookmarkMarker.getHeadBufferPosition(),
