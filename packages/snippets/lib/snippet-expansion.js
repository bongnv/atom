...
module.exports = class SnippetExpansion {
        ...
        this.editor.transact(() => {
          ...
          if (this.snippet.tabStopList.length > 0) {
            ...
            this.subscriptions.add(
              this.cursor.onDidChangePosition((event) =>
  ...
  cursorMoved({ oldBufferPosition, newBufferPosition, textChanged }) {
    ...
    const insertionAtCursor = this.insertionsByIndex[this.tabStopIndex].find(
      (insertion) => {
  ...
  applyTransformations(tabStopIndex) {
    ...
    const primaryRange = this.markersForInsertions
      .get(primaryInsertion)
      .getBufferRange();
    ...
    for (let [index, tabStop] of tabStops.entries()) {
      ...
      for (const insertion of insertions) {
        ...
        if (insertion.references) {
          references = insertion.references.map(
            (external) => indexTable[external]
          );
        }
        ...
        const shouldBeInclusive =
          index === 0 || (references && references.includes(0));
        ...
        if (references) {
          const relatedInsertions =
            this.relatedInsertionsByIndex.get(index) || [];
  ...
  replaceMarkerForInsertion(insertion, settings) {
    ...
    const replacement = this.getMarkerLayer(this.editor).markBufferRange(
      range,
      settings
    );
  ...
  goToEndOfLastTabStop() {
    ...
    const lastMarker = this.markersForInsertions.get(
