...
const HAS_NEW_TEXT_BUFFER_VERSION = new TextBuffer().getLanguageMode()
  ...
  describe('matching bracket highlighting', () => {
    ...
    function expectNoHighlights() {
      const decorations = editor
        ...
        .filter(
          (decoration) => decoration.properties.class === 'bracket-matcher'
        );
    ...
    function expectHighlights(startBufferPosition, endBufferPosition) {
      ...
      const gutterDecorations = editor
        ...
        .filter(
          (gutterDecoration) =>
            gutterDecoration.properties.class === 'bracket-matcher'
        );
      ...
      expect(decorations[0].marker.getStartBufferPosition()).toEqual(
        startBufferPosition
      );
      expect(decorations[1].marker.getStartBufferPosition()).toEqual(
        endBufferPosition
      );

      expect(gutterDecorations[0].marker.getStartBufferPosition()).toEqual(
        startBufferPosition
      );
      expect(gutterDecorations[1].marker.getStartBufferPosition()).toEqual(
        endBufferPosition
      );
    }
  ...
  describe('when bracket-matcher:remove-matching-brackets is triggered', () => {
    describe('when the cursor is not in front of any pair', () => {
      it('performs a regular backspace action', () => {
        ...
        expect(editor.lineTextForBufferRow(0)).toEqual(
          'ar quicksort = function () {'
        );
