...
module.exports = class BracketMatcher {
  ...
  insertText(text, options) {
    if (!text) return true;
    if ((options && options.select) || (options && options.undo === 'skip'))
      return true;
    ...
    const hasQuoteBeforeCursor =
      this.isQuote(previousCharacter) && previousCharacter === text[0];
    const hasEscapeCharacterBeforeCursor =
      endsWithEscapeCharacter(previousCharacters);
    const hasEscapeSequenceBeforeCursor =
      endsWithEscapeSequence(previousCharacters);
    ...
    if (
      ...
      bracketMarker = this.bracketMarkers.find(
        (marker) =>
          marker.isValid() &&
          marker.getBufferRange().end.isEqual(cursorBufferPosition)
      );
      if (
        bracketMarker ||
        this.getScopedSetting('bracket-matcher.alwaysSkipClosingPairs')
      ) {
    ...
    if (skipOverExistingClosingBracket) {
      ...
      const range = [
        cursorBufferPosition,
        cursorBufferPosition.traverse([0, text.length]),
      ];
  ...
  insertNewline() {
    ...
    if (
      this.matchManager.pairsWithExtraNewline[previousCharacter] ===
        nextCharacter &&
      !hasEscapeCharacterBeforeCursor
    ) {
      this.editor.transact(() => {
  ...
  isCursorOnInterpolatedString() {
    ...
    if (languageMode.getSyntaxNodeAtPosition) {
      const node = languageMode.getSyntaxNodeAtPosition(
        ...
        (node, grammar) =>
          grammar.scopeName === 'source.ruby' && /string|symbol/.test(node.type)
      );
      if (node) {
      ...
      if (this.interpolatedStringSelector == null) {
        ...
        this.interpolatedStringSelector = SelectorCache.get(
          segments.join(' | ')
        );
      }
