...
const generateTagStartOrEndRegex = function (tagNameRegexStr) {
  ...
  return new RegExp(
    `<(${tagNameRegexStr})${notSelfClosingTagEnd}|<\\/(${tagNameRegexStr})>`
  );
};

const tagStartOrEndRegex = generateTagStartOrEndRegex(
  '\\w[-\\w]*(?:(?:\\:|\\.)\\w[-\\w]*)*'
);
...
module.exports = class TagFinder {
  ...
  findEndTag(tagName, startPosition, fullRange = false) {
    const scanRange = new Range(
      startPosition,
      this.editor.buffer.getEndPosition()
    );
    const pattern = this.patternForTagName(tagName);
    let endRange = null;
    let unpairedCount = 0;
