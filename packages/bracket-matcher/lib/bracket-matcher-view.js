...
module.exports = class BracketMatcherView {
  ...
  updateMatch() {
    ...
    if (position && matchPosition) {
      ...
      if (this.hasSyntaxTree()) {
        ({ startRange, endRange } =
          this.findMatchingTagNameRangesWithSyntaxTree());
      } else {
  ...
  selectMatchingBrackets() {
    ...
    this.editor.setSelectedBufferRanges([
      this.bracket1Range,
      this.bracket2Range,
    ]);
  ...
  findMatchingEndBracket(startBracketPosition, startBracket, endBracket) {
    ...
    if (this.hasSyntaxTree()) {
      ...
      const scopeDescriptor =
        this.editor.scopeDescriptorForBufferPosition(startBracketPosition);
      if (this.isScopeCommentedOrString(scopeDescriptor.getScopesArray()))
        return;
  ...
  findMatchingStartBracket(endBracketPosition, startBracket, endBracket) {
    ...
    if (this.hasSyntaxTree()) {
      ...
      const scopeDescriptor =
        this.editor.scopeDescriptorForBufferPosition(endBracketPosition);
      if (this.isScopeCommentedOrString(scopeDescriptor.getScopesArray()))
        return;
  ...
  findMatchingEndBracketWithSyntaxTree(
    bracketPosition,
    startBracket,
    endBracket
  ) {
    let result;
    const bracketEndPosition = bracketPosition.traverse([
      0,
      startBracket.length,
    ]);
    this.editor.buffer
  ...
  findMatchingStartBracketWithSyntaxTree(
    bracketPosition,
    startBracket,
    endBracket
  ) {
  ...
  findMatchingTagNameRangesWithSyntaxTree() {
    ...
    const { startTag, endTag } =
      this.findContainingTagsWithSyntaxTree(position);
    if (
      startTag &&
      (startTag.range.containsPoint(position) ||
        endTag.range.containsPoint(position))
    ) {
      if (startTag === endTag) {
  ...
  findContainingTagsWithSyntaxTree(position) {
    ...
    this.editor.buffer
      .getLanguageMode()
      .getSyntaxNodeAtPosition(position, (node) => {
  ...
  findMatchingEndBracketWithRegexSearch(
    startBracketPosition,
    startBracket,
    endBracket
  ) {
  ...
  findMatchingStartBracketWithRegexSearch(
    endBracketPosition,
    startBracket,
    endBracket
  ) {
  ...
  findPrecedingStartBracketWithSyntaxTree(cursorPosition) {
    ...
    this.editor.buffer
      .getLanguageMode()
      .getSyntaxNodeAtPosition(cursorPosition, (node) => {
  ...
  findPrecedingStartBracketWithRegexSearch(cursorPosition) {
    ...
    const startBracket = _.escapeRegExp(
      _.keys(this.matchManager.pairedCharacters).join('')
    );
    const endBracket = _.escapeRegExp(
      _.keys(this.matchManager.pairedCharactersInverse).join('')
    );
  ...
  findCurrentPair() {
    ...
    const previousPosition = currentPosition.traverse(
      ONE_CHAR_BACKWARD_TRAVERSAL
    );
    ...
    if (
      (matchingBracket = this.matchManager.pairedCharacters[currentCharacter])
    ) {
      position = currentPosition;
      currentBracket = currentCharacter;
      matchPosition = this.findMatchingEndBracket(
        position,
        currentBracket,
        matchingBracket
      );
    } else if (
      (matchingBracket = this.matchManager.pairedCharacters[previousCharacter])
    ) {
      ...
      (matchingBracket =
        this.matchManager.pairedCharactersInverse[previousCharacter])
    ) {
      position = previousPosition;
      currentBracket = previousCharacter;
      ...
      (matchingBracket =
        this.matchManager.pairedCharactersInverse[currentCharacter])
    ) {
      position = currentPosition;
      currentBracket = currentCharacter;
      matchPosition = this.findMatchingStartBracket(
    ...
    if (this.tagHighlighted) {
      ...
      if (position.isLessThan(endRange.start)) {
        ...
        this.editor.setCursorBufferPosition(
          endRange.start.traverse([0, tagCharacterOffset])
        );
      } else {
        ...
        this.editor.setCursorBufferPosition(
          startRange.start.traverse([0, tagCharacterOffset])
        );
      }
    } else {
      ...
      if (position.isEqual(startPosition)) {
        this.editor.setCursorBufferPosition(
          endPosition.traverse(ONE_CHAR_FORWARD_TRAVERSAL)
        );
        ...
        this.editor.setCursorBufferPosition(
          startPosition.traverse(ONE_CHAR_FORWARD_TRAVERSAL)
        );
  ...
  gotoPrecedingStartBracket() {
    ...
    const matchPosition = this.findPrecedingStartBracket(
  ...
  closeTag() {
    ...
    const preFragment = this.editor.getTextInBufferRange([
      Point.ZERO,
      cursorPosition,
    ]);
    const postFragment = this.editor.getTextInBufferRange([
      cursorPosition,
      Point.INFINITY,
    ]);

    const tag = this.tagFinder.closingTagForFragments(
      preFragment,
      postFragment
    );
    if (tag) {
