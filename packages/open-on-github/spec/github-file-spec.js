...
describe('GitHubFile', function () {
    ...
    describe('open', () => {
      ...
      describe("when the root directory doesn't have a git repo", () => {
        ...
        it('does nothing', () => {
          ...
          expect(
            atom.notifications.addWarning.mostRecentCall.args[0]
          ).toContain('No repository found');
        });
      });
  ...
  describe('githubRepoURL', () => {
    ...
    it('will only strip a single .git suffix', () => {
      ...
      expect(githubFile.githubRepoURL()).toBe(
        'https://github.com/foo/bar.git.other'
      );
    });
    ...
    it('returns a GitHub enterprise URL for a non-Github.com remote URL', () => {
      ...
      expect(githubFile.githubRepoURL()).toBe(
        'https://git.enterprize.me/foo/bar'
      );
