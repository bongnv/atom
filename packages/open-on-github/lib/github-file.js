...
export default class GitHubFile {
  ...
  open(lineRange) {
    if (this.validateRepo()) {
      this.openURLInBrowser(
        this.blobURL() + this.getLineRangeSuffix(lineRange)
      );
    }
  }
  ...
  openOnMaster(lineRange) {
    if (this.validateRepo()) {
      this.openURLInBrowser(
        this.blobURLForMaster() + this.getLineRangeSuffix(lineRange)
      );
    }
  }
  ...
  blame(lineRange) {
    if (this.validateRepo()) {
      if (this.type === 'repo') {
        this.openURLInBrowser(
          this.blameURL() + this.getLineRangeSuffix(lineRange)
        );
      } else {
    ...
    if (this.validateRepo()) {
      if (this.type === 'repo') {
        ...
        atom.notifications.addWarning(
          `Branches do not exist for ${this.type}s`
        );
      }
    }
  }
  ...
  openPullRequests() {
    if (this.validateRepo()) {
      if (this.type === 'repo') {
        ...
        atom.notifications.addWarning(
          `Pull requests do not exist for ${this.type}s`
        );
      }
    }
  }
  ...
  validateRepo() {
    if (!this.repo) {
      atom.notifications.addWarning(
        `No repository found for path: ${this.filePath}.`
      );
      return false;
    } else if (!this.gitURL()) {
      atom.notifications.addWarning(
        `No URL defined for remote: ${this.remoteName()}`
      );
      return false;
    } else if (!this.githubRepoURL()) {
      atom.notifications.addWarning(
        `Remote URL is not hosted on GitHub: ${this.gitURL()}`
      );
      return false;
    }
    return true;
  ...
  blobURL() {
    ...
    if (this.type === 'wiki') {
      ...
      return `${gitHubRepoURL}#file-${this.encodeSegments(
        repoRelativePath.replace(/\./g, '-')
      )}`;
    } else {
      return `${gitHubRepoURL}/blob/${this.remoteBranchName()}/${this.encodeSegments(
        repoRelativePath
      )}`;
    }
  }
  ...
  blobURLForMaster() {
    ...
    if (this.type === 'repo') {
      return `${gitHubRepoURL}/blob/master/${this.encodeSegments(
  ...
  shaURL() {
    ...
    if (this.type === 'wiki') {
      return `${gitHubRepoURL}/${this.extractFileName(
        repoRelativePath
      )}/${encodedSHA}`;
    } else if (this.type === 'gist') {
      return `${gitHubRepoURL}/${encodedSHA}#file-${this.encodeSegments(
        repoRelativePath.replace(/\./g, '-')
      )}`;
    } else {
      return `${gitHubRepoURL}/blob/${encodedSHA}/${this.encodeSegments(
        repoRelativePath
      )}`;
    }
  }
  ...
  // Internal
  blameURL() {
    return `${this.githubRepoURL()}/blame/${this.remoteBranchName()}/${this.encodeSegments(
      this.repoRelativePath()
    )}`;
  }
  ...
  historyURL() {
    ...
    if (this.type === 'wiki') {
      ...
      return `${gitHubRepoURL}/commits/${this.remoteBranchName()}/${this.encodeSegments(
        this.repoRelativePath()
      )}`;
    }
  }
  ...
  // Internal
  branchCompareURL() {
    return `${this.githubRepoURL()}/compare/${this.encodeSegments(
      this.branchName()
    )}`;
  }
  ...
  gitURL() {
    ...
    if (remoteName != null) {
      return this.repo.getConfigValue(
  ...
  // Internal
  remoteName() {
    const gitConfigRemote = this.repo.getConfigValue(
      'atom.open-on-github.remote',
      this.filePath
    );
    ...
    const branchRemote = this.repo.getConfigValue(
      `branch.${shortBranch}.remote`,
      this.filePath
    );
  ...
  branchName() {
    ...
    const branchMerge = this.repo.getConfigValue(
      `branch.${shortBranch}.merge`,
      this.filePath
    );
  ...
  // Internal
  remoteBranchName() {
    const gitConfigBranch = this.repo.getConfigValue(
      'atom.open-on-github.branch',
      this.filePath
    );
