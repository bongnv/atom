...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.workspace.onDidAddPaneItem(({ item }) =>
        this.autosavePaneItem(item, true)
      )
    );
    this.subscriptions.add(
      atom.workspace.onWillDestroyPaneItem(({ item }) =>
        this.autosavePaneItem(item)
      )
    );
  },
  ...
  autosavePaneItem(paneItem, create = false) {
    ...
    if (typeof paneItem.isModified !== 'function' || !paneItem.isModified())
      return;
  ...
  autosaveAllPaneItems() {
    return Promise.all(
      atom.workspace
        .getPaneItems()
        .map((paneItem) => this.autosavePaneItem(paneItem))
    );
  },
};
