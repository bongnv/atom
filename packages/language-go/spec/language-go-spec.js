...
describe('Go settings', function () {
  ...
  it('matches lines correctly using the increaseIndentPattern', function () {
    const increaseIndentRegex =
      languageMode.increaseIndentRegexForScopeDescriptor(['source.go']);
    ...
    expect(
      increaseIndentRegex.testSync('  for i := 0; i < 10; i++ {')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  for i := 0; i < 10; i++ {')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  type something struct {')
    ).toBeTruthy();
    expect(increaseIndentRegex.testSync('  fmt.Printf("some%s",')).toBeTruthy();
    return expect(
      increaseIndentRegex.testSync('  aSlice := []string{}{')
    ).toBeTruthy();
  });

  it('matches lines correctly using the decreaseIndentPattern', function () {
    const decreaseIndentRegex =
      languageMode.decreaseIndentRegexForScopeDescriptor(['source.go']);
  ...
  return it('matches lines correctly using the decreaseNextIndentPattern', function () {
    const decreaseNextIndentRegex =
      languageMode.decreaseNextIndentRegexForScopeDescriptor(['source.go']);

    expect(
      decreaseNextIndentRegex.testSync('  fmt.Println("something"))')
    ).toBeTruthy();
    expect(
      decreaseNextIndentRegex.testSync('  fmt.Println("something")),')
    ).toBeTruthy();
    expect(
      decreaseNextIndentRegex.testSync('  fmt.Println("something"), "x"),')
    ).toBeTruthy();
    expect(
      decreaseNextIndentRegex.testSync(
        '  fmt.Println(fmt.Sprint("something"))),'
      )
    ).toBeTruthy();
    expect(
      decreaseNextIndentRegex.testSync(
        '  fmt.Println(fmt.Sprint("something"), "x")),'
      )
    ).toBeTruthy();
