...
module.exports = {
  ...
  readMetadata(directory, callback) {
    return fs.readFile(
      path.join(directory.getPath(), 'package.json'),
  ...
  isAtomPackage(metadata) {
    return (
      __guard__(
        __guard__(
          metadata != null ? metadata.engines : undefined,
          (x1) => x1.atom
        ),
        (x) => x.length
      ) > 0
  ...
  isEditingAnAtomPackageFile(editor) {
    ...
    if (editorPath != null) {
      ...
      if (path.basename(parsedPath.dir) === '.atom') {
        if (
          parsedPath.base === 'init.coffee' ||
          parsedPath.base === 'init.js'
        ) {
          return true;
        }
      }
    }
    for (var directory of this.packageDirectories != null
      ? this.packageDirectories
      : []) {
  ...
  getCompletions(line) {
    ...
    const propertyCompletions =
      (this.completions[property] != null
        ...
        ? this.completions[property] != null
          ...
          : undefined
        : [];
    for (var completion of propertyCompletions) {
  ...
  getPropertyClass(name) {
    return __guard__(
      atom[name] != null ? atom[name].constructor : undefined,
      (x) => x.name
    );
  },
  ...
  loadProperty(propertyName, className, classes, parent) {
    ...
    for (var completion of classCompletions) {
