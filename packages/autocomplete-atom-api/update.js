...
request(requestOptions, function(error, response, release) {
  ...
  return request(apiRequestOptions, function(error, response, atomApi) {
    ...
    for (var name in classes) {
      ...
      var pluckPropertyAttributes = convertPropertyToSuggestion.bind(this, name);
      var pluckMethodAttributes = convertMethodToSuggestion.bind(this, name);
      var properties = instanceProperties.filter(isVisible).map(pluckPropertyAttributes).sort(textComparator);
      var methods = instanceMethods.filter(isVisible).map(pluckMethodAttributes).sort(textComparator);
