...
request(requestOptions, function (error, response, release) {
  ...
  const [apiAsset] = Array.from(
    release.assets.filter(({ name }) => name === 'atom-api.json')
  );
  ...
  return request(apiRequestOptions, function (error, response, atomApi) {
    ...
    for (var name in classes) {
      var { instanceProperties, instanceMethods } = classes[name];
      var pluckPropertyAttributes = convertPropertyToSuggestion.bind(
        this,
        name
      );
      var pluckMethodAttributes = convertMethodToSuggestion.bind(this, name);
      var properties = instanceProperties
        .filter(isVisible)
        .map(pluckPropertyAttributes)
        .sort(textComparator);
      var methods = instanceMethods
        .filter(isVisible)
        .map(pluckMethodAttributes)
        .sort(textComparator);

      if (
        (properties != null ? properties.length : undefined) > 0 ||
        methods.length > 0
      ) {
    ...
    return fs.writeFileSync(
      'completions.json',
      JSON.stringify(publicClasses, null, '  ')
    );
  });
});

var isVisible = ({ visibility }) =>
  ['Essential', 'Extended', 'Public'].includes(visibility);
...
var convertMethodToSuggestion = function (className, method) {
  ...
  const returnValue = __guard__(
    returnValues != null ? returnValues[0] : undefined,
    (x) => x.type
  );
...
var convertPropertyToSuggestion = function (className, { name, summary }) {
  const text = name;
  const returnValue = __guard__(
    summary != null ? summary.match(/\{(\w+)\}/) : undefined,
    (x) => x[1]
  );
  const description = summary;
  const descriptionMoreURL = getDocLink(className, name);
...
var getDocLink = (className, instanceName) =>
