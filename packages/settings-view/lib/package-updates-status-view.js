...
export default class PackageUpdatesStatusView {
  initialize(statusBar, packageManager, updates) {
    ...
    this.disposables.add(
      packageManager.on(
        'package-update-available theme-update-available',
        ({ pack, error }) => {
  ...
  updateTile() {
    if (this.updates.length) {
      ...
      if (this.updatingPackages.length) {
        ...
        tooltipText += `, ${_.pluralize(
          this.updatingPackages.length,
          'package'
        )} currently updating`;
      }
