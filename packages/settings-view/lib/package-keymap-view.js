...
export default class PackageKeymapView {
  constructor(pack) {
    ...
    const packagesWithKeymapsDisabled =
      atom.config.get('core.packagesWithKeymapsDisabled') || [];
    this.refs.keybindingToggle.checked = !packagesWithKeymapsDisabled.includes(
      this.namespace
    );
    ...
    const changeHandler = (event) => {
      ...
      if (value) {
        atom.config.removeAtKeyPath(
          'core.packagesWithKeymapsDisabled',
          this.namespace
        );
      } else {
        atom.config.pushAtKeyPath(
          'core.packagesWithKeymapsDisabled',
          this.namespace
        );
      }
    ...
    for (let [packageKeymapsPath, keymap] of atom.packages.getLoadedPackage(
      this.namespace
    ).keymaps) {
      if (keymap.length > 0) {
        hasKeymaps = true;
  ...
  updateKeyBindingView() {
    ...
    const packagesWithKeymapsDisabled =
      atom.config.get('core.packagesWithKeymapsDisabled') || [];
    const keybindingsDisabled = packagesWithKeymapsDisabled.includes(
      this.namespace
    );
    ...
    if (atom.keymaps.build) {
      // eslint-disable-next-line no-unused-vars
      for (const [keymapPath, keymap] of atom.packages.getLoadedPackage(
        this.namespace
      ).keymaps) {
        keyBindings.push(
          ...atom.keymaps.build(this.namespace, keymap, 0, false)
        );
      }
    } else {
      ...
      for (const keyBinding of atom.keymaps.getKeyBindings()) {
        ...
        if (
          command &&
          command.indexOf &&
          command.indexOf(`${this.namespace}:`) === 0
        ) {
