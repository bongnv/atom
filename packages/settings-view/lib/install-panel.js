  ...
  constructor(settingsView, packageManager) {
    ...
    this.disposables.add(
      this.packageManager.on('package-install-failed', ({ pack, error }) => {
        this.refs.searchErrors.appendChild(
    ...
    this.disposables.add(
      this.packageManager.on(
        'package-installed theme-installed',
        ({ pack }) => {
          const gitUrlInfo =
          ...
          <div className="section-container">
            ...
            <div className="text native-key-bindings" tabIndex="-1">
              ...
              <span>
                ...
                {path.join(process.env.ATOM_HOME, 'packages')}
              </span>
            </div>
  ...
  async search(query) {
    ...
    try {
      ...
      if (packages.length === 0) {
        this.refs.searchMessage.textContent = `No ${this.searchType.replace(
          /s$/,
          ''
        )} results for \u201C${query}\u201D`;
        this.refs.searchMessage.style.display = '';
