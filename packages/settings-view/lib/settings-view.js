...
export default class SettingsView {
      ...
      atom.packages.onDidActivateInitialPackages(() => {
        this.disposables.add(
          atom.packages.onDidActivatePackage((pack) =>
            this.removePanelCache(pack.name)
          ),
  ...
  initializePanels() {
    ...
    this.disposables.add(
      new Disposable(() =>
        this.refs.openDotAtom.removeEventListener(
          'click',
          openDotAtomClickHandler
        )
      )
    );
  ...
  getPackages() {
    ...
    try {
      // FIXME: bongnv - we don't support resourcePath
      const packageMetadata = require(/* webpackIgnore: true */ path.join(
        atom.getLoadSettings().resourcePath,
        'package.json'
      ));
      bundledPackageMetadataCache = packageMetadata
        ? packageMetadata._atomPackages
        : null;
    } catch (error) {}
    ...
    for (const packageName of disabledPackages) {
      ...
      try {
        metadata = require(/* webpackIgnore: true */ path.join(
          packagePath,
          'package.json'
        ));
      } catch (error) {
  ...
  getOrCreatePanel(name, options) {
    ...
    if (name in this.panelCreateCallbacks) {
      ...
      panel = new PackageDetailView(
        options.pack,
        this,
        this.packageManager,
        this.snippetsProvider
      );
    }
    if (panel) {
