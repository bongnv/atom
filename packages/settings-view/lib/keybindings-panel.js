...
export default class KeybindingsPanel {
  constructor() {
    ...
    this.platformPattern = new RegExp(
      `\\.platform-${_.escapeRegExp(process.platform)}\\b`
    );
    ...
    this.disposables.add(
      this.refs.searchEditor.onDidStopChanging(() => {
        this.filterKeyBindings(
          this.keyBindings,
          this.refs.searchEditor.getText()
        );
      })
    );
  ...
  filterKeyBindings(keyBindings, filterString) {
    ...
    for (let keyBinding of keyBindings) {
      ...
      var searchString =
        `${selector}${keystrokes}${command}${source}`.toLowerCase();
      if (!searchString) {
        continue;
      }
  ...
  showSelector(selector) {
    ...
    return segments.some(
      (s) => this.platformPattern.test(s) || !this.otherPlatformPattern.test(s)
    );
  }
  ...
  static determineSource(filePath) {
    ...
    if (
      filePath.indexOf(
        path.join(atom.getLoadSettings().resourcePath, 'keymaps')
      ) === 0
    ) {
      return 'Core';
      ...
      const packageName =
        pathParts[packageNameIndex] != null ? pathParts[packageNameIndex] : '';
