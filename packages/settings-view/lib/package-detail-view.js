...
export default class PackageDetailView {
  constructor(pack, settingsView, packageManager, snippetsProvider) {
    ...
    const packageRepoClickHandler = (event) => {
      ...
      if (typeof repoUrl === 'string') {
        if (url.parse(repoUrl).pathname === '/atom/atom') {
          shell.openExternal(
            `${repoUrl}/tree/master/packages/${this.pack.name}`
          );
        } else {
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.issueButton.removeEventListener(
          'click',
    ...
    if (atom.packages.isPackageLoaded(this.pack.name)) {
      if (!atom.packages.isPackageDisabled(this.pack.name)) {
        ...
        if (this.pack.path) {
          ...
          this.snippetsView = new PackageSnippetsView(
            this.pack,
  ...
  updateFileButtons() {
    ...
    const packagePath =
      this.pack.path != null
        ? this.pack.path
        : atom.packages.resolvePackagePath(this.pack.name);
