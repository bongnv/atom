...
export default class ThemesPanel extends CollapsibleSectionPanel {
  ...
  constructor(settingsView, packageManager) {
    ...
    this.disposables.add(
      this.packageManager.on(
        'theme-install-failed theme-uninstall-failed',
        ({ pack, error }) => {
          this.refs.themeErrors.appendChild(
    ...
    this.disposables.add(
      atom.themes.onDidChangeActiveThemes(() => this.updateActiveThemes())
    );
    this.disposables.add(
      atom.tooltips.add(this.refs.activeUiThemeSettings, { title: 'Settings' })
    );
    this.disposables.add(
      atom.tooltips.add(this.refs.activeSyntaxThemeSettings, {
        title: 'Settings',
      })
    );
  ...
  createThemeMenuItem(themeName) {
    const title = _.undasherize(
      _.uncamelcase(
        themeName.replace(/-(ui|syntax)/g, '').replace(/-theme$/g, '')
      )
    );
  ...
  didClickActiveUiThemeSettings(event) {
    ...
    const theme = atom.themes
      .getActiveThemes()
      .find((theme) => theme.metadata.theme === 'ui');
  ...
  didClickActiveSyntaxThemeSettings(event) {
    ...
    const theme = atom.themes
      .getActiveThemes()
      .find((theme) => theme.metadata.theme === 'syntax');
