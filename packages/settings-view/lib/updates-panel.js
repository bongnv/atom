...
export default class UpdatesPanel {
  constructor(settingsView, packageManager) {
    ...
    this.disposables.add(
      this.packageManager.on(
        'package-updating theme-updating',
        ({ pack, error }) => {
      ...
      <div tabIndex="0" className="panels-item">
        <section className="section packages">
          <div className="section-container updates-container">
            ...
            >
              The following packages are pinned to their current version and are
              not being checked for updates:{' '}
              <strong>
  ...
  async updateAll() {
    ...
    if (successfulUpdatesCount > 0) {
      const message = `Restart Atom to complete the update of ${successfulUpdatesCount} ${pluralize(
        'package',
      ...
      this.packageCards.forEach((card) => {
        ...
        if (
          ...
          card.pack.apmInstallSource.type === 'git'
        ) {
