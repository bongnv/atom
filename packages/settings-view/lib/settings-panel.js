...
export default class SettingsPanel extends CollapsibleSectionPanel {
  ...
  elementForSettings(namespace, settings) {
    ...
    const includeTitle =
      this.options.includeTitle != null ? this.options.includeTitle : true;
    if (includeTitle) {
      if (title == null) {
  ...
  bindInputFields() {
    const disposables = Array.from(
      this.element.querySelectorAll('input[id]')
    ).map((input) => {
      let type = input.type;
      ...
      this.observe(name, (value) => {
        if (type === 'checkbox') {
          ...
          input.checked =
            value ===
            this.parseValue(atom.config.getSchema(name).type, input.value);
        } else {
          if (type === 'color') {
  ...
  bindSelectFields() {
    const disposables = Array.from(
      this.element.querySelectorAll('select[id]')
    ).map((select) => {
  ...
  bindEditors() {
    const disposables = Array.from(
      this.element.querySelectorAll('atom-text-editor')
    ).map((editorElement) => {
      ...
      subscriptions.add(
        new Disposable(() =>
          editorElement.removeEventListener('focus', focusHandler)
        )
      );
  ...
  bindTooltips() {
    const disposables = Array.from(
      this.element.querySelectorAll(
        'input[id], select[id], atom-text-editor[id]'
      )
    ).map((element) => {
      ...
      if (defaultValue != null) {
        if (schema.enum && _.findWhere(schema.enum, { value: defaultValue })) {
          defaultValue = _.findWhere(schema.enum, {
            value: defaultValue,
          }).description;
        }
...
function elementForSetting(namespace, name, value) {
  ...
  if (schema && schema.enum) {
    controls.appendChild(
      elementForOptions(namespace, name, value, { radio: schema.radio })
    );
...
function elementForObject(namespace, name, value) {
  if (_.keys(value).length === 0) {
    ...
    for (const key of sortSettings(keyPath, value)) {
      div.appendChild(
        elementForSetting(namespace, `${name}.${key}`, value[key])
      );
    }
