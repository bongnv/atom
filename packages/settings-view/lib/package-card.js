...
export default class PackageCard {
    ...
    this.packageManager.loadCompatiblePackageVersion(
      ...
      (err, pack) => {
        ...
        if (packageVersion) {
          ...
          console.error(
            `No available version compatible with the installed Atom version: ${atom.getVersion()}`
          );
        }
  ...
  handleButtonEvents(options) {
    ...
    const updateButtonClickHandler = (event) => {
      ...
      this.update().then(() => {
        ...
        const notification = atom.notifications.addSuccess(
          `Restart Atom to complete the update of \`${this.pack.name}\`.`,
          {
            dismissable: true,
            buttons: [
              {
    ...
    const packageAuthorClickHandler = (event) => {
      event.stopPropagation();
      shell.openExternal(
        `https://atom.io/users/${ownerFromRepository(this.pack.repository)}`
      );
    };
    this.refs.loginLink.addEventListener('click', packageAuthorClickHandler);
    this.disposables.add(
      new Disposable(() => {
        this.refs.loginLink.removeEventListener(
          'click',
          packageAuthorClickHandler
        );
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.refs.avatarLink.removeEventListener(
          'click',
          packageAuthorClickHandler
        );
      })
  ...
  */
  ...
  installAlternative() {
    ...
    const loadedPack = atom.packages.getLoadedPackage(
