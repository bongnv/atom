...
describe('KeybindingsPanel', function () {
  ...
  beforeEach(function () {
    ...
    const keySource = `${atom.getLoadSettings().resourcePath}${
      path.sep
    }keymaps`;
    keyBindings = [
      {
  ...
  it('loads and displays core key bindings', function () {
    ...
    return expect(row.querySelector('.selector').textContent).toBe(
...
`);
  ...
  return describe('when searching key bindings', function () {
    it('find case-insensitive results', function () {
      ...
      expect(row.querySelector('.command').textContent).toBe(
