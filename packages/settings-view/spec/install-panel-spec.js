...
describe('InstallPanel', function () {
  ...
  describe('searching packages', () =>
    it('displays the packages in the order returned', function () {
      spyOn(this.panel.client, 'search').andCallFake(() =>
        Promise.resolve([{ name: 'not-first' }, { name: 'first' }])
      );
      ...
      return runs(function () {
        expect(this.panel.getPackageCardView.argsForCall[0][0].name).toEqual(
          'not-first'
        );
        return expect(
          this.panel.getPackageCardView.argsForCall[1][0].name
        ).toEqual('first');
      });
    }));
  ...
  return describe('searching git packages', function () {
    ...
    it("doesn't show a git installation card for normal packages", function () {
      ...
      this.panel.performSearchForQuery(query);
      return expect(
        this.panel.showGitInstallPackageCard
