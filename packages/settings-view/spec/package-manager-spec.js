...
describe('PackageManager', function () {
  ...
  beforeEach(function () {
    spyOn(atom.packages, 'getApmPath').andReturn(
      '/an/invalid/apm/command/to/run'
    );
  ...
  it('handle errors spawning apm', function () {
    const noSuchCommandError =
      process.platform === 'win32' ? ' cannot find the path ' : 'ENOENT';
    ...
    runs(function () {
      ...
      expect(installArg.message).toBe(
        'Installing \u201Cfoo@1.0.0\u201D failed.'
      );
    ...
    return runs(function () {
      ...
      expect(updateArg.message).toBe(
        'Updating to \u201Cfoo@1.0.0\u201D failed.'
      );
  ...
  describe('::isPackageInstalled()', function () {
    ...
    it('returns true when a package is loaded', function () {
      ...
      return expect(packageManager.isPackageInstalled('some-package')).toBe(
        true
      );
    });

    return it('returns true when a package is disabled', function () {
      spyOn(atom.packages, 'getAvailablePackageNames').andReturn([
        'some-package',
      ]);
      return expect(packageManager.isPackageInstalled('some-package')).toBe(
        true
  ...
  describe('::install()', function () {
    ...
    beforeEach(() =>
      spyOn(packageManager, 'runCommand').andCallFake(function (
        args,
        callback
      ) {
        runArgs = args;
    ...
    return describe('git url installation', function () {
      ...
      it('installs and activates git pacakges with names different from the repo name', function () {
        ...
        return expect(atom.packages.activatePackage).toHaveBeenCalledWith(
          json.metadata.name
        );
      });
      ...
      return it('emits an installed event with a copy of the pack including the full package metadata', function () {
        ...
        const originalPackObject = {
          name: 'git-repo-name',
          otherData: { will: 'beCopied' },
        };
        ...
        for (var call of packageManager.emitPackageEvent.calls) {
  ...
  describe('::uninstall()', function () {
    ...
    beforeEach(function () {
      ...
      return spyOn(packageManager, 'runCommand').andCallFake(function (
        args,
        callback
      ) {
        runCallback = callback;
    ...
    return it('removes the package from the core.disabledPackages list', function () {
      ...
      runCallback(0, '', '');
      return expect(atom.config.get('core.disabledPackages')).not.toContain(
        'something'
      );
    });
  });
  ...
  describe('::installAlternative', function () {
    beforeEach(function () {
      ...
      return waitsFor(
        () => atom.packages.isPackageLoaded('language-test') === true
      );
    });
    ...
    return it('installs the latest version when a package version is not specified', function () {
      ...
      packageManager.installAlternative(
        { name: 'language-test' },
        'a-new-package',
        installedCallback
      );
      expect(packageManager.runCommand).toHaveBeenCalled();
      expect(packageManager.runCommand.calls[0].args[0]).toEqual([
        'uninstall',
        '--hard',
        'language-test',
      ]);
      expect(packageManager.runCommand.calls[1].args[0]).toEqual([
        'install',
        'a-new-package',
        '--json',
      ]);
      ...
      return runs(function () {
        ...
        return expect(installedCallback.mostRecentCall.args[1]).toEqual(
          eventArg
        );
      });
    });
  ...
  describe('::packageHasSettings', function () {
    it('returns true when the pacakge has config', function () {
      ...
      return expect(
        packageManager.packageHasSettings('package-with-config')
      ).toBe(true);
    });
    ...
    return it('returns true when the pacakge does not have config, but does define language grammars', function () {
      ...
      waitsForPromise(() =>
        atom.packages.activatePackage(
          path.join(__dirname, 'fixtures', packageName)
        )
      );
  ...
  return describe('::loadOutdated', function () {
    it('caches results', function () {
      ...
      expect(packageManager.apmCache.loadOutdated.value).toMatch([
        { name: 'boop' },
      ]);
    ...
    it('expires results if it is called with clearCache set to true', function () {
      ...
      return expect(packageManager.apmCache.loadOutdated.value).toEqual([
        { name: 'boop' },
      ]);
    });
