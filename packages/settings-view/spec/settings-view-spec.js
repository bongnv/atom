...
describe('SettingsView', function () {
  ...
  describe('serialization', function () {
    it('remembers which panel was visible', function () {
      ...
      return expect(newSettingsView.activePanel).toEqual({
        name: 'Themes',
        options: {},
      });
    });
    ...
    return it('serializes the active panel name even when the panels were never initialized', function () {
      ...
      return expect(settingsView3.activePanel).toEqual({
        name: 'Themes',
        options: {},
      });
    });
  ...
  describe('.addCorePanel(name, iconName, view)', () =>
    it('adds a menu entry to the left and a panel that can be activated by clicking it', function () {
      ...
      expect(
        settingsView.refs.panelMenu.querySelector('li[name="Panel 1"]')
      ).toExist();
      expect(
        settingsView.refs.panelMenu.querySelector('li[name="Panel 2"]')
      ).toExist();
      ...
      expect(
        settingsView.refs.panelMenu.querySelectorAll('.active').length
      ).toBe(1);
      expect(
        settingsView.refs.panelMenu.querySelector('li[name="Panel 1"]')
      ).toHaveClass('active');
      ...
      return expect(
        settingsView.refs.panels.querySelector('#panel-2')
      ).toBeVisible();
    }));
  ...
  describe('when the package is activated', function () {
    ...
    describe('when the settings view is opened with a settings-view:* command', function () {
      ...
      return describe('settings-view:check-for-package-updates', () =>
        it('opens the settings view to the install page', function () {
    ...
    describe('when atom.workspace.open() is used with a config URI', function () {
      const focusIsWithinActivePanel = function () {
        const activePanel =
          settingsView.panelsByName[settingsView.activePanel.name];
        return (
      ...
      it("doesn't use cached package detail when package re-activated and opnes the package view with atom://config/packages/<package-name>", function () {
        ...
        runs(function () {
          ...
          return expect(
            settingsView.getOrCreatePanel('package-with-readme')
          ).toBe(detailInitial);
        });
        ...
        return runs(function () {
          detailAfterReactivate = settingsView.getOrCreatePanel(
            'package-with-readme'
          );
          expect(settingsView.getOrCreatePanel('package-with-readme')).toBe(
            detailAfterReactivate
          );
      ...
      it("passes the URI to a pane's beforeShow() method on settings view initialization", function () {
        ...
        return runs(function () {
          ...
          return expect(InstallPanel.prototype.beforeShow).toHaveBeenCalledWith(
    ...
    return describe('when the package is then deactivated', function () {
      ...
      return it('calls the dispose method on all panels', function () {
        ...
        return runs(function () {
          ...
          waitsForPromise(() =>
            Promise.resolve(atom.packages.deactivatePackage('settings-view'))
  ...
  describe('when an installed package is clicked from the Install panel', () =>
    it('displays the package details', function () {
      ...
      runs(function () {
        ...
        spyOn(
          settingsView.packageManager.client,
          'featuredPackages'
      ...
      waitsFor(
        () =>
          settingsView.element.querySelectorAll('.package-card:not(.hidden)')
            .length > 0
      );

      return runs(function () {
        settingsView.element
          .querySelectorAll('.package-card:not(.hidden)')[0]
          .click();

        const packageDetail = settingsView.element.querySelector(
  ...
  return describe('when the active theme has settings', function () {
    ...
    beforeEach(function () {
      ...
      atom.config.set('core.themes', [
        'ui-theme-with-config',
        'syntax-theme-with-config',
      ]);
    ...
    describe("when the UI theme's settings button is clicked", () =>
      it("navigates to that theme's detail view", function () {
        ...
        const packageDetail = settingsView.element.querySelector(
          '.package-detail li.active'
        );
        return expect(packageDetail.textContent).toBe('Ui Theme With Config');
      }));

    return describe("when the syntax theme's settings button is clicked", () =>
      it("navigates to that theme's detail view", function () {
        jasmine.attachToDOM(settingsView.element);
        ...
        return expect(packageDetail.textContent).toBe(
          'Syntax Theme With Config'
        );
      }));
  });
