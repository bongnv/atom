...
describe('InstalledPackageView', function () {
  ...
  it('displays the grammars registered by the package', function () {
    ...
    waitsForPromise(() =>
      atom.packages.activatePackage(
        path.join(__dirname, 'fixtures', 'language-test')
      )
    );
    ...
    return runs(function () {
      ...
      settingsPanels = view.element.querySelectorAll(
        '.package-grammars .settings-panel'
      );

      waitsFor(function () {
        const children = Array.from(settingsPanels).map(
          (s) => s.children.length
        );
      ...
      expect(
        settingsPanels[0].querySelector('.grammar-filetypes').textContent
      ).toBe('File Types: .a, .aa, a');
  ...
  it('displays the snippets registered by the package', function () {
    ...
    waitsForPromise(() =>
      atom.packages.activatePackage('snippets').then(function (p) {
    ...
    runs(function () {
      ...
      return (snippetsTable = view.element.querySelector(
        '.package-snippets-table tbody'
      ));
    });
    ...
    return runs(function () {
      expect(
        snippetsTable.querySelector('tr:nth-child(1) td:nth-child(1)')
          .textContent
      ).toBe('b');
      expect(
        snippetsTable.querySelector('tr:nth-child(1) td:nth-child(2)')
          .textContent
      ).toBe('BAR');
      if (shouldRunScopeTest) {
        expect(
          snippetsTable.querySelector('tr:nth-child(1) td.snippet-scope-name')
            .textContent
        ).toBe('.b.source');
      }

      expect(
        snippetsTable.querySelector('tr:nth-child(2) td:nth-child(1)')
          .textContent
      ).toBe('f');
      expect(
        snippetsTable.querySelector('tr:nth-child(2) td:nth-child(2)')
          .textContent
      ).toBe('FOO');
      if (shouldRunScopeTest) {
        return expect(
          snippetsTable.querySelector('tr:nth-child(2) td.snippet-scope-name')
            .textContent
        ).toBe('.a.source');
      }
    });
    ...
    describe('when a snippet is copied', function () {
      ...
      beforeEach(function () {
        ...
        runs(function () {
          ...
          return (snippetsTable = card.element.querySelector(
            '.package-snippets-table tbody'
          ));
        });
      ...
      describe('when the snippets file ends in .cson', () =>
        it('writes a CSON snippet to the clipboard', function () {
          spyOn(SnippetsProvider, 'getUserSnippetsPath').andReturn(
            'snippets.cson'
          );
          card.element
            .querySelector(
...
`);
      ...
      return describe('when the snippets file ends in .json', () =>
...
`);
  ...
  describe('when the snippets toggle is clicked', () =>
    it('sets the packagesWithSnippetsDisabled config to include the package name', function () {
      ...
      runs(function () {
        ...
        return expect(
          _.include(
            (left = atom.config.get('core.packagesWithSnippetsDisabled')) !=
              null
              ? left
              : [],
            'language-test'
  ...
  it('does not display keybindings from other platforms', function () {
    ...
    return runs(function () {
      ...
      keybindingsTable = view.element.querySelector(
        '.package-keymap-table tbody'
      );
  ...
  describe('when the keybindings toggle is clicked', () =>
    it('sets the packagesWithKeymapsDisabled config to include the package name', function () {
      waitsForPromise(() =>
        atom.packages.activatePackage(
          path.join(__dirname, 'fixtures', 'language-test')
        )
      ...
      return runs(function () {
        ...
        expect(
          _.include(
            (left = atom.config.get('core.packagesWithKeymapsDisabled')) != null
              ? left
              : [],
            'language-test'
          )
        ...
        expect(
          _.include(
            (left1 = atom.config.get('core.packagesWithKeymapsDisabled')) !=
              null
              ? left1
              : [],
            'language-test'
...
`);
    ...
    return describe('when the keybinding file ends in .json', () =>
      it('writes a JSON snippet to the clipboard', function () {
...
`);
  ...
  describe('when the package is not active', function () {
    ...
    return it('still loads the config schema for the package', function () {
      ...
      waitsFor(
        () => atom.packages.isPackageLoaded('package-with-config') === true
      );
      ...
      return runs(function () {
        ...
        return expect(atom.config.get('package-with-config.setting')).toBe(
          'something'
        );
      });
    });
  ...
  return describe('when the package was not installed from atom.io', function () {
    ...
    return it('still displays the Readme', function () {
      ...
      waitsFor(
        () => atom.packages.isPackageLoaded('package-with-readme') === true
      );
