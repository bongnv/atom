...
describe('SettingsPanel', function () {
  ...
  describe('sorted settings', function () {
    beforeEach(function () {
      ...
      return (settingsPanel = new SettingsPanel({
        namespace: 'foo',
        includeTitle: false,
      }));
    });
    ...
    it('presents enum options with their descriptions', function () {
      ...
      const pairs = select.children.map((opt) => [opt.value, opt.innerText]);
      return expect(pairs).toEqual([
        ['one', 'One'],
        ['Two', 'Two'],
      ]);
    });
    ...
    return it('presents radio options with their descriptions', function () {
      ...
      const options = (() => {
        const result = [];
        for (var label of radio.querySelectorAll('label')) {
          var button = label.querySelector(
            'input[type=radio][name="foo.radio"]'
          );
  ...
  describe('default settings', function () {
    ...
    it('allows 0 to be a default', function () {
      const zeroEditor = settingsPanel.element.querySelector(
        '[id="foo.testZero"]'
      );
    ...
    it('selects the default choice for radio options', function () {
      ...
      return expect(
        settingsPanel.element.querySelector('#foo\\.radio\\[Two\\]')
      ).toBeChecked();
    });
    ...
    return describe('scoped settings', function () {
      ...
      it('displays the scoped default', function () {
        ...
        const tabLengthEditor = settingsPanel.element.querySelector(
          '[id="editor.tabLength"]'
        );
        expect(tabLengthEditor.getModel().getText()).toBe('');
        return expect(tabLengthEditor.getModel().getPlaceholderText()).toBe(
          'Default: 4'
        );
      });
      ...
      it('allows the scoped setting to be changed to its normal default if the unscoped value is different', function () {
        ...
        expect(tabLengthEditor.getModel().getText()).toBe('');
        expect(tabLengthEditor.getModel().getPlaceholderText()).toBe(
          'Default: 8'
        );
      ...
      return it('allows the scoped setting to be changed to the unscoped default if it is different', function () {
        ...
        expect(tabLengthEditor.getModel().getText()).toBe('2');
        return expect(
          atom.config.get('editor.tabLength', { scope: ['source.python'] })
        ).toBe(2);
      });
    });
  });
  ...
  describe('grouped settings', function () {
    ...
    it('ensures that only grouped settings have a group title', function () {
      expect(
        settingsPanel.element.querySelectorAll(
      ...
      expect(
        controlGroups[0].querySelectorAll('.sub-section .sub-section-heading')
      ).toHaveLength(1);
      expect(
        controlGroups[0].querySelector('.sub-section .sub-section-heading')
          .textContent
      ).toBe('Bar group');
      expect(
        controlGroups[0].querySelectorAll('.sub-section .sub-section-body')
      ).toHaveLength(1);
      let subsectionBody = controlGroups[0].querySelector(
        '.sub-section .sub-section-body'
      );
      expect(subsectionBody.querySelectorAll('.control-group')).toHaveLength(1);
      expect(
        controlGroups[1].querySelectorAll('.sub-section .sub-section-heading')
      ).toHaveLength(1);
      expect(
        controlGroups[1].querySelector('.sub-section .sub-section-heading')
          .textContent
      ).toBe('Baz Group');
      expect(
        controlGroups[1].querySelectorAll('.sub-section .sub-section-body')
      ).toHaveLength(1);
      subsectionBody = controlGroups[1].querySelector(
        '.sub-section .sub-section-body'
      );
      expect(subsectionBody.querySelectorAll('.control-group')).toHaveLength(1);
      expect(controlGroups[2].querySelectorAll('.sub-section')).toHaveLength(0);
      return expect(
        controlGroups[2].querySelectorAll('.sub-section-heading')
      ).toHaveLength(0);
    });
    ...
    return it('ensures grouped settings can have a description', function () {
      ...
      expect(controlGroups).toHaveLength(3);
      expect(
        controlGroups[0].querySelectorAll('.sub-section > .setting-description')
      ).toHaveLength(1);
      return expect(
        controlGroups[0].querySelector('.sub-section > .setting-description')
          .textContent
  ...
  return describe('settings validation', function () {
    ...
    it('prevents setting a value below the minimum', function () {
      const minMaxEditor =
        settingsPanel.element.querySelector('[id="foo.minMax"]');
      minMaxEditor.getModel().setText('0');
      advanceClock(
        minMaxEditor.getModel().getBuffer().getStoppedChangingDelay()
      );
    ...
    it('allows setting a valid scoped value', function () {
      ...
      const minMaxEditor =
        settingsPanel.element.querySelector('atom-text-editor');
      minMaxEditor.getModel().setText('15');
      advanceClock(
        minMaxEditor.getModel().getBuffer().getStoppedChangingDelay()
      );
    ...
    return describe('commaValueArray', function () {
      it('comma in value is escaped', function () {
        ...
        advanceClock(
          commaValueArrayEditor.getModel().getBuffer().getStoppedChangingDelay()
        );
