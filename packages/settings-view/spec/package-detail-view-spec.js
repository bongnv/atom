...
describe('PackageDetailView', function () {
  ...
  const loadPackageFromRemote = function (packageName, opts) {
    ...
    packageManager.client.package.andCallFake(function (name, cb) {
      const packageData = require(path.join(
        __dirname,
        'fixtures',
        packageName,
        'package.json'
  ...
  it('shows a loading message and calls out to atom.io when package metadata is missing', function () {
    ...
    expect(view.refs.loadingMessage.classList.contains('hidden')).not.toBe(
      true
    );
  ...
  it('shows an error when package metadata cannot be loaded via the API', function () {
    ...
    return expect(view.element.querySelectorAll('.package-card').length).toBe(
      0
    );
  });
  ...
  it('renders the README successfully after a call to the atom.io api', function () {
    ...
    expect(view.packageCard.refs.packageName.textContent).toBe(
      'package-with-readme'
    );
    return expect(view.element.querySelectorAll('.package-readme').length).toBe(
      1
    );
  });
  ...
  it('renders the README successfully with sanitized html', function () {
    loadPackageFromRemote('package-with-readme');
    expect(view.element.querySelectorAll('.package-readme script').length).toBe(
      0
    );
    expect(view.element.querySelectorAll('.package-readme iframe').length).toBe(
      0
    );
    expect(
      view.element.querySelectorAll(
        '.package-readme input[type="checkbox"][disabled]'
      ).length
    ).toBe(2);
    expect(
      view.element.querySelector('img[alt="AbsoluteImage"]').getAttribute('src')
    ).toBe('https://example.com/static/image.jpg');
    expect(
      view.element.querySelector('img[alt="RelativeImage"]').getAttribute('src')
    ).toBe(
      'https://github.com/example/package-with-readme/blob/master/static/image.jpg'
    );
    return expect(
      view.element.querySelector('img[alt="Base64Image"]').getAttribute('src')
    ).toBe(
  ...
  it('triggers a report issue button click and checks that the fallback repository issue tracker URL was opened', function () {
    ...
    return expect(shell.openExternal).toHaveBeenCalledWith(
  ...
  return it('should open internal package repository url', function () {
    ...
    view.refs.packageRepo.click();
    return expect(shell.openExternal).toHaveBeenCalledWith(
      'https://github.com/atom/atom/tree/master/packages/package-internal'
    );
  });
});
