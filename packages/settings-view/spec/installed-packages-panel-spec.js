...
describe('InstalledPackagesPanel', function () {
  describe('when the packages are loading', () =>
    it('filters packages by name once they have loaded', function () {
      ...
      spyOn(this.packageManager, 'getOutdated').andReturn(
        new Promise(function () {})
      );
      spyOn(this.packageManager, 'loadCompatiblePackageVersion').andCallFake(
        function () {}
      );
      spyOn(this.packageManager, 'getInstalled').andReturn(
        Promise.resolve(this.installed)
      );
      this.panel = new InstalledPackagesPanel(
        settingsView,
        this.packageManager
      );
      this.panel.refs.filterEditor.setText('user-');
      window.advanceClock(
        this.panel.refs.filterEditor.getBuffer().stoppedChangingDelay
      );
      ...
      return runs(function () {
        ...
        expect(
          this.panel.refs.communityPackages.querySelectorAll(
            '.package-card:not(.hidden)'
          ).length
        ).toBe(1);

        expect(this.panel.refs.coreCount.textContent.trim()).toBe('0/1');
        expect(
          this.panel.refs.corePackages.querySelectorAll(
            '.package-card:not(.hidden)'
          ).length
        ).toBe(0);
        ...
        expect(
          this.panel.refs.devPackages.querySelectorAll(
            '.package-card:not(.hidden)'
          ).length
        ).toBe(0);
        ...
        return expect(
          this.panel.refs.deprecatedPackages.querySelectorAll(
            '.package-card:not(.hidden)'
          ).length
        ).toBe(0);
      });
  ...
  describe('when the packages have finished loading', function () {
    ...
    it('adds newly installed packages to the list', function () {
      ...
      spyOn(this.packageManager, 'runCommand').andCallFake(function (
        args,
        callback
      ) {
    ...
    return it('correctly handles deprecated packages', function () {
      ...
      spyOn(atom.packages, 'isDeprecatedPackage').andCallFake(() => {
        if (this.installed.user[0].version === '1.0.0') {
          return true;
        }
        return false;
      });
      ...
      return runs(() =>
        expect(
          PackageCard.prototype.displayAvailableUpdate
        ).toHaveBeenCalledWith('1.1.0')
      );
    });
  });
  ...
  describe('expanding and collapsing sub-sections', function () {
    ...
    it('collapses and expands a sub-section if its header is clicked', function () {
      this.panel.element
        ...
        .click();
      expect(
        this.panel.element.querySelector('.sub-section.installed-packages')
      ).toHaveClass('collapsed');

      expect(
        this.panel.element.querySelector('.sub-section.deprecated-packages')
      ).not.toHaveClass('collapsed');
      expect(
        this.panel.element.querySelector('.sub-section.core-packages')
      ).not.toHaveClass('collapsed');
      expect(
        this.panel.element.querySelector('.sub-section.dev-packages')
      ).not.toHaveClass('collapsed');
    ...
    it('can collapse and expand any of the sub-sections', function () {
      ...
      for (element of this.panel.element.querySelectorAll(
        '.sub-section-heading.has-items'
      )) {
        element.click();
      }
    ...
    return it('can collapse sub-sections when filtering', function () {
      ...
      const hasItems = this.panel.element.querySelectorAll(
        '.sub-section-heading.has-items'
      );
  ...
  return describe('when there are no packages', function () {
    ...
    it('has a count of zero in all headings', function () {
      expect(
        this.panel.element.querySelector('.section-heading-count').textContent
      ).toMatch(/^0+$/);
      expect(
        this.panel.element.querySelectorAll('.sub-section .icon-package').length
      ).toBe(5);
      return expect(
        this.panel.element.querySelectorAll(
          '.sub-section .icon-package.has-items'
        ).length
      ).toBe(0);
    });
    ...
    return it('does not allow collapsing on any section when filtering', function () {
      ...
      return expect(
        this.panel.element.querySelectorAll(
          '.sub-section .icon-paintcan.has-items'
        ).length
      ).toBe(0);
    });
  });
