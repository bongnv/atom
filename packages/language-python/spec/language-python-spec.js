...
describe('Python settings', function () {
  ...
  it('matches lines correctly using the increaseIndentPattern', function () {
    const increaseIndentRegex =
      languageMode.increaseIndentRegexForScopeDescriptor(['source.python']);

    expect(increaseIndentRegex.testSync('for i in range(n):')).toBeTruthy();
    expect(increaseIndentRegex.testSync('  for i in range(n):')).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('async for i in range(n):')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  async for i in range(n):')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('class TheClass(Object):')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  class TheClass(Object):')
    ).toBeTruthy();
    ...
    expect(increaseIndentRegex.testSync('  async def f(x):')).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('if this_var == that_var:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  if this_var == that_var:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('elif this_var == that_var:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  elif this_var == that_var:')
    ).toBeTruthy();
    ...
    expect(
      increaseIndentRegex.testSync('  except Exception as e:')
    ).toBeTruthy();
    expect(increaseIndentRegex.testSync('finally:')).toBeTruthy();
    expect(increaseIndentRegex.testSync('  finally:')).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('with open("filename") as f:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  with open("filename") as f:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('async with open("filename") as f:')
    ).toBeTruthy();
    expect(
      increaseIndentRegex.testSync('  async with open("filename") as f:')
    ).toBeTruthy();
    ...
    expect(increaseIndentRegex.testSync('  while True:')).toBeTruthy();
    return expect(
      increaseIndentRegex.testSync('\t\t  while True:')
    ).toBeTruthy();
  });
  ...
  it('does not match lines incorrectly using the increaseIndentPattern', function () {
    ...
    return expect(
      increaseIndentRegex.testSync('"for i in range(n):"')
    ).toBeFalsy();
  });

  it('matches lines correctly using the decreaseIndentPattern', function () {
    const decreaseIndentRegex =
      languageMode.decreaseIndentRegexForScopeDescriptor(['source.python']);

    expect(
      decreaseIndentRegex.testSync('elif this_var == that_var:')
    ).toBeTruthy();
    expect(
      decreaseIndentRegex.testSync('  elif this_var == that_var:')
    ).toBeTruthy();
    ...
    expect(
      decreaseIndentRegex.testSync('  except Exception as e:')
    ).toBeTruthy();
