...
describe('Python grammar', function () {
  ...
  it('recognises shebang on firstline', function () {
    expect(
      grammar.firstLineRegex.scanner.findNextMatchSync('#!/usr/bin/env python')
    ).not.toBeNull();
    return expect(
      grammar.firstLineRegex.scanner.findNextMatchSync('#! /usr/bin/env python')
    ).not.toBeNull();
  ...
  it('terminates a single-quoted raw string containing opening parenthesis at closing quote', function () {
    ...
    return expect(tokens[0][7].scopes).toEqual([
      'source.python',
      'comment.line.number-sign.python',
    ]);
  });
  ...
  it('terminates referencing an item in a list variable after a sequence of a closing and opening bracket', function () {
    ...
    return expect(tokens[0][12].scopes).toEqual([
      'source.python',
      'meta.structure.list.python',
  ...
  it('tokenizes a hex escape inside a string', function () {
    ...
    return expect(tokens[0][1].scopes).toEqual([
      'source.python',
      'string.quoted.double.single-line.python',
      'constant.character.escape.hex.python',
    ]);
...
`);
  ...
  grammarTest(
    path.join(__dirname, 'fixtures/grammar/syntax_test_python_functions.py')
  );
  grammarTest(
    path.join(__dirname, 'fixtures/grammar/syntax_test_python_lambdas.py')
  );
  grammarTest(
    path.join(__dirname, 'fixtures/grammar/syntax_test_python_typing.py')
  );
