...
`);
      ...
      if (parseFloat(atom.getVersion()) <= 1.2) {
        expect(lines[1][1]).toEqual({
          value: '->',
          scopes: [
            'text.html.basic',
...
`);
  ...
  grammarTest(
    path.join(__dirname, 'fixtures/syntax_test_html_template_fragments.html')
  );
  ...
  describe('attributes', function () {
    ...
    it('recognizes multiple attributes with varying values', function () {
      const { tokens } = grammar.tokenizeLine(
        "<span class='btn' disabled spellcheck=true>"
      );
      ...
      for (quote in quotes) {
        ...
        it(`tokenizes ${type}-quoted style attribute values as CSS property lists`, function () {
          let { tokens } = grammar.tokenizeLine(
            `<span style=${quote}display: none;${quote}>`
          );
...
`);
      ...
      it('tokenizes unquoted property lists', function () {
        ...
        ({ tokens } = grammar.tokenizeLine(
          '<span style=display:none;z-index:10></span>'
        ));
  ...
  describe('character references', function () {
    it('tokenizes & and characters after it', function () {
      ...
      const lines = grammar.tokenizeLines('&\n');
      return expect(lines[0][0]).toEqual({
        value: '&',
  ...
  describe('firstLineMatch', function () {
    it('recognises HTML5 doctypes', function () {
      expect(
        grammar.firstLineRegex.scanner.findNextMatchSync('<!DOCTYPE html>')
      ).not.toBeNull();
      return expect(
        grammar.firstLineRegex.scanner.findNextMatchSync('<!doctype HTML>')
      ).not.toBeNull();
...
`;
  ...
  return describe('snippets', function () {
    ...
    beforeEach(function () {
      ...
      snippetsModule = require(atom.packages
        .loadPackage(snippetsPath)
        .getMainModulePath());

      // Disable loading of user snippets before the package is activated
      spyOn(snippetsModule, 'loadUserSnippets').andCallFake((callback) =>
        callback({})
      );

      snippetsModule.activate();

      return waitsFor('snippets to load', (done) =>
    ...
    it('suggests snippets', () =>
      expect(
        Object.keys(snippetsModule.parsedSnippetsForScopes(['.text.html']))
          .length
      ).toBeGreaterThan(10));

    return it('does not suggest any HTML snippets when in embedded scripts', () =>
      expect(
        Object.keys(
          snippetsModule.parsedSnippetsForScopes([
            '.text.html .source.js.embedded.html',
          ])
        ).length
      ).toBe(0));
  });
