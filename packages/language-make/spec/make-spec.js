...
describe('Makefile grammar', function () {
  ...
  it('selects the Makefile grammar for files that start with a hashbang make -f command', () =>
    expect(atom.grammars.selectGrammar('', '#!/usr/bin/make -f')).toBe(
      grammar
    ));
  ...
  const testFunctionCall = function (functionName) {
    const { tokens } = grammar.tokenizeLine(
      'foo: echo $(' + functionName + ' /foo/bar.txt)'
    );
  ...
  it('parses nested interpolated strings and function calls correctly', function () {
    ...
    return runs(function () {
      ...
      return expect(lines[1][19]).toEqual({
        value: ')',
        scopes: [
          'source.makefile',
          'meta.scope.target.makefile',
