...
module.exports = class BufferView extends FuzzyFinderView {
  ...
  async buildItems() {
    ...
    const projectHasMultipleDirectories =
    ...
    for (let i = 0; i < localEditors.length; i++) {
      ...
      if (remoteEditor) {
        ...
        const [projectRootPath, projectRelativePath] =
          atom.project.relativizePath(localEditorPath);
  ...
  async getRemoteEditorsByURI() {
    ...
    const remoteEditors = this.teletypeService
      ? await this.teletypeService.getRemoteEditors()
      : [];
