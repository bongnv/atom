...
function showNotification(
  message,
  { description, confirmBtn, cancelBtn, confirmFn, cancelFn }
) {
...
function enableExperimentalFuzzyFinder(metricsReporter) {
  ...
  showNotification('Introducing experimental fast mode', {
    description:
      "The fuzzy finder has a new experimental _fast mode_. It dramatically speeds up the experience of finding files, especially in large projects. Would you like to try it?\n\n(You can always switch back to the fuzzy finder's _normal mode_ later.)",
    ...
    confirmFn: () => {
      ...
      atom.notifications.addSuccess(
        'Fasten your seatbelt! Here comes a faster fuzzy finder.',
        { icon: 'rocket' }
      );
    },
...
function disableExperimentalFuzzyFinder(metricsReporter) {
  ...
  showNotification(
    ...
    {
      description:
        'If the experimental fast mode is negatively impacting your experience, please leave a comment to [**let us know**](https://github.com/atom/fuzzy-finder/issues/379).\n\n(You can reenable fast mode later from the fuzzy finder.)',
      ...
      confirmFn: () => {
        ...
        atom.config.set(
          'fuzzy-finder.scoringSystem',
          SCORING_SYSTEMS.ALTERNATE
        );
        atom.notifications.addSuccess(
          'OK. Experimental fast mode is disabled.'
        );
      },
      cancelBtn: 'No, thanks',
      cancelFn: () => {
...
function renderExperimentPrompt(metricsReporter) {
  if (isFastModeEnabled()) {
    ...
    return $.span({ className: 'experiment-prompt' }, [
      $.a(
        {
          onmousedown: disableExperimentalFuzzyFinder.bind(
            null,
            metricsReporter
          ),
        },
  ...
  return $.span({ className: 'experiment-prompt' }, [
    $.a(
      {
        onmousedown: enableExperimentalFuzzyFinder.bind(null, metricsReporter),
      },
      [
        shouldShowBadge()
