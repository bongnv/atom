...
describe('FuzzyFinder', () => {
    ...
    describe(`file-finder behavior (ripgrep=${useRipGrep}, scoringSystem=${scoringSystem})`, () => {
      ...
      describe('toggling', () => {
        describe('when the project has multiple paths', () => {
          it('shows or hides the fuzzy-finder and returns focus to the active editor if it is already showing', async () => {
            ...
            expect(atom.workspace.panelForItem(projectView).isVisible()).toBe(
              true
            );
            expect(
              projectView.selectListView.refs.queryEditor.element
            ).toHaveFocus();
          ...
          it('shows all files for the current project and selects the first', async () => {
            ...
            eachFilePath([rootDir1, rootDir2], (filePath) => {
              const item = Array.from(
                projectView.element.querySelectorAll('li')
              ).find((a) => a.textContent.includes(filePath));
              expect(item).toExist();
            ...
            expect(
              projectView.element.querySelector('.loading')
            ).not.toBeVisible();
          });
          ...
          it('puts the last opened path first', async () => {
            ...
            expect(results[results.length - 1].textContent).toContain(
              'sample.html'
            );
          });
          ...
          describe('symlinks on #darwin or #linux', () => {
            ...
            beforeEach(() => {
              ...
              fs.symlinkSync(
                ...
                atom.project
                  .getDirectories()[0]
                  .resolve('symlink-to-internal-file')
              );
              fs.symlinkSync(
                atom.project.getDirectories()[0].resolve('dir'),
                atom.project
                  .getDirectories()[0]
                  .resolve('symlink-to-internal-dir')
              );
              fs.symlinkSync(
                atom.project.getDirectories()[0].resolve('..'),
                atom.project
                  .getDirectories()[0]
                  .resolve('symlink-to-project-dir-ancestor')
              );
            ...
            it('indexes project paths that are symlinks', async () => {
              const symlinkProjectPath = path.join(
                junkDirPath,
                'root-dir-symlink'
              );
              ...
              expect(
                Array.from(projectView.element.querySelectorAll('li')).find(
                  (a) => a.textContent.includes('sample.txt')
                )
              ).toBeDefined();
            });
            ...
            it('includes symlinked file paths', async () => {
              ...
              const destSymlink = results.find((a) =>
                a.textContent.includes('symlink-to-file')
              );
            ...
            it('excludes symlinked folder paths if followSymlinks is false', async () => {
              ...
              expect(
                results.find((a) =>
                  a.textContent.includes('symlink-to-internal-dir')
                )
              ).not.toBeDefined();
              expect(
                results.find((a) =>
            ...
            it('includes symlinked folder paths if followSymlinks is true', async () => {
              ...
              const destSymlink = results.find((a) =>
                a.textContent.includes('symlink-to-dir/a')
              );
          ...
          describe('socket files on #darwin or #linux', () => {
            ...
            it('does not interfere with ability to load files', async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll('li')).find(
                  (a) => a.textContent.includes('a')
                )
              ).toBeDefined();
              expect(
                Array.from(projectView.element.querySelectorAll('li')).find(
                  (a) => a.textContent.includes('some.sock')
                )
              ).not.toBeDefined();
            });
          });
          ...
          it('ignores paths that match entries in config.fuzzy-finder.ignoredNames', async () => {
            atom.config.set('fuzzy-finder.ignoredNames', [
              'sample.js',
              '*.txt',
            ]);
          ...
          it("only shows a given path once, even if it's within multiple root folders", async () => {
            ...
            expect(
              Array.from(projectView.element.querySelectorAll('li')).filter(
                (a) => a.textContent.includes('child-file.txt')
              ).length
            ).toBe(1);
          });
          ...
          it('returns all the results if they have the same relative path across multiple root folders', async () => {
            ...
            await projectView.selectListView.refs.queryEditor.setText(
              'whatever.js'
            );
            ...
            expect(results).toEqual([
              {
                label: 'whatever.js',
                description: path.join('root-dir1', 'whatever.js'),
              },
              {
                label: 'whatever.js',
                description: path.join('root-dir2', 'whatever.js'),
              },
            ]);
          });
        ...
        describe('when the project has no path', () => {
          ...
          it('shows an empty message with no files in the list', async () => {
            ...
            expect(
              projectView.selectListView.refs.emptyMessage.textContent
            ).toBe('Project is empty');
      ...
      describe('when a path selection is confirmed', () => {
        it('opens the file associated with that path in that split', async () => {
          ...
          const expectedPath = atom.project
            .getDirectories()[0]
            .resolve('dir/a');
        ...
        describe('when the selected path is a directory', () =>
          it("leaves the the tree view open, doesn't open the path in the editor, and displays an error", async () => {
            ...
            projectView.confirm({
              uri: atom.project.getDirectories()[0].resolve('dir'),
            });
          ...
          describe('when there are pane items with paths', () => {
            ...
            it("shows the FuzzyFinder if it isn't showing, or hides it and returns focus to the active editor", async () => {
              ...
              const [editor1, editor2, editor3] =
                atom.workspace.getTextEditors(); // eslint-disable-line no-unused-vars
              ...
              expect(atom.workspace.panelForItem(bufferView).isVisible()).toBe(
                true
              );
              expect(
                workspaceElement.querySelector('.fuzzy-finder')
              ).toHaveFocus();
            ...
            it('lists the paths of the current items, sorted by most recently opened but with the current item last', async () => {
              ...
              expect(
                Array.from(
                  bufferView.element.querySelectorAll('li > div.file')
                ).map((e) => e.textContent)
              ...
              expect(bufferView.element.querySelector('li')).toHaveClass(
                'selected'
              );
            });
            ...
            it('serializes the list of paths and their last opened time', async () => {
              ...
              let states = _.map(
                atom.packages.getPackageState('fuzzy-finder'),
                (path, time) => [path, time]
              );
          ...
          describe('when the active pane does not have an item for the selected path and fuzzy-finder.searchAllPanes is true', () => {
            ...
            it('switches to the pane with the item for the selected path', async () => {
              ...
              bufferView.confirm(
                { uri: expectedPath },
                {
                  searchAllPanes: atom.config.get(
                    'fuzzy-finder.searchAllPanes'
                  ),
                }
              );
      ...
      describe('opening a path into a split', () => {
        it('opens the path by splitting the active editor left', async () => {
          ...
          await conditionPromise(
            () => atom.workspace.getCenter().getPanes().length === 2
          );
          ...
          describe('when the numbers are not followed by another colon', () => {
            describe('when the filter text has a file path', () => {
              it('opens the selected path to that line number', async () => {
                ...
                expect(
                  atom.workspace.panelForItem(bufferView).isVisible()
                ).toBe(true);
                ...
                const { filePath } =
                  bufferView.selectListView.getSelectedItem();
                ...
                await conditionPromise(
                  () => bufferView.moveToCaretPosition.callCount > 0
                );
            ...
            describe("when the filter text doesn't have a file path", () => {
              it('moves the cursor in the active editor to that line number', async () => {
                ...
                expect(bufferView.element.querySelectorAll('li').length).toBe(
                  0
                );
                spyOn(bufferView, 'moveToCaretPosition').andCallThrough();
                atom.commands.dispatch(bufferView.element, 'core:confirm');
            ...
            describe('when the colon is followed by more numbers', () => {
              ...
              describe("when the filter text doesn't have a file path", () => {
                it('moves the cursor in the active editor to that line number and column', async () => {
                  ...
                  expect(
                    bufferView.selectListView.refs.emptyMessage.innerText
                  ).toEqual('Jump to line and column in active editor');
                  expect(bufferView.element.querySelectorAll('li').length).toBe(
                    0
                  );
                  spyOn(bufferView, 'moveToCaretPosition').andCallThrough();
        ...
        describe('when the colon is not followed by numbers', () => {
          ...
          describe("when the filter text doesn't have a file path", () => {
            it('shows an error and does not move the cursor', async () => {
              ...
              expect(
                bufferView.selectListView.refs.errorMessage.innerText
              ).toEqual('Invalid line number');
            ...
            it('updates the message when the error gets resolved', async () => {
              ...
              expect(
                bufferView.selectListView.refs.emptyMessage.innerText
              ).toEqual(emptyMessage);
              expect(
                bufferView.selectListView.refs.errorMessage
              ).toBeUndefined();

              bufferView.selectListView.refs.queryEditor.setText(':42a');
              await getOrScheduleUpdatePromise();
              expect(
                bufferView.selectListView.refs.emptyMessage
              ).toBeUndefined();
              expect(
                bufferView.selectListView.refs.errorMessage.innerText
      ...
      describe('match highlighting', () => {
        ...
        it('highlights an exact match', async () => {
          ...
          const primaryMatches = resultView.querySelectorAll(
            '.primary-line .character-match'
          );
          const secondaryMatches = resultView.querySelectorAll(
            '.secondary-line .character-match'
          );
          expect(primaryMatches.length).toBe(1);
          expect(primaryMatches[primaryMatches.length - 1].textContent).toBe(
            'sample.js'
          );
          // Use `toBeGreaterThan` because dir may have some characters in it
          expect(secondaryMatches.length).toBeGreaterThan(0);
          expect(
            secondaryMatches[secondaryMatches.length - 1].textContent
          ).toBe('sample.js');
        });
        ...
        it('highlights matches in the directory and file name', async () => {
          spyOn(bufferView, 'projectRelativePathsForFilePaths').andCallFake(
            (paths) => paths
          );
      ...
      describe('preserve last search', () => {
        ...
        it('preserves last search when the config is set', async () => {
          ...
          expect(
            projectView.selectListView.refs.queryEditor.getSelectedText()
          ).toBe('this should show up next time we open finder');
        });
      });
      ...
      describe('prefill query from selection', () => {
        it('should not be enabled by default', async () => {
          ...
          expect(atom.workspace.getActiveTextEditor().getSelectedText()).toBe(
            'sample.txt'
          );
      ...
      describe('default file icons', () => {
        it('shows a text icon for text-based formats', async () => {
          ...
          const firstResult =
            bufferView.element.querySelector('li .primary-line');
          expect(
            DefaultFileIcons.iconClassForPath(firstResult.dataset.path)
          ).toBe('icon-file-text');
        });
      ...
      describe('icon services', () => {
        describe('atom.file-icons', () => {
          ...
          it('allows services to replace the default handler', async () => {
            ...
            expect(firstResult.className).toBe(
              'primary-line file icon foo bar'
            );
            disposable.dispose();
            expect(getIconServices().fileIcons).toBe(DefaultFileIcons);
          });
        ...
        describe('file-icons.element-icons', () => {
          ...
          it('uses the element-icon service if available', async () => {
            ...
            disposable.dispose();
            expect(getIconServices().elementIcons).toBe(null);
            expect(firstResult.classList).not.toBe(
              'primary-line file icon foo bar'
            );
          });
        });
      ...
      describe('Git integration', () => {
        ...
        beforeEach(() => {
          projectPath = atom.project
            .getDirectories()[0]
            .resolve('git/working-dir');
          fs.moveSync(
          ...
          return new Promise((resolve) =>
            gitRepository.onDidChangeStatuses(resolve)
          );
        });
        ...
        describe('git-status-finder behavior', () => {
          ...
          it('displays all new and modified paths', async () => {
            ...
            expect(atom.workspace.panelForItem(gitStatusView).isVisible()).toBe(
              true
            );
            expect(gitStatusView.element.querySelectorAll('.file').length).toBe(
              4
            );
            expect(
              gitStatusView.element.querySelectorAll('.status.status-modified')
                .length
            ).toBe(1);
            expect(
              gitStatusView.element.querySelectorAll('.status.status-added')
                .length
            ).toBe(3);
          });
        });
        ...
        describe('status decorations', () => {
          ...
          describe('when a modified file is shown in the list', () =>
            it('displays the modified icon', async () => {
              ...
              expect(
                bufferView.element.querySelectorAll('.status.status-modified')
                  .length
              ).toBe(1);
              expect(
                bufferView.element
          ...
          describe('when a new file is shown in the list', () =>
            it('displays the new icon', async () => {
              ...
              expect(
                bufferView.element.querySelectorAll('.status.status-added')
                  .length
              ).toBe(1);
              expect(
                bufferView.element
        ...
        describe('when core.excludeVcsIgnoredPaths is set to true', () => {
          ...
          describe("when the project's path is the repository's working directory", () => {
            ...
            it('excludes paths that are git ignored', async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll('li')).find(
                  (a) => a.textContent.includes('ignored.txt')
                )
              ).not.toBeDefined();
            });
          });
          ...
          describe("when the project's path is a subfolder of the repository's working directory", () => {
            ...
            if (useRipGrep) {
              it('does excludes paths that are git ignored', async () => {
                fs.writeFileSync(
                  path.join(projectPath, 'dir', 'a.txt'),
                  'something'
                );
                ...
                expect(
                  Array.from(projectView.element.querySelectorAll('li')).find(
                    (a) => a.textContent.includes('b.txt')
                  )
                ).not.toBeDefined();
              });
            } else {
          ...
          describe('when the .gitignore matches parts of the path to the root folder', () => {
            ...
            it('only applies the .gitignore patterns to relative paths within the root folder', async () => {
              ...
              expect(
                Array.from(projectView.element.querySelectorAll('li')).find(
                  (a) => a.textContent.includes('file.txt')
                )
              ).toBeDefined();
            });
          });
        ...
        describe('logging of metrics events', () => {
          it('logs the crawling time', async () => {
            ...
            expect(reporterStub.addTiming.firstCall.args[0]).toEqual(
              'fuzzy-finder-v1'
            );
          ...
          if (scoringSystem !== 'standard') {
            it('logs the filtering time', async () => {
              ...
              expect(reporterStub.addTiming.lastCall.args[0]).toEqual(
                'fuzzy-finder-v1'
              );
  ...
  it('shows all files for the current project once the initial crawler has finished', async () => {
    ...
    eachFilePath([rootDir1, rootDir2], (filePath) => {
      const item = Array.from(projectView.element.querySelectorAll('li')).find(
        (a) => a.textContent.includes(filePath)
      );
      expect(item).toExist();
      const nameDiv = item.querySelector('div:first-child');
      expect(nameDiv.dataset.name).toBe(path.basename(filePath));
