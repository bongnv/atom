...
describe('Notifications', function () {
  ...
  describe('when the package is activated', () =>
    it('attaches an atom-notifications element to the dom', () =>
      expect(
        workspaceElement.querySelector('atom-notifications')
      ).toBeDefined()));
    ...
    return it('displays all non displayed notifications', function () {
      ...
      return runs(function () {
        const notificationContainer =
          workspaceElement.querySelector('atom-notifications');
        let notification = notificationContainer.querySelector(
          'atom-notification.warning'
        );
        expect(notification).toExist();
        notification = notificationContainer.querySelector(
  ...
  return describe('when notifications are added to atom.notifications', function () {
    ...
    beforeEach(function () {
      const enableInitNotification = atom.notifications.addSuccess(
    ...
    it('adds an atom-notification element to the container with a class corresponding to the type', function () {
      ...
      const notification = notificationContainer.querySelector(
        'atom-notification.success'
      );
      expect(notificationContainer.childNodes.length).toBe(1);
      expect(notification).toHaveClass('success');
      expect(notification.querySelector('.message').textContent.trim()).toBe(
        'A message'
      );
    ...
    it('displays notification with a detail when a detail is specified', function () {
      ...
      expect(notification.querySelector('.detail').textContent).toContain(
        'Some detail'
      );
      ...
      return expect(
        notification.querySelector('.detail').textContent
      ).toContain('cats,ok');
    });
    ...
    it('does not add the has-stack class if a stack is provided without any detail', function () {
      ...
      const notificationElement = notificationContainer.querySelector(
    ...
    it('renders the message as sanitized markdown', function () {
      atom.notifications.addInfo(
        'test <b>html</b> <iframe>but sanitized</iframe>'
      );
      const notification = notificationContainer.childNodes[0];
    ...
    describe('when a dismissable notification is added', function () {
      it('is removed when Notification::dismiss() is called', function () {
        const notification = atom.notifications.addSuccess('A message', {
          dismissable: true,
        });
        const notificationElement = notificationContainer.querySelector(
          'atom-notification.success'
      ...
      return it('focuses the active pane only if the dismissed notification has focus', function () {
        ...
        return runs(function () {
          const notification1 = atom.notifications.addSuccess('First message', {
            dismissable: true,
          });
          const notification2 = atom.notifications.addError('Second message', {
            dismissable: true,
          });
          const notificationElement1 = notificationContainer.querySelector(
            'atom-notification.success'
          );
          const notificationElement2 = notificationContainer.querySelector(
            'atom-notification.error'
          );
    ...
    describe('when the `description` option is used', () =>
      it('displays the description text in the .description element', function () {
        atom.notifications.addSuccess('A message', {
        ...
        expect(
          notification.querySelector('.description').textContent.trim()
        ).toBe('This is a link');
    ...
    return describe('when an exception is thrown', function () {
      ...
      describe('when the editor is in dev mode', function () {
        beforeEach(function () {
          ...
          notificationContainer =
            workspaceElement.querySelector('atom-notifications');
          return (fatalError = notificationContainer.querySelector(
      ...
      describe('when the exception has no core or package paths in the stack trace', () =>
        it('does not display a notification', function () {
          atom.notifications.clear();
      ...
      describe('when the message contains a newline', () =>
        it('removes the newline when generating the issue title', function () {
          const message =
            "Uncaught Error: Cannot read property 'object' of undefined\nTypeError: Cannot read property 'object' of undefined";
          atom.notifications.addFatalError(message);
          notificationContainer =
            workspaceElement.querySelector('atom-notifications');
          fatalError = notificationContainer.querySelector(
            'atom-notification.fatal'
          );
          ...
          waitsForPromise(() =>
            fatalError
              .getRenderPromise()
              .then(() => (issueTitle = fatalError.issue.getIssueTitle()))
          );
          return runs(function () {
      ...
      describe('when the message contains continguous newlines', () =>
        it('removes the newlines when generating the issue title', function () {
          const message =
            'Uncaught Error: Cannot do the thing\n\nSuper sorry about this';
          atom.notifications.addFatalError(message);
          notificationContainer =
            workspaceElement.querySelector('atom-notifications');
          fatalError = notificationContainer.querySelector(
...
`;
        ...
        return it('chooses the first package in the trace', () =>
      ...
      describe('when an exception is thrown from a package', function () {
        ...
        it('displays a fatal error with the package name in the error', function () {
          waitsForPromise(() =>
            fatalError.getRenderPromise().then(function () {
              issueTitle = fatalError.issue.getIssueTitle();
              return fatalError.issue
                .getIssueBody()
                .then((result) => (issueBody = result));
            })
          );
          ...
          return runs(function () {
            ...
            expect(issueBody).toContain(
              'Thrown From**: [notifications](https://github.com/atom/notifications) package '
            );
      ...
      describe('when an exception is thrown from a linked package', function () {
        beforeEach(function () {
          ...
          fs.makeTreeSync(
            path.dirname(path.join(packagesDir, 'linked-package'))
          );
          fs.symlinkSync(
            packageDir,
            path.join(packagesDir, 'linked-package'),
            'junction'
          );
          fs.writeFileSync(
...
ReferenceError: path is not defined
  at Object.module.exports.LinkedPackage.wow (${path.join(
    fs.realpathSync(packageDir),
    'linked-package.coffee'
  )}:29:15)
  at atom-workspace.subscriptions.add.atom.commands.add.linked-package:wow (${path.join(
    packageDir,
    'linked-package.coffee'
  )}:18:102)
...
`;
        ...
        return it('displays a fatal error with the package name in the error', function () {
          ...
          return runs(function () {
            ...
            expect(fatalError.innerHTML).toContain(
              '<a href="https://github.com/atom/notifications">linked-package package</a>'
            );
            return expect(fatalError.issue.getPackageName()).toBe(
              'linked-package'
            );
          });
        });
      ...
      describe('when an exception is thrown from an unloaded package', function () {
        beforeEach(function () {
          ...
          atom.packages.packageDirPaths.push(
            path.join(packagesDir, '.atom', 'packages')
          );
          const packageDir = path.join(
            packagesDir,
            '.atom',
...
`
        ...
        return it('displays a fatal error with the package name in the error', function () {
          ...
          return runs(function () {
            ...
            expect(fatalError).toHaveClass('has-close');
            expect(fatalError.innerHTML).toContain(
              'ReferenceError: unloaded error'
            );
            expect(fatalError.innerHTML).toContain(
              '<a href="https://github.com/atom/notifications">unloaded package</a>'
            );
...
`
        ...
        return it('displays a fatal error with the package name in the error', function () {
          ...
          return runs(function () {
            ...
            expect(fatalError.innerHTML).toContain(
              '<a href="https://github.com/atom/notifications">broken-load package</a>'
            );
            return expect(fatalError.issue.getPackageName()).toBe(
              'broken-load'
            );
...
"#\\{" "end": "\\}"
       ^^^^^
Unexpected string in ${path.join(
            'packageDir',
            'grammars',
            'broken-grammar.cson'
          )}
...
`;
        ...
        return it('displays a fatal error with the package name in the error', function () {
          ...
          return runs(function () {
            ...
            expect(fatalError.innerHTML).toContain(
              '<a href="https://github.com/atom/notifications">language-broken-grammar package</a>'
            );
            return expect(fatalError.issue.getPackageName()).toBe(
              'language-broken-grammar'
            );
...
`
        ...
        return it('displays a fatal error with the package name in the error', function () {
          ...
          return runs(function () {
            ...
            expect(fatalError.innerHTML).toContain(
              '<a href="https://github.com/atom/notifications">broken-activation package</a>'
            );
            return expect(fatalError.issue.getPackageName()).toBe(
              'broken-activation'
            );
      ...
      describe('when the error has not been reported', function () {
        ...
        return describe('when the message is longer than 100 characters', function () {
          const message =
            "Uncaught Error: Cannot find module 'dialog'Error: Cannot find module 'dialog' at Function.Module._resolveFilename (module.js:351:15) at Function.Module._load (module.js:293:25) at Module.require (module.js:380:17) at EventEmitter.<anonymous> (/Applications/Atom.app/Contents/Resources/atom/browser/lib/rpc-server.js:128:79) at EventEmitter.emit (events.js:119:17) at EventEmitter.<anonymous> (/Applications/Atom.app/Contents/Resources/atom/browser/api/lib/web-contents.js:99:23) at EventEmitter.emit (events.js:119:17)";
          const expectedIssueTitle =
          ...
          return it('truncates the issue title to 100 characters', function () {
            ...
            return runs(function () {
              const button = fatalError.querySelector('.btn');
              expect(button.textContent).toContain('Create issue');
              return expect(fatalError.issue.getIssueTitle()).toBe(
                expectedIssueTitle
              );
            });
          });
      ...
      describe('when the package is out of date', function () {
        beforeEach(function () {
          ...
          spyOn(UserUtilities, 'getPackageVersion').andCallFake(
            () => installedVersion
          );
        ...
        describe('when the package is a non-core package', function () {
          beforeEach(function () {
            ...
            spyOn(NotificationIssue.prototype, 'getPackageName').andCallFake(
              () => 'somepackage'
            );
            spyOn(NotificationIssue.prototype, 'getRepoUrl').andCallFake(
              () => 'https://github.com/someguy/somepackage'
            );
            generateException();
            fatalError = notificationContainer.querySelector(
              'atom-notification.fatal'
        ...
        return describe('when the package is a core package', function () {
          ...
          describe("when the locally installed version is lower than Atom's version", function () {
            beforeEach(function () {
              ...
              spyOn(
                UserUtilities,
                'getPackageVersionShippedWithAtom'
              ).andCallFake(() => versionShippedWithAtom);
            ...
            return it('tells the user that the package is a locally installed core package and out of date', function () {
              ...
              expect(fatalNotification.textContent).toContain(
                'Locally installed core Atom package'
              );
              return expect(fatalNotification.textContent).toContain(
                'is out of date'
              );
      ...
      describe('when Atom is out of date', function () {
        ...
        return it('provides a link to the latest released version', function () {
          ...
          return expect(fatalNotification.innerHTML).toContain(
            '<a href="https://github.com/atom/atom/releases/tag/v0.180.0">latest version</a>'
          );
        });
      });
      ...
      describe('when the error has been reported', function () {
        ...
        describe('when the issue is open', function () {
            ...
            generateFakeFetchResponses({
              issuesResponse: {
                items: [
                  {
                    title:
                      'ReferenceError: a is not defined in $ATOM_HOME/somewhere',
          ...
          return it('shows the user a view issue button', function () {
            ...
            return expect(fetch.calls[0].args[0]).toContain(
              encodeURIComponent('atom/notifications')
            );
          });
        });
      ...
      describe('when a BufferedProcessError is thrown', () =>
        it('adds an error to the notifications', function () {
          expect(
            notificationContainer.querySelector('atom-notification.error')
          ).not.toExist();
          ...
          const error = notificationContainer.querySelector(
            'atom-notification.error'
          );
      ...
      return describe('when a spawn ENOENT error is thrown', function () {
        ...
        describe('when the binary has no path', function () {
          ...
          return it('displays a dismissable error without the stack trace', function () {
            ...
            return expect(error.textContent).toContain(
              "'some_binary' could not be spawned"
            );
          });
        });
        ...
        return describe('when the binary has /atom in the path', function () {
          beforeEach(function () {
            try {
              ...
              e.code = 'ENOENT';
              const message =
                'Error: spawn /opt/atom/Atom Helper (deleted) ENOENT';
