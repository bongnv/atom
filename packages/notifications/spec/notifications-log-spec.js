...
describe('Notifications Log', function () {
  ...
  beforeEach(function () {
    ...
    return waitsForPromise(() =>
      atom.workspace.open(NotificationsLog.prototype.getURI())
    );
  });
  ...
  describe('when the package is activated', () =>
    it('attaches an atom-notifications element to the dom', () =>
      expect(
        workspaceElement.querySelector('.notifications-log-items')
      ).toBeDefined()));

  describe('when there are notifications before activation', function () {
    beforeEach(() =>
      waitsForPromise(() => atom.packages.deactivatePackage('notifications'))
    );
    ...
    return it('displays all non displayed notifications', function () {
      ...
      return runs(function () {
        const notificationsLogContainer = workspaceElement.querySelector(
          '.notifications-log-items'
        );
        let notification = notificationsLogContainer.querySelector(
  ...
  describe('when notifications are added to atom.notifications', function () {
    ...
    it('adds an .notifications-log-item element to the container with a class corresponding to the type', function () {
      ...
      let notification = notificationsLogContainer.querySelector(
        '.notifications-log-item.success'
      );
      ...
      expect(
        notificationsLogContainer.querySelector(
          '.notifications-log-item.warning'
        )
      ).toBeDefined();
    ...
    describe('when the `buttons` options is used', () =>
      it('displays the buttons in the .btn-toolbar element', function () {
        const clicked = [];
        atom.notifications.addSuccess('A message', {
          buttons: [
            {
    ...
    describe('when an exception is thrown', function () {
      ...
      describe('when the there is an error searching for the issue', function () {
        beforeEach(function () {
          ...
          generateException();
          fatalError = notificationsLogContainer.querySelector(
            '.notifications-log-item.fatal'
          );
    ...
    return describe('when a log item is clicked', function () {
      ...
      describe('when the notification is not dismissed', () =>
        describe('when the notification is not dismissable', function () {
          beforeEach(function () {
          ...
          return it('makes the notification dismissable', function () {
            logItem.click();
            expect(
              notificationView.element.classList.contains('has-close')
            ).toBe(true);
      ...
      return describe('when the notification is dismissed', function () {
        beforeEach(function () {
          ...
          notificationView = atom.views.getView(notification);
          logItem = notificationsLogContainer.querySelector(
            '.notifications-log-item.info'
          );
        ...
        return describe('when the notification is dismissed again', () =>
          it('emits did-dismiss', function () {
  ...
  describe('when notifications are cleared', function () {
    beforeEach(function () {
      const clearButton = workspaceElement.querySelector(
        '.notifications-log .notifications-clear-log'
      );
    ...
    return it('clears the notifications', function () {
      ...
      const logItems = workspaceElement.querySelector(
        '.notifications-log-items'
      );
  ...
  return describe('the dock pane', function () {
    ...
    beforeEach(
      () =>
        (notificationsLogPane = atom.workspace.paneForURI(
        ...
        it('opens the pane', function () {
          ...
          waitsForPromise(() =>
            atom.workspace
              ...
              .then((paneItem) => (notificationsLog = paneItem))
          );
        ...
        return describe('when notifications are displayed', function () {
          ...
          return it('lists all notifications', function () {
            ...
            return runs(function () {
              ...
              return expect(notificationsLogContainer.childNodes).toHaveLength(
                1
              );
            });
          });
