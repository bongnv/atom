...
module.exports = NotificationIssue = class NotificationIssue {
      ...
      .then(function (data) {
        if ((data != null ? data.items : undefined) != null) {
          ...
          for (var issue of data.items) {
            if (
              issue.title.indexOf(issueTitle) > -1 &&
  ...
  getIssueUrl() {
    return this.getIssueBody().then((issueBody) => {
      ...
      return `${repoUrl}/issues/new?title=${this.encodeURI(
        this.getIssueTitle()
  ...
  getIssueBody() {
    return new Promise((resolve, reject) => {
      ...
      return Promise.all([systemPromise, nonCorePackagesPromise]).then(
        (all) => {
...
`;
  ...
  getPackageNameFromFilePath(filePath) {
    ...
    let packageName = __guard__(
      /\/\.atom\/dev\/packages\/([^\/]+)\//.exec(filePath),
      (x) => x[1]
    );
    if (packageName) {
      return packageName;
    ...
    const getPackageName = (filePath) => {
      ...
      if (path.isAbsolute(filePath)) {
        for (var packName in packagePaths) {
          ...
          var isSubfolder =
            filePath.indexOf(path.normalize(packagePath + path.sep)) === 0;
          if (isSubfolder) {
            return packName;
          }
        }
