...
const Notifications = {
  ...
  activate(state) {
    ...
    for (notification of atom.notifications.getNotifications()) { this.addNotificationView(notification); }
...
`;
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', 'core:cancel', () => (() => {
      const result = [];
      for (notification of atom.notifications.getNotifications()) {           result.push(notification.dismiss());
      }
      return result;
    })())
    );
