...
const Notifications = {
  ...
  activate(state) {
    ...
    for (notification of atom.notifications.getNotifications()) {
    ...
    this.subscriptions.add(
      atom.onWillThrowError(function ({
        message,
        url,
        line,
        originalError,
...
`;
          ...
          if (
            originalError.stack &&
            !isCoreOrPackageStackTrace(originalError.stack)
          ) {
            return;
          }
  ...
  addNotificationView(notification) {
    ...
    if (this.lastNotification != null) {
      ...
      const timeSpan =
        notification.getTimestamp() - this.lastNotification.getTimestamp();
      if (
        timeSpan >= this.duplicateTimeDelay ||
        !notification.isEqual(this.lastNotification)
      ) {
        this.notificationsElement.appendChild(
          atom.views.getView(notification).element
        );
        if (this.notificationsLog != null) {
...
var isCoreOrPackageStackTrace = function (stack) {
  ...
  for (var { file } of StackTraceParser.parse(stack)) {
