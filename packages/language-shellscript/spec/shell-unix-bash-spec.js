...
describe('Shell script grammar', function () {
  ...
  it('tokenizes if as a keyword', function () {
    ...
    return (() => {
      const result = [];
      for (var openingBracket in brackets) {
  ...
  it('tokenizes for...in loops', function () {
    let { tokens } = grammar.tokenizeLine(
      'for variable in file do do-something-done done'
    );
    expect(tokens[0]).toEqual({
      value: 'for',
      scopes: [
    ...
    ({ tokens } = grammar.tokenizeLine(
      'for "variable" in "${list[@]}" do something done'
    ));
    expect(tokens[0]).toEqual({
      value: 'for',
      scopes: [
...
`));
    ...
    return it('indents newline-style while loop', () =>
      expectPreservedIndentation(`\
while [ $x -gt 0 ]
do
x=$(($x-1))
