...
describe('Shell session grammar', function () {
  ...
  it('tokenizes prompts', () =>
    (() => {
      const result = [];
      for (var delim of prompts) {
        var { tokens } = grammar.tokenizeLine(delim + ' echo $FOO');
...
`);
  ...
  it('tokenizes prompts with prefixes and a leading parenthetical', function () {
    const { tokens } = grammar.tokenizeLine(
      '(venv) machine:pwd user$ echo $FOO'
    );
...
`);
    ...
    return expect(tokens[1][0]).toEqual({
      value: 'foo',
      scopes: ['text.shell-session', 'meta.output.shell-session'],
    });
  });
