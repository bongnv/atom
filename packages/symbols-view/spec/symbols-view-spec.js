...
describe('SymbolsView', () => {
  ...
  describe('when tags can be generated for a file', () => {
    ...
    it('initially displays all JavaScript functions with line numbers', async () => {
      ...
      await conditionPromise(
        () => symbolsView.element.querySelectorAll('li').length > 0
      );
      ...
      expect(
        symbolsView.element.querySelector('li:first-child .primary-line')
      ).toHaveText('quicksort');
      expect(
        symbolsView.element.querySelector('li:first-child .secondary-line')
      ).toHaveText('Line 1');
      expect(
        symbolsView.element.querySelector('li:last-child .primary-line')
      ).toHaveText('quicksort.sort');
      expect(
        symbolsView.element.querySelector('li:last-child .secondary-line')
      ).toHaveText('Line 2');
    ...
    it('displays an error when no tags match text in mini-editor', async () => {
      ...
      expect(
        symbolsView.selectListView.refs.emptyMessage.textContent.length
      ).toBeGreaterThan(0);
  ...
  describe('go to declaration', () => {
    ...
    it('displays matches when more than one exists and opens the selected match', async () => {
      ...
      expect(atom.workspace.getActiveTextEditor().getPath()).toBe(
        directory.resolve('tagged-duplicate.js')
      );
      expect(
        atom.workspace.getActiveTextEditor().getCursorBufferPosition()
      ).toEqual([0, 4]);
  ...
  describe('project symbols', () => {
    it('displays all tags', async () => {
      ...
      expect(
        symbolsView.element.querySelector('li:first-child .primary-line')
      ).toHaveText('callMeMaybe');
      expect(
        symbolsView.element.querySelector('li:first-child .secondary-line')
      ).toHaveText(taggedFile);
      expect(
        symbolsView.element.querySelector('li:last-child .primary-line')
      ).toHaveText('thisIsCrazy');
      expect(
        symbolsView.element.querySelector('li:last-child .secondary-line')
      ).toHaveText(taggedFile);
      atom.commands.dispatch(
      ...
      await conditionPromise(
        () => symbolsView.selectListView.refs.loadingMessage
      );
      await conditionPromise(
        () => symbolsView.element.querySelectorAll('li').length === 0
      );
    ...
    describe('when selecting a tag', () => {
      describe("when the file doesn't exist", () => {
        ...
        it("doesn't open the editor", async () => {
          ...
          await conditionPromise(
            () => symbolsView.selectListView.refs.errorMessage
          );
  ...
  describe('when useEditorGrammarAsCtagsLanguage is set to true', () => {
    it("uses the language associated with the editor's grammar", async () => {
      ...
      atom.workspace
        .getActiveTextEditor()
        .setGrammar(atom.grammars.grammarForScopeName('source.js'));
      atom.commands.dispatch(
        getEditorView(),
        'symbols-view:toggle-file-symbols'
      );
      await conditionPromise(
        () => symbolsView.element.querySelectorAll('li').length === 1
      );
      expect(document.body.contains(symbolsView.element)).toBe(true);
      expect(
        symbolsView.element.querySelector('li:first-child .primary-line')
  ...
  describe('quickjump to symbol', () => {
    ...
    it('restores previous editor state on cancel', async () => {
      ...
      atom.workspace
        .getActiveTextEditor()
        .setSelectedBufferRanges(bufferRanges);
      atom.commands.dispatch(
        getEditorView(),
        'symbols-view:toggle-file-symbols'
      );
      ...
      await symbolsView.cancel();
      expect(
        atom.workspace.getActiveTextEditor().getSelectedBufferRanges()
      ).toEqual(bufferRanges);
    });
  });
