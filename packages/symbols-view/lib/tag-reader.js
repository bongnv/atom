...
let wordAtCursor = (text, cursorIndex, wordSeparator, noStripBefore) => {
  ...
  const beforeCursorWordBegins = noStripBefore
    ? 0
  ...
  return (
    beforeCursor.slice(beforeCursorWordBegins) +
...
export default {
  find(editor, callback) {
    ...
    if (!symbols.length) {
      ...
      const rubyScopes = scope
        .getScopesArray()
        .filter((s) => /^source\.ruby($|\.)/.test(s));
        ...
        ({ range, match }) => {
          if (range.containsPoint(cursorPosition)) {
            ...
            if (rubyScopes.length && symbol.indexOf(':') > -1) {
              const cursorWithinSymbol =
                cursorPosition.column - range.start.column;
  ...
  getAllTags(callback) {
    const projectTags = [];
    const task = Task.once(handlerPath, atom.project.getPaths(), () =>
      callback(projectTags)
    );
    task.on('tags', (tags) => {
