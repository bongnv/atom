...
module.exports = TabBarView = class TabBarView {
  constructor(pane, location) {
    ...
    this.paneElement.addEventListener(
      'dragenter',
      this.onPaneDragEnter.bind(this)
    );
    this.paneElement.addEventListener(
      'dragleave',
      this.onPaneDragLeave.bind(this)
    );

    this.paneContainer = this.pane.getContainer();
    for (var item of this.pane.getItems()) {
    ...
    this.subscriptions.add(
      atom.config.observe('tabs.tabScrolling', (value) =>
        this.updateTabScrolling(value)
      )
    );
    this.subscriptions.add(
  ...
  onDragStart(event) {
    ...
    if (typeof item.getAllowedLocations === 'function') {
      for (var location of item.getAllowedLocations()) {
  ...
  removeDropTargetClasses() {
    ...
    for (dropTarget of workspaceElement.querySelectorAll(
