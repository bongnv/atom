...
(TabBarView = class TabBarView {
  constructor(pane, location) {
    ...
    for (var item of this.pane.getItems()) { this.addTabForItem(item); }
  ...
  terminatePendingStates() {
    for (var tab of this.getTabs()) { if (typeof tab.terminatePendingState === 'function') {
  ...
  removeTabForItem(item) {
    ...
    for (tab of this.getTabs()) { tab.updateTitle(); }
    return this.updateTabBarVisibility();
  }
  ...
  onMouseLeave() {
    for (var tab of this.getTabs()) { tab.element.style.maxWidth = ''; }
  }
