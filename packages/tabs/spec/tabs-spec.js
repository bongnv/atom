...
describe('Tabs package main', function () {
  ...
  return describe('.deactivate()', () =>
    it('removes all tab bar views and stops adding them to new panes', function () {
      ...
      waitsForPromise(() =>
        Promise.resolve(atom.packages.deactivatePackage('tabs'))
      ); // Wrapped so works with Promise & non-Promise deactivate

      return runs(function () {
        expect(centerElement.querySelectorAll('.pane').length).toBe(2);
        expect(centerElement.querySelectorAll('.pane > .tab-bar').length).toBe(
          0
        );
        ...
        return expect(
          centerElement.querySelectorAll('.pane > .tab-bar').length
        ).toBe(0);
      });
    }));
});
...
describe('TabBarView', function () {
  let [deserializerDisposable, item1, item2, editor1, pane, tabBar] =
    Array.from([]);
  ...
  class TestView {
    ...
    emitTitleChanged() {
      return (this.titleCallbacks != null ? this.titleCallbacks : []).map(
        (callback) => callback()
      );
    }
    ...
    emitIconChanged() {
      return (this.iconCallbacks != null ? this.iconCallbacks : []).map(
        (callback) => callback()
      );
    }
    onDidChangeModified() {
  ...
  describe('when the mouse is moved over the tab bar', () =>
    it('fixes the width on every tab', function () {
      ...
      const initialWidth1 = tabBar
        .tabAtIndex(0)
        .element.getBoundingClientRect()
        .width.toFixed(0);
      const initialWidth2 = tabBar
        .tabAtIndex(2)
        .element.getBoundingClientRect()
        .width.toFixed(0);
      ...
      expect(
        parseFloat(
          tabBar.tabAtIndex(0).element.style.maxWidth.replace('px', '')
        ).toFixed(0)
      ).toBe(initialWidth1);
      return expect(
        parseFloat(
          tabBar.tabAtIndex(2).element.style.maxWidth.replace('px', '')
        ).toFixed(0)
      ).toBe(initialWidth2);
    }));
  ...
  describe('.initialize(pane)', function () {
    it("creates a tab for each item on the tab bar's parent pane", function () {
      ...
      expect(
        tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
          .textContent
      ).toBe(item1.getTitle());
      expect(
        tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
      ...
      expect(tabBar.element.querySelectorAll('.tab')[0].dataset.type).toBe(
        'TestView'
      );

      expect(
        tabBar.element.querySelectorAll('.tab')[1].querySelector('.title')
          .textContent
      ).toBe(editor1.getTitle());
      expect(
        tabBar.element.querySelectorAll('.tab')[1].querySelector('.title')
          .dataset.name
      ).toBe(path.basename(editor1.getPath()));
      expect(
        tabBar.element.querySelectorAll('.tab')[1].querySelector('.title')
          .dataset.path
      ).toBe(editor1.getPath());
      expect(tabBar.element.querySelectorAll('.tab')[1].dataset.type).toBe(
        'TextEditor'
      );

      expect(
        tabBar.element.querySelectorAll('.tab')[2].querySelector('.title')
          .textContent
      ).toBe(item2.getTitle());
      expect(
        tabBar.element.querySelectorAll('.tab')[2].querySelector('.title')
      ...
      return expect(
        tabBar.element.querySelectorAll('.tab')[0].dataset.type
      ).toBe('TestView');
    });
  ...
  describe('when the active pane item changes', () =>
    it('highlights the tab for the new active pane item', function () {
      ...
      pane.activateItem(item2);
      expect(tabBar.element.querySelectorAll('.active').length).toBe(1);
      return expect(tabBar.element.querySelectorAll('.tab')[2]).toHaveClass(
        'active'
      );
    }));
  ...
  describe('when a new item is added to the pane', function () {
    it("adds the 'modified' class to the new tab if the item is initially modified", function () {
      ...
      return runs(function () {
        ...
        return expect(tabBar.tabForItem(editor2).element).toHaveClass(
          'modified'
        );
      });
    });
    ...
    describe('when addNewTabsAtEnd is set to true in package settings', function () {
      it('adds a tab for the new item at the end of the tab bar', function () {
        ...
        return expect(
          tabBar.tabAtIndex(3).element.querySelector('.title').textContent
        ).toMatch('Item 3');
      });
      ...
      return it("puts the new tab at the last index of the pane's items", function () {
        ...
        pane.activateItem(item1);
        pane.activateItem(item3);
        return expect(pane.getItems()[pane.getItems().length - 1]).toEqual(
          item3
        );
      });
    });
    ...
    return describe('when addNewTabsAtEnd is set to false in package settings', () =>
      it('adds a tab for the new item at the same index as the item in the pane', function () {
        ...
        expect(tabBar.element.querySelectorAll('.tab').length).toBe(4);
        return expect(
          tabBar.tabAtIndex(1).element.querySelector('.title').textContent
        ).toMatch('Item 3');
      }));
  });
  ...
  describe('when an item is removed from the pane', function () {
    ...
    return it('updates the titles of the remaining tabs', function () {
      ...
      return expect(tabBar.tabForItem(item2).element.textContent).toMatch(
        'Item 2'
      );
    });
  });
  ...
  describe('when a tab is clicked', function () {
    it('shows the associated item on the pane and focuses the pane', function () {
      ...
      let { mousedown, click } = triggerClickEvent(
        tabBar.tabAtIndex(0).element,
        { button: 0 }
      );
      ...
      ({ mousedown, click } = triggerClickEvent(tabBar.tabAtIndex(2).element, {
        button: 0,
      }));
    ...
    it('closes the tab when middle clicked', function () {
      const { click } = triggerClickEvent(tabBar.tabForItem(editor1).element, {
        button: 1,
      });
    ...
    return it("doesn't switch tab when right (or ctrl-left) clicked", function () {
      ...
      let { mousedown } = triggerClickEvent(tabBar.tabAtIndex(0).element, {
        button: 2,
      });
  ...
  describe('when an item is activated', function () {
    ...
    beforeEach(function () {
      ...
      return expect(tabBar.element.scrollWidth).toBeGreaterThan(
        tabBar.element.clientWidth
      );
    });
    ...
    return it("scrolls to the item when it isn't completely visible", function () {
      ...
      pane.activateItem(item3);
      return expect(tabBar.element.scrollLeft).toBe(
  ...
  describe("when a tab item's title changes", () =>
    it("updates the title of the item's tab", function () {
      ...
      return expect(tabBar.tabForItem(editor1).element.textContent).toMatch(
        'test.txt'
      );
    }));
  ...
  describe('when two tabs have the same title', () =>
    it("displays the long title on the tab if it's available from the item", function () {
      ...
      expect(tabBar.tabForItem(item1).element.textContent).toMatch(
        'Grumpy Old Man'
      );
      expect(tabBar.tabForItem(item2).element.textContent).toMatch(
        'Jolly Old Man'
      );
  ...
  describe('when an item has an icon defined', function () {
    ...
    it('hides the icon from the tab if the icon is removed', function () {
      ...
      expect(
        tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
      ).not.toHaveClass('icon');
      return expect(
        tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
      ).not.toHaveClass('icon-squirrel');
    });
  ...
  describe('when a pane item moves to a new index', function () {
    ...
    describe('when addNewTabsAtEnd is set to true in package settings', () =>
      it('updates the order of the tabs to match the new item order', function () {
        ...
        return expect(
          tabBar.getTabs().map((tab) => tab.element.textContent)
  ...
  describe('context menu commands', function () {
    ...
    describe('when tabs:split-up is fired', () =>
      it('splits the selected tab up', function () {
        ...
        return expect(
          atom.workspace.getCenter().getPanes()[0].getItems()[0].getTitle()
        ).toBe(item2.getTitle());
      }));
    ...
    describe('when tabs:split-down is fired', () =>
      it('splits the selected tab down', function () {
        ...
        return expect(
          atom.workspace.getCenter().getPanes()[1].getItems()[0].getTitle()
        ).toBe(item2.getTitle());
      }));
  ...
  describe('dragging and dropping tabs', function () {
    ...
    describe('when a tab is dragged to a different pane', function () {
      ...
      describe('when the tab is dragged to an empty pane', () =>
        it('removes the tab and item from their original pane and moves them to the target pane', function () {
          ...
          expect(
            tabBar2.getTabs().map((tab) => tab.element.textContent)
          ).toEqual([]);
      ...
      describe('when alwaysShowTabBar is set to true in package settings', () =>
        it('always shows the tab bar in the new pane', function () {
    ...
    describe('when a tab is dragged over a pane item', function () {
      ...
      it('cleaves the pane in two', function () {
        ...
        return expect(atom.workspace.getActivePane().getItems().length).toEqual(
          1
        );
      });
      ...
      return describe('when the tab is not allowed in that pane', () =>
        it('does not move the tab, nor does it create a split', function () {
          layout.test = {
            pane,
            itemView: pane.getElement().querySelector('.item-views'),
            rect: { top: 0, left: 0, width: 100, height: 100 },
  ...
  describe('when the mouse wheel is used on the tab bar', function () {
    describe('when tabScrolling is true in package settings', function () {
      ...
      return describe('when the tabScrolling is changed to false', () =>
        it('does not change the active tab when scrolling', function () {
  ...
  describe('when alwaysShowTabBar is true in package settings', function () {
    ...
    return describe('when only one tab is open', () =>
      it('shows the tab bar', function () {
        expect(pane.getItems().length).toBe(3);
  ...
  describe('when alwaysShowTabBar is false in package settings', function () {
    ...
    return describe('when there are multiple panes', () =>
      it('hides each tab bar separately', function () {
        const item3 = new TestView('Item 3');
    ...
    describe("when tab's pane item is pending", function () {
      ...
      describe('when opening a new tab', () =>
        it("adds tab with class 'temp'", function () {
          ...
          waitsForPromise(() =>
            atom.workspace
              .open('sample.txt', { pending: true })
              .then((o) => (editor1 = o))
          );

          return runs(function () {
            pane.activateItem(editor1);
            expect(tabBar.element.querySelectorAll('.tab .temp').length).toBe(
              1
            );
            return expect(
              tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
      ...
      describe('when there is a temp tab already', function () {
        it('it will replace an existing temporary tab', function () {
          ...
          return runs(function () {
            ...
            return expect(
              tabBar.tabForItem(editor2).element.querySelector('.title')
            ).toHaveClass('temp');
          });
        });
      ...
      describe('when editing a file in pending state', () =>
        it('makes the item and tab permanent', function () {
          ...
          return runs(() =>
            expect(
              tabBar.tabForItem(editor1).element.querySelector('.title')
            ).not.toHaveClass('temp')
          );
        }));
      ...
      describe('when saving a file', () =>
        it('makes the tab permanent', function () {
          ...
          waitsForPromise(() =>
            atom.workspace
              .open(path.join(temp.mkdirSync('tabs-'), 'sample.txt'), {
                pending: true,
              })
              .then(function (o) {
                editor1 = o;
      ...
      describe('when splitting a pending tab', function () {
        ...
        it('makes the tab permanent in the new pane', function () {
          ...
          return expect(
            tabBar2.tabForItem(newEditor).element.querySelector('.title')
          ).not.toHaveClass('temp');
        });
      ...
      return describe('when dragging a pending tab to a different pane', () =>
        it('makes the tab permanent in the other pane', function () {
          ...
          return runs(function () {
            ...
            return expect(
              tabBar2
                .tabForItem(pane2.getActiveItem())
  ...
  return describe('integration with version control systems', function () {
    ...
    beforeEach(function () {
      ...
      repository.isStatusModified.andCallFake(
      ...
      repository.onDidChangeStatus = function (callback) {
        ...
        return {
          dispose: () => _.remove(this.changeStatusCallbacks, callback),
        };
      };
      repository.emitDidChangeStatus = function (event) {
        return (
          this.changeStatusCallbacks != null ? this.changeStatusCallbacks : []
      ...
      repository.onDidChangeStatuses = function (callback) {
        ...
        return {
          dispose: () => _.remove(this.changeStatusesCallbacks, callback),
        };
      };
      repository.emitDidChangeStatuses = function (event) {
        return (
          this.changeStatusesCallbacks != null
            ? this.changeStatusesCallbacks
            : []
        ).map((callback) => callback(event));
      };
      ...
      spyOn(atom.project, 'repositoryForDirectory').andReturn(
    ...
    describe('when working inside a VCS repository', function () {
      ...
      it('adds custom style for ignored items', function () {
        ...
        return expect(
          tabBar.element.querySelectorAll('.tab')[1].querySelector('.title')
        ).toHaveClass('status-ignored');
      });

      return it('does not add any styles for items not in the repository', function () {
        expect(
          tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
        ).not.toHaveClass('status-added');
        expect(
          tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
        ).not.toHaveClass('status-modified');
        return expect(
          tabBar.element.querySelectorAll('.tab')[0].querySelector('.title')
        ).not.toHaveClass('status-ignored');
      });
    });
