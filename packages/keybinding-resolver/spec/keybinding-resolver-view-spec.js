...
describe('KeyBindingResolverView', () => {
  ...
  describe('when the key-binding-resolver:toggle event is triggered', () => {
    it('toggles the view', async () => {
      ...
      expect(
        bottomDockElement.querySelector('.key-binding-resolver')
      ).not.toExist();
  ...
  describe('capturing keybinding events', () => {
    it('captures events when the keybinding resolver is visible', async () => {
      ...
      const keybindingResolverView = atom.workspace
        .getBottomDock()
        .getActivePaneItem();
      expect(keybindingResolverView.keybindingDisposables).not.toBe(null);

      document.dispatchEvent(
        atom.keymaps.constructor.buildKeydownEvent('x', {
          target: bottomDockElement,
        })
      );
      await etch.getScheduler().getNextUpdatePromise();
      expect(
        bottomDockElement.querySelector('.key-binding-resolver .keystroke')
          .textContent
      ).toBe('x');
    });
  ...
  describe('when a keydown event occurs', () => {
    it('displays all commands for the keydown event but does not clear for the keyup when there is no keyup binding', async () => {
      ...
      expect(
        bottomDockElement.querySelectorAll('.key-binding-resolver .used')
      ).toHaveLength(1);
      expect(
        bottomDockElement.querySelectorAll('.key-binding-resolver .unused')
      ).toHaveLength(1);
      expect(
        bottomDockElement.querySelectorAll('.key-binding-resolver .unmatched')
      ).toHaveLength(1);

      // It should not render the keyup event data because there is no match
      spyOn(etch.getScheduler(), 'updateDocument').andCallThrough();
      document.dispatchEvent(
        atom.keymaps.constructor.buildKeyupEvent('x', {
          target: bottomDockElement,
        })
      );
    ...
    it('displays all commands for the keydown event but does not clear for the keyup when there is no keyup binding', async () => {
      ...
      document.dispatchEvent(
        atom.keymaps.constructor.buildKeydownEvent('a', {
          target: bottomDockElement,
        })
      );
      await etch.getScheduler().getNextUpdatePromise();
      ...
      document.dispatchEvent(
        atom.keymaps.constructor.buildKeyupEvent('a', {
          target: bottomDockElement,
        })
      );
      await etch.getScheduler().getNextUpdatePromise();
