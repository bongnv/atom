...
export default class KeyBindingResolverView {
  constructor() {
    ...
    this.disposables.add(
      atom.workspace.getBottomDock().observeVisible((visible) => {
        if (visible) {
          if (atom.workspace.getBottomDock().getActivePaneItem() === this)
            this.attach();
        } else {
          this.detach();
        }
  ...
  attach() {
    ...
    this.keybindingDisposables.add(
      atom.keymaps.onDidPartiallyMatchBindings(
        ({ keystrokes, partiallyMatchedBindings }) => {
    ...
    this.keybindingDisposables.add(
      atom.keymaps.onDidFailToMatchBinding(
        ({ keystrokes, keyboardEventTarget, eventType }) => {
          if (eventType === 'keyup') {
            return;
          }
  ...
  renderKeystrokes() {
    if (this.keystrokes) {
      if (this.partiallyMatchedBindings.length > 0) {
        return (
          <span className="keystroke highlight-info">
            {this.keystrokes} (partial)
          </span>
        );
      } else {
        return (
          <span className="keystroke highlight-info">{this.keystrokes}</span>
        );
      }
    } else {
        ...
        usedKeyBinding = (
          <tr className="used">
            ...
            <td
              className="source"
              onClick={() =>
                this.openKeybindingFile(this.usedKeyBinding.source)
              }
            >
