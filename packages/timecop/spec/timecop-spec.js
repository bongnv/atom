...
describe('Timecop', () => {
  ...
  describe('the Timecop view', () => {
    ...
    it('shows the packages that loaded slowly', () => {
      ...
      expect(loadingPanel.element.textContent).toMatch(
        /1 package took longer than 5ms to load/
      );
      expect(loadingPanel.element.textContent).toMatch(/slow-loading-package/);

      expect(loadingPanel.element.textContent).not.toMatch(
    ...
    it('shows the packages that activated slowly', () => {
      ...
      expect(activationPanel.element.textContent).toMatch(
        /slow-activating-package-1/
      );
      expect(activationPanel.element.textContent).toMatch(
        /slow-activating-package-2/
      );

      expect(activationPanel.element.textContent).not.toMatch(
        /slow-loading-package/
      );
    ...
    it('shows how many files were transpiled from each language', () => {
      ...
      expect(cachePanel.element.textContent).toMatch(
        /CoffeeScript files compiled\s*8/
      );
      expect(cachePanel.element.textContent).toMatch(
        /Babel files compiled\s*4/
      );
      expect(cachePanel.element.textContent).toMatch(
        /Typescript files compiled\s*6/
      );
      expect(cachePanel.element.textContent).toMatch(
        /CSON files compiled\s*10/
      );
      expect(cachePanel.element.textContent).toMatch(
        /Less files compiled\s*12/
      );
    });
  });
});
